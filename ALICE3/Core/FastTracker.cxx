...
int FastTracker::FastTrack(o2::track::TrackParCov inputTrack, o2::track::TrackParCov& outputTrack, const float nch)
{
  ...
  for (uint32_t il = 0; il < layers.size(); il++) {
    ...
    if (targetX > 999.f) {
      LOGF(info, "Failed to find intercept for layer %d at radius %.2f cm", il, layers[il].getRadius());
    ...
    LOG(info) << "Propagation was " << (ok ? "successful" : "unsuccessful") << " up to layer " << il << " " << layers[il];
    ...
    if (firstLayerReached < 0) {
      LOGF(info, "First layer reached: %d", il);
