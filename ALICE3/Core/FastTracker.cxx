...
void FastTracker::Print()
{
  ...
  for(int il=0; il<layers.size(); il++){ 
    LOG(info) <<" Layer #"<<il<<"\t"<<layers[il].name.Data()<<"\tr = "<<Form("%.2f", layers[il].r)<<"cm\tz = "<<layers[il].z<<"\t"<<"x0 = "<<layers[il].x0<<"\txrho = "<<layers[il].xrho<<"\tresRPhi = "<<layers[il].resRPhi<<"\tresZ = "<<layers[il].resZ<<"\teff = "<<layers[il].eff;
  }
  LOG(info) << "+-~-<*>-~-+-~-<*>-~-+-~-<*>-~-+-~-<*>-~-+-~-<*>-~-+-~-<*>-~-+-~-<*>-~-+";
}
...
int FastTracker::FastTrack(o2::track::TrackParCov inputTrack, o2::track::TrackParCov& outputTrack)
{
    ...
    for (int ic=15;ic--;) largeCov[ic] = 0.;
    ...
    if(nIntercepts<4) return nIntercepts;
    ...
    for(int ii=0; ii<5; ii++){
      for(int jj=0; jj<5; jj++){ 
        if (ii==jj) continue; //don't evaluate diagonals 
