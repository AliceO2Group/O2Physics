...
struct HmpidQaPp {
  ...
  void process(aod::HmpidAnalysisPp const& hmpidtable)
  {
    // photocathods limits
    ...
    for (const auto& hmpid : hmpidtable) // loop on tracks contained in the table
    {
      ...
      if (hmpid.chAngle() > 0 && distance2D(hmpid.xTrack(), hmpid.yTrack(), hmpid.xMip(), hmpid.yMip()) < cutDistanceMipTrack && hmpid.chargeMip() > cutQmip) {
      ...
      if (distance2D(hmpid.xTrack(), hmpid.yTrack(), hmpid.xMip(), hmpid.yMip()) > maxDistanceForProb || hmpid.chargeMip() < cutQmip)
        continue;
...
WorkflowSpec defineDataProcessing(ConfigContext const& cfg) { return WorkflowSpec{adaptAnalysisTask<HmpidQaPp>(cfg)}; }
