...
Double_t expectedSignal(double mass, float mom)
{
  ...
  if(cos_chAngle_th > 1) return chAngle_th;
...
void getProbability(float hmpidSignal, float hmpidMomentum, double *probs)
{
  ...
  if(hmpidSignal<=0)
  {
    ...
    for(Int_t iPart=0;iPart<nSpecies;iPart++) probs[iPart]=1.0/(Float_t)nSpecies;
    return;
  } 
...
*/
  ...
  for(Int_t iPart=0;iPart<nSpecies;iPart++){                                                              // for each particle
    ...
    Double_t thetaCerTh = expectedSignal(mass[iPart], hmpidMomentum);;                                    // theoretical Theta Cherenkov
    ...
    if(TMath::Abs(hmpidSignal-thetaCerTh)<4*sigmaRing) desert = kFALSE;                                                               
  ...
  for(Int_t iPart=0;iPart<nSpecies;iPart++) {                                                             // species loop to assign probabilities
    ...
    if(!desert) probs[iPart]=h[iPart]/hTot;
    else        probs[iPart]=1.0/(Float_t)nSpecies;                                                        // all theoretical values are far away from experemental one
    ...
    for (auto& hmpid : hmpidtable) //loop on tracks contained in the table
    {
      ...
      if(distance2D(hmpid.xtrack(), hmpid.ytrack(), hmpid.xmip(), hmpid.ymip()) < cut_d_mip_track && hmpid.chargeMIP() > 120.)
      {
        ...
        sin2changle = (float)TMath::Power(TMath::Sin(hmpid.chAngle()),2);
	      if(hmpid.xmip() <= 100. && hmpid.xmip() >= 40. && hmpid.ymip() <= 100. && hmpid.ymip() >= 40.) {histos.fill(HIST("nPhotons_vs_sin2Ch"), sin2changle, hmpid.nphotons());}
      ...
      if (hmpid.chamber() == 0) 
      {
        ...
        if(distance2D(hmpid.xtrack(), hmpid.ytrack(), hmpid.xmip(), hmpid.ymip()) < cut_d_mip_track) {histos.fill(HIST("hmpidQMip0"), hmpid.chargeMIP());}
        ...
        if(distance2D(hmpid.xtrack(), hmpid.ytrack(), hmpid.xmip(), hmpid.ymip()) < cut_d_mip_track && hmpid.chargeMIP() > 120.)
        {
          ...
          sin2changle = (float)TMath::Power(TMath::Sin(hmpid.chAngle()),2);
          if(hmpid.xmip() <= 100. && hmpid.xmip() >= 40. && hmpid.ymip() <= 100. && hmpid.ymip() >= 40.) {histos.fill(HIST("nPhotons_vs_sin2Ch0"), sin2changle, hmpid.nphotons());}
        ...
        if(fParam->inHVSector(hmpid.ymip()) == 0)
        {
          ...
          if(distance2D(hmpid.xtrack(), hmpid.ytrack(), hmpid.xmip(), hmpid.ymip()) < cut_d_mip_track) {histos.fill(HIST("hmpidQMip_RICH0_HV0"), hmpid.chargeMIP());}
        ...
        if(fParam->inHVSector(hmpid.ymip()) == 1)
        {
          ...
          if(distance2D(hmpid.xtrack(), hmpid.ytrack(), hmpid.xmip(), hmpid.ymip()) < cut_d_mip_track) {histos.fill(HIST("hmpidQMip_RICH0_HV1"), hmpid.chargeMIP());}
          if(distance2D(hmpid.xtrack(), hmpid.ytrack(), hmpid.xmip(), hmpid.ymip()) < cut_d_mip_track && hmpid.chargeMIP() > 120. && hmpid.chAngle()>0)
          {
            for (int i = 0; i < 10; i++) 
            {
        ...
        if(fParam->inHVSector(hmpid.ymip()) == 2)
        {
          ...
          if(distance2D(hmpid.xtrack(), hmpid.ytrack(), hmpid.xmip(), hmpid.ymip()) < cut_d_mip_track) {histos.fill(HIST("hmpidQMip_RICH0_HV2"), hmpid.chargeMIP());}
          if(distance2D(hmpid.xtrack(), hmpid.ytrack(), hmpid.xmip(), hmpid.ymip()) < cut_d_mip_track && hmpid.chargeMIP() > 120. && hmpid.chAngle()>0)
          {
            for (int i = 0; i < 10; i++) 
            {
        ...
        if(fParam->inHVSector(hmpid.ymip()) == 3)
        {
          ...
          if(distance2D(hmpid.xtrack(), hmpid.ytrack(), hmpid.xmip(), hmpid.ymip()) < cut_d_mip_track) {histos.fill(HIST("hmpidQMip_RICH0_HV3"), hmpid.chargeMIP());}
          if(distance2D(hmpid.xtrack(), hmpid.ytrack(), hmpid.xmip(), hmpid.ymip()) < cut_d_mip_track && hmpid.chargeMIP() > 120. && hmpid.chAngle()>0)
          {
            for (int i = 0; i < 10; i++) 
            {
        ...
        if(fParam->inHVSector(hmpid.ymip()) == 4)
        {
          ...
          if(distance2D(hmpid.xtrack(), hmpid.ytrack(), hmpid.xmip(), hmpid.ymip()) < cut_d_mip_track) {histos.fill(HIST("hmpidQMip_RICH0_HV4"), hmpid.chargeMIP());}
          if(distance2D(hmpid.xtrack(), hmpid.ytrack(), hmpid.xmip(), hmpid.ymip()) < cut_d_mip_track && hmpid.chargeMIP() > 120. && hmpid.chAngle()>0)
          {
            for (int i = 0; i < 10; i++) 
            {
        ...
        if(fParam->inHVSector(hmpid.ymip()) == 5)
        {
          ...
          if(distance2D(hmpid.xtrack(), hmpid.ytrack(), hmpid.xmip(), hmpid.ymip()) < cut_d_mip_track) {histos.fill(HIST("hmpidQMip_RICH0_HV5"), hmpid.chargeMIP());}
          if(distance2D(hmpid.xtrack(), hmpid.ytrack(), hmpid.xmip(), hmpid.ymip()) < cut_d_mip_track && hmpid.chargeMIP() > 120. && hmpid.chAngle()>0)
          {
            for (int i = 0; i < 10; i++) 
            {
      ...
      if (hmpid.chamber() == 1) 
      {
        ...
        if(distance2D(hmpid.xtrack(), hmpid.ytrack(), hmpid.xmip(), hmpid.ymip()) < cut_d_mip_track) {histos.fill(HIST("hmpidQMip1"), hmpid.chargeMIP());}
        ...
        if(distance2D(hmpid.xtrack(), hmpid.ytrack(), hmpid.xmip(), hmpid.ymip()) < cut_d_mip_track && hmpid.chargeMIP() > 120.)
        {
          ...
          sin2changle = (float)TMath::Power(TMath::Sin(hmpid.chAngle()),2);
          if(hmpid.xmip() <= 100. && hmpid.xmip() >= 40. && hmpid.ymip() <= 100. && hmpid.ymip() >= 40.){histos.fill(HIST("nPhotons_vs_sin2Ch1"), sin2changle, hmpid.nphotons());}
        ...
        if(fParam->inHVSector(hmpid.ymip()) == 0)
        {
          if(distance2D(hmpid.xtrack(), hmpid.ytrack(), hmpid.xmip(), hmpid.ymip()) < cut_d_mip_track) {histos.fill(HIST("hmpidQMip_RICH1_HV0"), hmpid.chargeMIP());}
          if(distance2D(hmpid.xtrack(), hmpid.ytrack(), hmpid.xmip(), hmpid.ymip()) < cut_d_mip_track && hmpid.chargeMIP() > 120. && hmpid.chAngle()>0)
          {
            for (int i = 0; i < 10; i++) 
            {
        ...
        if(fParam->inHVSector(hmpid.ymip()) == 1)
        {
          if(distance2D(hmpid.xtrack(), hmpid.ytrack(), hmpid.xmip(), hmpid.ymip()) < cut_d_mip_track) {histos.fill(HIST("hmpidQMip_RICH1_HV1"), hmpid.chargeMIP());}
          if(distance2D(hmpid.xtrack(), hmpid.ytrack(), hmpid.xmip(), hmpid.ymip()) < cut_d_mip_track && hmpid.chargeMIP() > 120. && hmpid.chAngle()>0)
          {
            for (int i = 0; i < 10; i++) 
            {
        ...
        if(fParam->inHVSector(hmpid.ymip()) == 2)
        {
          if(distance2D(hmpid.xtrack(), hmpid.ytrack(), hmpid.xmip(), hmpid.ymip()) < cut_d_mip_track) {histos.fill(HIST("hmpidQMip_RICH1_HV2"), hmpid.chargeMIP());}
          if(distance2D(hmpid.xtrack(), hmpid.ytrack(), hmpid.xmip(), hmpid.ymip()) < cut_d_mip_track && hmpid.chargeMIP() > 120. && hmpid.chAngle()>0)
          {
            for (int i = 0; i < 10; i++) 
            {
        ...
        if(fParam->inHVSector(hmpid.ymip()) == 3)
        {
          if(distance2D(hmpid.xtrack(), hmpid.ytrack(), hmpid.xmip(), hmpid.ymip()) < cut_d_mip_track) {histos.fill(HIST("hmpidQMip_RICH1_HV3"), hmpid.chargeMIP());}
          if(distance2D(hmpid.xtrack(), hmpid.ytrack(), hmpid.xmip(), hmpid.ymip()) < cut_d_mip_track && hmpid.chargeMIP() > 120. && hmpid.chAngle()>0)
          {
            for (int i = 0; i < 10; i++) 
            {
        ...
        if(fParam->inHVSector(hmpid.ymip()) == 4)
        {
          if(distance2D(hmpid.xtrack(), hmpid.ytrack(), hmpid.xmip(), hmpid.ymip()) < cut_d_mip_track) {histos.fill(HIST("hmpidQMip_RICH1_HV4"), hmpid.chargeMIP());}
          if(distance2D(hmpid.xtrack(), hmpid.ytrack(), hmpid.xmip(), hmpid.ymip()) < cut_d_mip_track && hmpid.chargeMIP() > 120. && hmpid.chAngle()>0)
          {
            for (int i = 0; i < 10; i++) 
            {
        ...
        if(fParam->inHVSector(hmpid.ymip()) == 5)
        {
          if(distance2D(hmpid.xtrack(), hmpid.ytrack(), hmpid.xmip(), hmpid.ymip()) < cut_d_mip_track) {histos.fill(HIST("hmpidQMip_RICH1_HV5"), hmpid.chargeMIP());}
          if(distance2D(hmpid.xtrack(), hmpid.ytrack(), hmpid.xmip(), hmpid.ymip()) < cut_d_mip_track && hmpid.chargeMIP() > 120. && hmpid.chAngle()>0)
          {
            for (int i = 0; i < 10; i++) 
            {
              if (hmpid.photons_charge()[i] > 0)
              {histos.fill(HIST("hmpidPhotsCharge_RICH1_HV5"), hmpid.photons_charge()[i]);} //hmpidPhotsCharge_RICH%i_HV%i
            }
      ...
      if (hmpid.chamber() == 2) 
      {
        ...
        if(distance2D(hmpid.xtrack(), hmpid.ytrack(), hmpid.xmip(), hmpid.ymip()) < cut_d_mip_track) {histos.fill(HIST("hmpidQMip2"), hmpid.chargeMIP());}
        ...
        if(distance2D(hmpid.xtrack(), hmpid.ytrack(), hmpid.xmip(), hmpid.ymip()) < cut_d_mip_track && hmpid.chargeMIP() > 120.)
        {
          ...
          sin2changle = (float)TMath::Power(TMath::Sin(hmpid.chAngle()),2);
          if(hmpid.xmip() <= 100. && hmpid.xmip() >= 40. && hmpid.ymip() <= 100. && hmpid.ymip() >= 40.){histos.fill(HIST("nPhotons_vs_sin2Ch2"), sin2changle, hmpid.nphotons());}
        ...
        if(fParam->inHVSector(hmpid.ymip()) == 0)
        {
          if(distance2D(hmpid.xtrack(), hmpid.ytrack(), hmpid.xmip(), hmpid.ymip()) < cut_d_mip_track) {histos.fill(HIST("hmpidQMip_RICH2_HV0"), hmpid.chargeMIP());}
          if(distance2D(hmpid.xtrack(), hmpid.ytrack(), hmpid.xmip(), hmpid.ymip()) < cut_d_mip_track && hmpid.chargeMIP() > 120. && hmpid.chAngle()>0)
          {
            for (int i = 0; i < 10; i++) 
            {
        ...
        if(fParam->inHVSector(hmpid.ymip()) == 1)
        {
          if(distance2D(hmpid.xtrack(), hmpid.ytrack(), hmpid.xmip(), hmpid.ymip()) < cut_d_mip_track) {histos.fill(HIST("hmpidQMip_RICH2_HV1"), hmpid.chargeMIP());}
          if(distance2D(hmpid.xtrack(), hmpid.ytrack(), hmpid.xmip(), hmpid.ymip()) < cut_d_mip_track && hmpid.chargeMIP() > 120. && hmpid.chAngle()>0)
          {
            for (int i = 0; i < 10; i++) 
            {
        ...
        if(fParam->inHVSector(hmpid.ymip()) == 2)
        {
          if(distance2D(hmpid.xtrack(), hmpid.ytrack(), hmpid.xmip(), hmpid.ymip()) < cut_d_mip_track) {histos.fill(HIST("hmpidQMip_RICH2_HV2"), hmpid.chargeMIP());}
          if(distance2D(hmpid.xtrack(), hmpid.ytrack(), hmpid.xmip(), hmpid.ymip()) < cut_d_mip_track && hmpid.chargeMIP() > 120. && hmpid.chAngle()>0)
          {
            for (int i = 0; i < 10; i++) 
            {
        ...
        if(fParam->inHVSector(hmpid.ymip()) == 3)
        {
          if(distance2D(hmpid.xtrack(), hmpid.ytrack(), hmpid.xmip(), hmpid.ymip()) < cut_d_mip_track) {histos.fill(HIST("hmpidQMip_RICH2_HV3"), hmpid.chargeMIP());}
          if(distance2D(hmpid.xtrack(), hmpid.ytrack(), hmpid.xmip(), hmpid.ymip()) < cut_d_mip_track && hmpid.chargeMIP() > 120. && hmpid.chAngle()>0)
          {
            for (int i = 0; i < 10; i++) 
            {
        ...
        if(fParam->inHVSector(hmpid.ymip()) == 4)
        {
          if(distance2D(hmpid.xtrack(), hmpid.ytrack(), hmpid.xmip(), hmpid.ymip()) < cut_d_mip_track) {histos.fill(HIST("hmpidQMip_RICH2_HV4"), hmpid.chargeMIP());}
          if(distance2D(hmpid.xtrack(), hmpid.ytrack(), hmpid.xmip(), hmpid.ymip()) < cut_d_mip_track && hmpid.chargeMIP() > 120. && hmpid.chAngle()>0)
          {
            for (int i = 0; i < 10; i++) 
            {
        ...
        if(fParam->inHVSector(hmpid.ymip()) == 5)
        {
          if(distance2D(hmpid.xtrack(), hmpid.ytrack(), hmpid.xmip(), hmpid.ymip()) < cut_d_mip_track) {histos.fill(HIST("hmpidQMip_RICH2_HV5"), hmpid.chargeMIP());}
          if(distance2D(hmpid.xtrack(), hmpid.ytrack(), hmpid.xmip(), hmpid.ymip()) < cut_d_mip_track && hmpid.chargeMIP() > 120. && hmpid.chAngle()>0)
          {
            for (int i = 0; i < 10; i++) 
            {
      ...
      if (hmpid.chamber() == 3) 
      {
        ...
        if(distance2D(hmpid.xtrack(), hmpid.ytrack(), hmpid.xmip(), hmpid.ymip()) < cut_d_mip_track) {histos.fill(HIST("hmpidQMip3"), hmpid.chargeMIP());}
        ...
        if(distance2D(hmpid.xtrack(), hmpid.ytrack(), hmpid.xmip(), hmpid.ymip()) < cut_d_mip_track && hmpid.chargeMIP() > 120.)
        {
          ...
          sin2changle = (float)TMath::Power(TMath::Sin(hmpid.chAngle()),2);
          if(hmpid.xmip() <= 100. && hmpid.xmip() >= 40. && hmpid.ymip() <= 100. && hmpid.ymip() >= 40.){histos.fill(HIST("nPhotons_vs_sin2Ch3"), sin2changle, hmpid.nphotons());}
        ...
        if(fParam->inHVSector(hmpid.ymip()) == 0)
        {
          if(distance2D(hmpid.xtrack(), hmpid.ytrack(), hmpid.xmip(), hmpid.ymip()) < cut_d_mip_track) {histos.fill(HIST("hmpidQMip_RICH3_HV0"), hmpid.chargeMIP());}
          if(distance2D(hmpid.xtrack(), hmpid.ytrack(), hmpid.xmip(), hmpid.ymip()) < cut_d_mip_track && hmpid.chargeMIP() > 120. && hmpid.chAngle()>0)
          {
            for (int i = 0; i < 10; i++) 
            {
        ...
        if(fParam->inHVSector(hmpid.ymip()) == 1)
        {
          if(distance2D(hmpid.xtrack(), hmpid.ytrack(), hmpid.xmip(), hmpid.ymip()) < cut_d_mip_track) {histos.fill(HIST("hmpidQMip_RICH3_HV1"), hmpid.chargeMIP());}
          if(distance2D(hmpid.xtrack(), hmpid.ytrack(), hmpid.xmip(), hmpid.ymip()) < cut_d_mip_track && hmpid.chargeMIP() > 120. && hmpid.chAngle()>0)
          {
            for (int i = 0; i < 10; i++) 
            {
        ...
        if(fParam->inHVSector(hmpid.ymip()) == 2)
        {
          if(distance2D(hmpid.xtrack(), hmpid.ytrack(), hmpid.xmip(), hmpid.ymip()) < cut_d_mip_track) {histos.fill(HIST("hmpidQMip_RICH3_HV2"), hmpid.chargeMIP());}
          if(distance2D(hmpid.xtrack(), hmpid.ytrack(), hmpid.xmip(), hmpid.ymip()) < cut_d_mip_track && hmpid.chargeMIP() > 120. && hmpid.chAngle()>0)
          {
            for (int i = 0; i < 10; i++) 
            {
        ...
        if(fParam->inHVSector(hmpid.ymip()) == 3)
        {
          if(distance2D(hmpid.xtrack(), hmpid.ytrack(), hmpid.xmip(), hmpid.ymip()) < cut_d_mip_track) {histos.fill(HIST("hmpidQMip_RICH3_HV3"), hmpid.chargeMIP());}
          if(distance2D(hmpid.xtrack(), hmpid.ytrack(), hmpid.xmip(), hmpid.ymip()) < cut_d_mip_track && hmpid.chargeMIP() > 120. && hmpid.chAngle()>0)
          {
            for (int i = 0; i < 10; i++) 
            {
        ...
        if(fParam->inHVSector(hmpid.ymip()) == 4)
        {
          if(distance2D(hmpid.xtrack(), hmpid.ytrack(), hmpid.xmip(), hmpid.ymip()) < cut_d_mip_track) {histos.fill(HIST("hmpidQMip_RICH3_HV4"), hmpid.chargeMIP());}
          if(distance2D(hmpid.xtrack(), hmpid.ytrack(), hmpid.xmip(), hmpid.ymip()) < cut_d_mip_track && hmpid.chargeMIP() > 120. && hmpid.chAngle()>0)
          {
            for (int i = 0; i < 10; i++) 
            {
        ...
        if(fParam->inHVSector(hmpid.ymip()) == 5)
        {
          if(distance2D(hmpid.xtrack(), hmpid.ytrack(), hmpid.xmip(), hmpid.ymip()) < cut_d_mip_track) {histos.fill(HIST("hmpidQMip_RICH3_HV5"), hmpid.chargeMIP());}
          if(distance2D(hmpid.xtrack(), hmpid.ytrack(), hmpid.xmip(), hmpid.ymip()) < cut_d_mip_track && hmpid.chargeMIP() > 120. && hmpid.chAngle()>0)
          {
            for (int i = 0; i < 10; i++) 
            {
      ...
      if (hmpid.chamber() == 4) 
      {
        ...
        if(distance2D(hmpid.xtrack(), hmpid.ytrack(), hmpid.xmip(), hmpid.ymip()) < cut_d_mip_track) {histos.fill(HIST("hmpidQMip4"), hmpid.chargeMIP());}
        ...
        if(distance2D(hmpid.xtrack(), hmpid.ytrack(), hmpid.xmip(), hmpid.ymip()) < cut_d_mip_track && hmpid.chargeMIP() > 120.)
        {
          ...
          sin2changle = (float)TMath::Power(TMath::Sin(hmpid.chAngle()),2);
          if(hmpid.xmip() <= 100. && hmpid.xmip() >= 40. && hmpid.ymip() <= 100. && hmpid.ymip() >= 40.){histos.fill(HIST("nPhotons_vs_sin2Ch4"), sin2changle, hmpid.nphotons());}
        ...
        if(fParam->inHVSector(hmpid.ymip()) == 0)
        {
          if(distance2D(hmpid.xtrack(), hmpid.ytrack(), hmpid.xmip(), hmpid.ymip()) < cut_d_mip_track) {histos.fill(HIST("hmpidQMip_RICH4_HV0"), hmpid.chargeMIP());}
          if(distance2D(hmpid.xtrack(), hmpid.ytrack(), hmpid.xmip(), hmpid.ymip()) < cut_d_mip_track && hmpid.chargeMIP() > 120. && hmpid.chAngle()>0)
          {
            for (int i = 0; i < 10; i++) 
            {
        ...
        if(fParam->inHVSector(hmpid.ymip()) == 1)
        {
          if(distance2D(hmpid.xtrack(), hmpid.ytrack(), hmpid.xmip(), hmpid.ymip()) < cut_d_mip_track) {histos.fill(HIST("hmpidQMip_RICH4_HV1"), hmpid.chargeMIP());}
          if(distance2D(hmpid.xtrack(), hmpid.ytrack(), hmpid.xmip(), hmpid.ymip()) < cut_d_mip_track && hmpid.chargeMIP() > 120. && hmpid.chAngle()>0)
          {
            for (int i = 0; i < 10; i++) 
            {
        ...
        if(fParam->inHVSector(hmpid.ymip()) == 2)
        {
          if(distance2D(hmpid.xtrack(), hmpid.ytrack(), hmpid.xmip(), hmpid.ymip()) < cut_d_mip_track) {histos.fill(HIST("hmpidQMip_RICH4_HV2"), hmpid.chargeMIP());}
          if(distance2D(hmpid.xtrack(), hmpid.ytrack(), hmpid.xmip(), hmpid.ymip()) < cut_d_mip_track && hmpid.chargeMIP() > 120. && hmpid.chAngle()>0)
          {
            for (int i = 0; i < 10; i++) 
            {
        ...
        if(fParam->inHVSector(hmpid.ymip()) == 3)
        {
          if(distance2D(hmpid.xtrack(), hmpid.ytrack(), hmpid.xmip(), hmpid.ymip()) < cut_d_mip_track) {histos.fill(HIST("hmpidQMip_RICH4_HV3"), hmpid.chargeMIP());}
          if(distance2D(hmpid.xtrack(), hmpid.ytrack(), hmpid.xmip(), hmpid.ymip()) < cut_d_mip_track && hmpid.chargeMIP() > 120. && hmpid.chAngle()>0)
          {
            for (int i = 0; i < 10; i++) 
            {
        ...
        if(fParam->inHVSector(hmpid.ymip()) == 4)
        {
          if(distance2D(hmpid.xtrack(), hmpid.ytrack(), hmpid.xmip(), hmpid.ymip()) < cut_d_mip_track) {histos.fill(HIST("hmpidQMip_RICH4_HV4"), hmpid.chargeMIP());}
          if(distance2D(hmpid.xtrack(), hmpid.ytrack(), hmpid.xmip(), hmpid.ymip()) < cut_d_mip_track && hmpid.chargeMIP() > 120. && hmpid.chAngle()>0)
          {
            for (int i = 0; i < 10; i++) 
            {
        ...
        if(fParam->inHVSector(hmpid.ymip()) == 5)
        {
          if(distance2D(hmpid.xtrack(), hmpid.ytrack(), hmpid.xmip(), hmpid.ymip()) < cut_d_mip_track) {histos.fill(HIST("hmpidQMip_RICH4_HV5"), hmpid.chargeMIP());}
          if(distance2D(hmpid.xtrack(), hmpid.ytrack(), hmpid.xmip(), hmpid.ymip()) < cut_d_mip_track && hmpid.chargeMIP() > 120. && hmpid.chAngle()>0)
          {
            for (int i = 0; i < 10; i++) 
            {
      ...
      if (hmpid.chamber() == 5) 
      {
        ...
        if(distance2D(hmpid.xtrack(), hmpid.ytrack(), hmpid.xmip(), hmpid.ymip()) < cut_d_mip_track) {histos.fill(HIST("hmpidQMip5"), hmpid.chargeMIP());}
        ...
        if(distance2D(hmpid.xtrack(), hmpid.ytrack(), hmpid.xmip(), hmpid.ymip()) < cut_d_mip_track && hmpid.chargeMIP() > 120.)
        {
          ...
          sin2changle = (float)TMath::Power(TMath::Sin(hmpid.chAngle()),2);
          if(hmpid.xmip() <= 100. && hmpid.xmip() >= 40. && hmpid.ymip() <= 100. && hmpid.ymip() >= 40.){histos.fill(HIST("nPhotons_vs_sin2Ch5"), sin2changle, hmpid.nphotons());}
        ...
        if(fParam->inHVSector(hmpid.ymip()) == 0)
        {
          if(distance2D(hmpid.xtrack(), hmpid.ytrack(), hmpid.xmip(), hmpid.ymip()) < cut_d_mip_track) {histos.fill(HIST("hmpidQMip_RICH5_HV0"), hmpid.chargeMIP());}
          if(distance2D(hmpid.xtrack(), hmpid.ytrack(), hmpid.xmip(), hmpid.ymip()) < cut_d_mip_track && hmpid.chargeMIP() > 120. && hmpid.chAngle()>0)
          {
            for (int i = 0; i < 10; i++) 
            {
        ...
        if(fParam->inHVSector(hmpid.ymip()) == 1)
        {
          if(distance2D(hmpid.xtrack(), hmpid.ytrack(), hmpid.xmip(), hmpid.ymip()) < cut_d_mip_track) {histos.fill(HIST("hmpidQMip_RICH5_HV1"), hmpid.chargeMIP());}
          if(distance2D(hmpid.xtrack(), hmpid.ytrack(), hmpid.xmip(), hmpid.ymip()) < cut_d_mip_track && hmpid.chargeMIP() > 120. && hmpid.chAngle()>0)
          {
            for (int i = 0; i < 10; i++) 
            {
        ...
        if(fParam->inHVSector(hmpid.ymip()) == 2)
        {
          if(distance2D(hmpid.xtrack(), hmpid.ytrack(), hmpid.xmip(), hmpid.ymip()) < cut_d_mip_track) {histos.fill(HIST("hmpidQMip_RICH5_HV2"), hmpid.chargeMIP());}
          if(distance2D(hmpid.xtrack(), hmpid.ytrack(), hmpid.xmip(), hmpid.ymip()) < cut_d_mip_track && hmpid.chargeMIP() > 120. && hmpid.chAngle()>0)
          {
            for (int i = 0; i < 10; i++) 
            {
        ...
        if(fParam->inHVSector(hmpid.ymip()) == 3)
        {
          if(distance2D(hmpid.xtrack(), hmpid.ytrack(), hmpid.xmip(), hmpid.ymip()) < cut_d_mip_track) {histos.fill(HIST("hmpidQMip_RICH5_HV3"), hmpid.chargeMIP());}
          if(distance2D(hmpid.xtrack(), hmpid.ytrack(), hmpid.xmip(), hmpid.ymip()) < cut_d_mip_track && hmpid.chargeMIP() > 120. && hmpid.chAngle()>0)
          {
            for (int i = 0; i < 10; i++) 
            {
        ...
        if(fParam->inHVSector(hmpid.ymip()) == 4)
        {
          if(distance2D(hmpid.xtrack(), hmpid.ytrack(), hmpid.xmip(), hmpid.ymip()) < cut_d_mip_track) {histos.fill(HIST("hmpidQMip_RICH5_HV4"), hmpid.chargeMIP());}
          if(distance2D(hmpid.xtrack(), hmpid.ytrack(), hmpid.xmip(), hmpid.ymip()) < cut_d_mip_track && hmpid.chargeMIP() > 120. && hmpid.chAngle()>0)
          {
            for (int i = 0; i < 10; i++) 
            {
        ...
        if(fParam->inHVSector(hmpid.ymip()) == 5)
        {
          if(distance2D(hmpid.xtrack(), hmpid.ytrack(), hmpid.xmip(), hmpid.ymip()) < cut_d_mip_track) {histos.fill(HIST("hmpidQMip_RICH5_HV5"), hmpid.chargeMIP());}
          if(distance2D(hmpid.xtrack(), hmpid.ytrack(), hmpid.xmip(), hmpid.ymip()) < cut_d_mip_track && hmpid.chargeMIP() > 120. && hmpid.chAngle()>0)
          {
            for (int i = 0; i < 10; i++) 
            {
      ...
      if (hmpid.chamber() == 6) 
      {
        ...
        if(distance2D(hmpid.xtrack(), hmpid.ytrack(), hmpid.xmip(), hmpid.ymip()) < cut_d_mip_track) {histos.fill(HIST("hmpidQMip6"), hmpid.chargeMIP());}
        ...
        if(distance2D(hmpid.xtrack(), hmpid.ytrack(), hmpid.xmip(), hmpid.ymip()) < cut_d_mip_track && hmpid.chargeMIP() > 120.)
        {
          ...
          sin2changle = (float)TMath::Power(TMath::Sin(hmpid.chAngle()),2);
          if(hmpid.xmip() <= 100. && hmpid.xmip() >= 40. && hmpid.ymip() <= 100. && hmpid.ymip() >= 40.){histos.fill(HIST("nPhotons_vs_sin2Ch6"), sin2changle, hmpid.nphotons());}
        ...
        if(fParam->inHVSector(hmpid.ymip()) == 0)
        {
          if(distance2D(hmpid.xtrack(), hmpid.ytrack(), hmpid.xmip(), hmpid.ymip()) < cut_d_mip_track) {histos.fill(HIST("hmpidQMip_RICH6_HV0"), hmpid.chargeMIP());}
          if(distance2D(hmpid.xtrack(), hmpid.ytrack(), hmpid.xmip(), hmpid.ymip()) < cut_d_mip_track && hmpid.chargeMIP() > 120. && hmpid.chAngle()>0)
          {
            for (int i = 0; i < 10; i++) 
            {
        ...
        if(fParam->inHVSector(hmpid.ymip()) == 1)
        {
          if(distance2D(hmpid.xtrack(), hmpid.ytrack(), hmpid.xmip(), hmpid.ymip()) < cut_d_mip_track) {histos.fill(HIST("hmpidQMip_RICH6_HV1"), hmpid.chargeMIP());}
          if(distance2D(hmpid.xtrack(), hmpid.ytrack(), hmpid.xmip(), hmpid.ymip()) < cut_d_mip_track && hmpid.chargeMIP() > 120. && hmpid.chAngle()>0)
          {
            for (int i = 0; i < 10; i++) 
            {
        ...
        if(fParam->inHVSector(hmpid.ymip()) == 2)
        {
          if(distance2D(hmpid.xtrack(), hmpid.ytrack(), hmpid.xmip(), hmpid.ymip()) < cut_d_mip_track) {histos.fill(HIST("hmpidQMip_RICH6_HV2"), hmpid.chargeMIP());}
          if(distance2D(hmpid.xtrack(), hmpid.ytrack(), hmpid.xmip(), hmpid.ymip()) < cut_d_mip_track && hmpid.chargeMIP() > 120. && hmpid.chAngle()>0)
          {
            for (int i = 0; i < 10; i++) 
            {
        ...
        if(fParam->inHVSector(hmpid.ymip()) == 3)
        {
          if(distance2D(hmpid.xtrack(), hmpid.ytrack(), hmpid.xmip(), hmpid.ymip()) < cut_d_mip_track) {histos.fill(HIST("hmpidQMip_RICH6_HV3"), hmpid.chargeMIP());}
          if(distance2D(hmpid.xtrack(), hmpid.ytrack(), hmpid.xmip(), hmpid.ymip()) < cut_d_mip_track && hmpid.chargeMIP() > 120. && hmpid.chAngle()>0)
          {
            for (int i = 0; i < 10; i++) 
            {
        ...
        if(fParam->inHVSector(hmpid.ymip()) == 4)
        {
          if(distance2D(hmpid.xtrack(), hmpid.ytrack(), hmpid.xmip(), hmpid.ymip()) < cut_d_mip_track) {histos.fill(HIST("hmpidQMip_RICH6_HV4"), hmpid.chargeMIP());}
          if(distance2D(hmpid.xtrack(), hmpid.ytrack(), hmpid.xmip(), hmpid.ymip()) < cut_d_mip_track && hmpid.chargeMIP() > 120. && hmpid.chAngle()>0)
          {
            for (int i = 0; i < 10; i++) 
            {
        ...
        if(fParam->inHVSector(hmpid.ymip()) == 5)
        {
          if(distance2D(hmpid.xtrack(), hmpid.ytrack(), hmpid.xmip(), hmpid.ymip()) < cut_d_mip_track) {histos.fill(HIST("hmpidQMip_RICH6_HV5"), hmpid.chargeMIP());}
          if(distance2D(hmpid.xtrack(), hmpid.ytrack(), hmpid.xmip(), hmpid.ymip()) < cut_d_mip_track && hmpid.chargeMIP() > 120. && hmpid.chAngle()>0)
          {
            for (int i = 0; i < 10; i++) 
            {
...
*/
      ...
      if(distance2D(hmpid.xtrack(), hmpid.ytrack(), hmpid.xmip(), hmpid.ymip()) > 1.5 || hmpid.chargeMIP() < 120.) continue;
      
      histos.fill(HIST("hmpidCkovvsMom_nocut"), fabs(hmpid.momentumHMPID()), hmpid.chAngle());
       
      if(probsHMP[0]<0.7 && probsHMP[1]<0.7 && probsHMP[2]<0.7) continue; 
