...
struct pidHmpidQaPb
{
  ...
  void process(aod::HMPID_analysisPb const &hmpidtable)
  {
    ...
    for (auto &hmpid : hmpidtable) // loop on tracks contained in the table
    {
      ...
      ///////////////////////////////////////////////////////////////////////////////////
      // fill tree
      /*ch_angle = hmpid.chAngle();
      azim_angle = hmpid.phi();
      pseudorapidity = hmpid.eta();
      momHMPID = hmpid.momentumHMPID();
      momTrack = hmpid.momentumTrack();
      xTrack = hmpid.xtrack();
      yTrack = hmpid.ytrack();
      xMip = hmpid.xmip();
      yMip = hmpid.ymip();
      nPhotons = hmpid.nphotons();
      qMip = hmpid.chargeMIP();
      clusSize = hmpid.clustersize();
      rich_ch = hmpid.chamber();
      photCharge = hmpid.photons_charge(); //qui modifica per qahmpid new
      eta_track = hmpid.etatrack();
      phi_track = hmpid.phitrack();

      its_cluster = hmpid.itsNcluster();
      tpc_cluster = hmpid.tpcNcluster();
      tpc_crossed_rows = hmpid.tpcNClsCrossedRows();
      tpcChi = hmpid.tpcChi2();
      itsChi = hmpid.itsChi2();
      dcaxy_tree = hmpid.dcaxy();
      dcaz_tree = hmpid.dcaz();

      tpc_nsigmaPi = hmpid.tpcNsigmaPi();
      tof_nsigmaPi = hmpid.tofNsigmaPi();
      tpc_nsigmaK = hmpid.tpcNsigmaKa();
      tof_nsigmaK = hmpid.tofNsigmaKa();
      tpc_nsigmaPr = hmpid.tpcNsigmaPr();
      tof_nsigmaPr = hmpid.tofNsigmaPr();
      tpc_nsigmaDe = hmpid.tpcNsigmaDe();
      tof_nsigmaDe = hmpid.tofNsigmaDe(); */

      ///////////////////////////////////////////////////////////////////////////////////
      Double_t cut_d_mip_track = 1.5; // cut for the distance mip-track
 ...
 */
      // if(probsHMP[0]<0.7 && probsHMP[1]<0.7 && probsHMP[2]<0.7 && hmpid.nphotons()<10) continue;
      if (hmpid.chamber() == 2 || hmpid.chamber() == 4)
        continue;
