...
#undef particleCase
  ...
  void process2(TrkPID const& tracks)
  {
    ...
    if (tracks.size() != tracksWithPid.size()) {
      ...
      LOG(info) << "Track table size: " << tracks.size() << " vs " << tracksWithPid.size();
    }
    LOG(info) << "Runningnnnnnnnnnnnnnnnnnnnnnn ";
    for (const auto& t : tracksWithPid) {
      histos.fill(HIST("check/beta"), t.beta() - t.tofBeta());
      histos.fill(HIST("check/mass"), t.mass() - t.tofMass());

      histos.fill(HIST("check/El/reso"), t.tofExpSigmaEl() - t.tofExpSigma2El());
      histos.fill(HIST("check/El/nsigma"), t.tofNSigmaEl() - t.tofNSigma2El());

      histos.fill(HIST("check/Mu/reso"), t.tofExpSigmaMu() - t.tofExpSigma2Mu());
      histos.fill(HIST("check/Mu/nsigma"), t.tofNSigmaMu() - t.tofNSigma2Mu());

      histos.fill(HIST("check/Pi/reso"), t.tofExpSigmaPi() - t.tofExpSigma2Pi());
      histos.fill(HIST("check/Pi/nsigma"), t.tofNSigmaPi() - t.tofNSigma2Pi());

      histos.fill(HIST("check/Ka/reso"), t.tofExpSigmaKa() - t.tofExpSigma2Ka());
      histos.fill(HIST("check/Ka/nsigma"), t.tofNSigmaKa() - t.tofNSigma2Ka());

      histos.fill(HIST("check/Pr/reso"), t.tofExpSigmaPr() - t.tofExpSigma2Pr());
      histos.fill(HIST("check/Pr/nsigma"), t.tofNSigmaPr() - t.tofNSigma2Pr());

      histos.fill(HIST("check/De/reso"), t.tofExpSigmaDe() - t.tofExpSigma2De());
      histos.fill(HIST("check/De/nsigma"), t.tofNSigmaDe() - t.tofNSigma2De());

      histos.fill(HIST("check/Tr/reso"), t.tofExpSigmaTr() - t.tofExpSigma2Tr());
      histos.fill(HIST("check/Tr/nsigma"), t.tofNSigmaTr() - t.tofNSigma2Tr());

      histos.fill(HIST("check/He/reso"), t.tofExpSigmaHe() - t.tofExpSigma2He());
      histos.fill(HIST("check/He/nsigma"), t.tofNSigmaHe() - t.tofNSigma2He());

      histos.fill(HIST("check/Al/reso"), t.tofExpSigmaAl() - t.tofExpSigma2Al());
