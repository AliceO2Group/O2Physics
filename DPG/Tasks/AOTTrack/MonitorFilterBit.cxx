...
struct CheckFilterBit {
  ...
  { // test whether the name can be changed using the PROCESS_SWITCH method (see qaEventTrack class)
    ...
    for (auto& track : tracks) {
      ...
      if(std::abs(track.eta())<0.9)histos.fill(HIST("Tracks/Reco/histptAll"), track.pt());
      histos.fill(HIST("Tracks/Reco/histpt3DAll"), track.pt(),track.eta(),track.phi());
      if(track.isGlobalTrack())histos.fill(HIST("Tracks/Reco/histpt3DFB0"), track.pt(),track.eta(),track.phi());
      if(track.trackCutFlagFb1())histos.fill(HIST("Tracks/Reco/histpt3DFB1"), track.pt(),track.eta(),track.phi());
      if(track.trackCutFlagFb2())histos.fill(HIST("Tracks/Reco/histpt3DFB2"), track.pt(),track.eta(),track.phi());
    }
  }
  PROCESS_SWITCH(CheckFilterBit, processData, "process data", true);

  void processRecoMC(soa::Join<aod::Collisions, aod::McCollisionLabels>::iterator const& collision, soa::Join<aod::Tracks, aod::TrackSelection, aod::TrackSelectionExtension, aod::McTrackLabels> const& tracks, aod::McParticles const& mcParticles, aod::McCollisions const& mcCollisions){// this will loop over data (PV) collisions

   
   
    histos.fill(HIST("EventProp/histDatacollZ"),collision.posZ());
    if(std::abs(collision.posZ())>10.)return;
    ncollisionCounter++;
    for (auto & track : tracks){

      if (track.collisionId() < 0)histos.fill(HIST("EventProp/histPtTrackNegCollID"),track.pt());      
	  ...
	  // auto collReco = track.collision_as<CollisionTableMC>();
	    ...
	    if(mcparticle.isPhysicalPrimary() && (std::abs(mcparticle.pdgCode())==211 || std::abs(mcparticle.pdgCode())==321 || std::abs(mcparticle.pdgCode())==2212 || std::abs(mcparticle.pdgCode())==11 || std::abs(mcparticle.pdgCode())==13)){
	      if(std::abs(track.eta())<0.9)histos.fill(HIST("Tracks/RecoMCPhysPrimCollMatch/histpt"), mcparticle.pt());// note: one needs to avoid double counting of tracks reco both in TPC and ITS but not matched
	      if(track.isGlobalTrack())histos.fill(HIST("Tracks/RecoMCPhysPrimCollMatch/histptFB0"), track.pt(),track.eta(),track.phi());
	      if(track.trackCutFlagFb1())histos.fill(HIST("Tracks/RecoMCPhysPrimCollMatch/histptFB1"), track.pt(),track.eta(),track.phi());
	      if(track.trackCutFlagFb2())histos.fill(HIST("Tracks/RecoMCPhysPrimCollMatch/histptFB2"), track.pt(),track.eta(),track.phi());	      
	    }
	      ...
	      if(track.isGlobalTrack())histos.fill(HIST("Tracks/RecoMCRad1to15cmCollMatch/histptFB0"), track.pt(),track.eta(),track.phi());
	      if(track.trackCutFlagFb1())histos.fill(HIST("Tracks/RecoMCRad1to15cmCollMatch/histptFB1"), track.pt(),track.eta(),track.phi());
	      if(track.trackCutFlagFb2())histos.fill(HIST("Tracks/RecoMCRad1to15cmCollMatch/histptFB2"), track.pt(),track.eta(),track.phi());	      	      
	    } 
	    if(prodRadius2>1.e-4 && prodRadius2<0.5 && (std::abs(mcparticle.pdgCode())==211 || std::abs(mcparticle.pdgCode())==321 || std::abs(mcparticle.pdgCode())==2212 || std::abs(mcparticle.pdgCode())==11 || std::abs(mcparticle.pdgCode())==13)){
	      if(track.isGlobalTrack())histos.fill(HIST("Tracks/RecoMCRad1mumto5mmCollMatch/histptFB0"), track.pt(),track.eta(),track.phi());
	      if(track.trackCutFlagFb1())histos.fill(HIST("Tracks/RecoMCRad1mumto5mmCollMatch/histptFB1"), track.pt(),track.eta(),track.phi());
	      if(track.trackCutFlagFb2())histos.fill(HIST("Tracks/RecoMCRad1mumto5mmCollMatch/histptFB2"), track.pt(),track.eta(),track.phi());	      
	    } 
	  }
      }
    }
  ...
  void processMC(aod::McCollision const& mcCollision,aod::McParticles const& mcParticles){
    ...
    if(std::abs(mcCollision.posZ())>10.)return;
    ncollisionCounter++;
	...
	if(std::abs(mcpart.eta())<0.9)histos.fill(HIST("Tracks/MCgen/histMCgenpt"), mcpart.pt());
