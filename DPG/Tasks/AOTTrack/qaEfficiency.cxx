...
struct QaEfficiency {
  ...
  void fillMCTrackHistograms(const TrackCandidatesMC::iterator& track, const bool doMakeHistograms)
  {
    ...
    if (passedITS && passedTPC) {
      ...
      if (doPtRadius) {
        hPtRadiusItsTpc[histogramIndex]->Fill(mcParticle.pt(), mcParticle.radius());
        if (passedTOF) {
          hPtRadiusItsTpcTof[histogramIndex]->Fill(mcParticle.pt(), mcParticle.radius());
        }
      }
    }
  ...
  void fillMCParticleHistograms(const o2::aod::McParticles::iterator& mcParticle, const bool doMakeHistograms)
  {
    ...
    if (doPtRadius) {
      hPtRadiusGenerated[histogramIndex]->Fill(mcParticle.pt(), mcParticle.radius());
    }
  }
