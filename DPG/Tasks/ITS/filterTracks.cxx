...
struct FilterTracks {
   ...
   if (has_MCparticle) {
	 ...
	 std::vector<int>indxDaughers;
		 ...
		 // std::cout<<"########       FOUND D0, MATCHED! pdg: " <<particleMother.pdgCode()<<"################ size array "<<indxDaughers.size()<<std::endl;
	       ...
	       else ndaught=-indxDaughers.size();
	     }
     ...
     if(pdgParticleMother==0)filteredTracksMC(mcparticle.pdgCode(),mcparticle.isPhysicalPrimary(),0,0,-1,0,0,0,0,0);
  ...
  void processMC(soa::Filtered<TracksWithSelAndDcaMc> const& tracks,aod::McParticles const& mcParticles,aod::McCollisions const& mcCollisions,aod::McCollisionLabels const& mcCollisionLabels)
  {
	...
	else selectedGenParticles(mcpart.pdgCode(),mcpart.mcCollisionId(),0,mcpart.pt(),mcpart.y(),etamax,0,0);
      }
      //	 
	}
  }
