...
struct qaEventTrackLite {
  ...
  void processDataLite(o2::soa::Filtered<aod::DPGTracks> const& tracks, aod::DPGCollisions const&)
  {
    for (const auto& track : tracks) {
      ...
      if(!applyTrackSelectionsNotFiltered(track))continue;
  ...
  void processMCLite(o2::soa::Filtered<soa::Join<aod::DPGTracks, aod::DPGRecoParticles>> const& tracks, aod::DPGCollisions const&, aod::DPGNonRecoParticles const& particles)
  {
    ...
    for (const auto& particle : particles) {
      ...
      if(particle.productionMode() == 0)histos.fill(HIST("Particle/selPtEtaPhiMCGenPrimary"),particle.ptMC(),particle.etaMC(),particle.phiMC());
    }
  }
  ...
  {
    if(track.tpcNClsFound() < nClusterTpcMin)return false;
    if(track.tpcNClsCrossedRows() < nCrossedRowsTpcMin)return false;
    if(track.tpcCrossedRowsOverFindableCls() < nCrossedRowsTpcOverFindableClustersTpcMin) return false;
    return true;
  }
  ...
  {
    if(track.tpcNClsFound() < nClusterTpcMinSel1)return false;
    if(track.tpcNClsCrossedRows() < nCrossedRowsTpcMinSel1)return false;
    if(track.tpcCrossedRowsOverFindableCls() < nCrossedRowsTpcOverFindableClustersTpcMinSel1) return false;
    if(track.itsChi2NCl() > chi2ItsMaxSel1)return false;
    if(track.tpcChi2NCl() > chi2TpcMaxSel1)return false;
