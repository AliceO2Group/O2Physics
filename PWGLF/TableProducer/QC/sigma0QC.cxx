...
struct sigma0QC {
  ...
  void runAnalysis(TCollision const& collisions, TV0s const& fullV0s, bool fDavidTest)
  {
  ...
  {    
    ...
    runAnalysis(collisions, fullV0s, false);
  ...
  void processRealDataDavid(soa::Join<aod::StraCollisions, aod::StraCents, aod::StraEvSels, aod::StraStamps> const& collisions, V0StandardDerivedDatas const& fullV0s, dauTracks const&)
  {
    ...
    runAnalysis(collisions, fullV0s, true);
  ...
  PROCESS_SWITCH(sigma0QC, processRealData, "process using sliceby unsorted", true);  
  PROCESS_SWITCH(sigma0QC, processRealDataDavid, "process using David's grouping approach", true);  
};

struct sigma0QCSorted {
  Service<o2::ccdb::BasicCCDBManager> ccdb;
  ctpRateFetcher rateFetcher;
  ...
  // Real data processing - no MC subscription
  template <typename TCollision, typename TV0s>
  void runAnalysis(TCollision const& collisions, TV0s const& fullV0s)
  {
    uint64_t CollIDBuffer = 0;
    for (const auto& coll : collisions) {
  ...
  {    
    ...
    runAnalysis(collisions, fullV0s);

    auto end = std::chrono::high_resolution_clock::now();
    std::chrono::duration<double> elapsed = end - start;
    
    if (fverbose) LOGF(info, "[Process function call, PreSlice] N. Collisions: %i, N. V0s: %i, Processing time (s): %lf", collisions.size(), fullV0s.size(), elapsed.count());      
  } 
  PROCESS_SWITCH(sigma0QCSorted, processRealData, "process using sliceby sorted", true);    
};

struct sigma0QCIterator {
  Service<o2::ccdb::BasicCCDBManager> ccdb;
  ctpRateFetcher rateFetcher;
  ...
  // Real data processing - no MC subscription
  template <typename TCollision, typename TV0s>
  void runAnalysis(TCollision const& coll, TV0s const& fullV0s)
  {
    if (!IsEventAccepted(coll))
      return;
  ...
  void processRealData(soa::Join<aod::StraCollisions, aod::StraCents, aod::StraEvSels, aod::StraStamps>::iterator const& coll, V0StandardDerivedDatas const& fullV0s, dauTracks const&)
  {    
    processStartTime();
    runAnalysis(coll, fullV0s);
  }
  void processEndTime(soa::Join<aod::StraCollisions, aod::StraCents, aod::StraEvSels, aod::StraStamps> const& collisions, V0StandardDerivedDatas const& fullV0s)
  {
  ...
  PROCESS_SWITCH(sigma0QCIterator, processRealData, "process using iterators", true);    
  PROCESS_SWITCH(sigma0QCIterator, processEndTime, "process using iterators. End time.", true);    
};

WorkflowSpec defineDataProcessing(ConfigContext const& cfgc)
{
  return WorkflowSpec{
      adaptAnalysisTask<sigma0QC>(cfgc),
      adaptAnalysisTask<sigma0QCSorted>(cfgc),
      adaptAnalysisTask<sigma0QCIterator>(cfgc)};
}
