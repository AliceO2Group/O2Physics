...
namespace {
  ...
  enum DAUGHTERS{kPion, kProton, kDeuteron, kTriton, kHe3, kAlpha};
  ...
  {"alpha",    "pion",     "none", "none"},
  {"triton",   "proton",   "pion", "none"},
  {"helion",   "proton",   "pion", "none"},
...
//----------------------------------------------------------------------------------------------------------------             
  struct daughterParticle {
    ...
    void Print() {
      std::cout << std::endl << "Daughter: " << name << std::endl;
      std::cout << "PDG: " << pdgCode << ", Mass: " << mass << ", Charge: " << charge << std::endl;
      for (double d : betheParams) std::cout << d << ", " << std::flush;
  ...
  struct hyperNucleus {
    ...
    hyperNucleus(std::string name_, int pdgCode_, bool active_, std::vector<int> daughters_, std::vector<int> daughterTrackSigns_) {
      ...
      pdgCode = pdgCode_;
      active = active_;
      for (int d : daughters_) daughters.push_back(d);        
      for (int dc : daughterTrackSigns_) daughterTrackSigns.push_back(dc);            
    }
    ...
    void Print() {
      std::cout << std::endl << "Hypernucleus: " << name << " (" << pdgCode << "):"<< (active ? " active" : " not active") << std::endl;
      for (double d : daughters) std::cout << d << ", " << std::flush;
  ...
  struct hyperNucCandidate {
    ...
    hyperNucCandidate(int species_, std::vector<KFParticle> kfpDaughters_, std::vector<int64_t> daughterTrackIds_) : species(species_), hypNucDaughter(0), devToVtx(999), dcaToVtxXY(999), dcaToVtxZ(999), chi2(999), mcTrue(false), isPhysPrimary(false), isPrimaryCandidate(false), isSecondaryCandidate(false), isUsedSecondary(false), mcParticleId(-1), tableId(-1) {
      for (auto kfd : kfpDaughters_) kfpDaughters.push_back(kfd); 
      for (auto dt : daughterTrackIds_) daughterTrackIds.push_back(dt); 
      Init();   
    }
    ...
    void Init() {
      ...
      for (size_t i = 0; i < kfpDaughters.size(); i++) kfp.AddDaughter(kfpDaughters.at(i)); 
    ...
    bool CheckKfp() {
      if (kfp.GetMass() == 0) return false;
      if (std::isnan(kfp.GetMass())) return false;
      return true;
    }
    int GetDaughterTableId() {
      if (hypNucDaughter) return hypNucDaughter->tableId;
      return -1;
    }
    bool IsCascade() { return hypNucDaughter != 0; }
    int GetSign() { 
      if (kfp.GetQ()==0) return kfpDaughters.front().GetQ() / std::abs(kfpDaughters.front().GetQ());
...
//----------------------------------------------------------------------------------------------------------------     
  ...
  void findDaughterParticles(aod::TrackAssoc const& tracksByColl, TracksFull const& tracks) {
    ...
    for (const auto& trackId : tracksByColl) {
      ...
      if (std::abs(track.eta()) > cfgCutEta) continue;
      if (!cfgUsePVcontributors && track.isPVContributor()) continue; 
      for (size_t i = 0; i < daughterParticles.size(); i++) {
        if (track.tpcNClsFound() < cfgTrackPIDsettings->get(i, "minTPCnCls")) continue;
        if (track.tpcChi2NCl() > cfgTrackPIDsettings->get(i, "maxTPCchi2")) continue;        
        if (track.itsNCls() < cfgTrackPIDsettings->get(i, "minITSnCls")) continue;     
        if (track.itsChi2NCl() > cfgTrackPIDsettings->get(i, "maxITSchi2")) continue;                   
        if (std::abs(getTPCnSigma(track, daughterParticles.at(i))) > cfgTrackPIDsettings->get(i, "maxTPCnSigma")) continue; 
        filldedx(track, i);
        if (std::abs(track.dcaXY()) < cfgTrackPIDsettings->get(i, "minDcaToPvXY")) continue;
        if (std::abs(track.dcaZ())  < cfgTrackPIDsettings->get(i, "minDcaToPvZ"))  continue;        
        if (getRigidity(track) < cfgTrackPIDsettings->get(i, "minRigidity") || getRigidity(track) > cfgTrackPIDsettings->get(i, "maxRigidity")) continue;
        if (cfgTrackPIDsettings->get(i, "TOFrequiredabove") >= 0 && getRigidity(track) > cfgTrackPIDsettings->get(i, "TOFrequiredabove") && (track.mass() < cfgTrackPIDsettings->get(i, "minTOFmass") || track.mass() > cfgTrackPIDsettings->get(i, "maxTOFmass"))) continue;
  ...
  void checkMCTrueTracks(aod::McTrackLabels const& trackLabels, aod::McParticles const&) {
    for (int i = 0; i < nDaughterParticles; i++) {  
      ...
      for (auto it = daughterVec.begin(); it < daughterVec.end(); it++) {
        ...
        if (!mcLab.has_mcParticle()) { mcTrue = false; }
        else {
          const auto& mcPart = mcLab.mcParticle_as<aod::McParticles>();
          if (std::abs(mcPart.pdgCode()) != daughterParticles.at(i).pdgCode) { mcTrue = false; }
          if (!mcPart.has_mothers()) { mcTrue = false; }
        }
        if (!mcTrue) { daughterVec.erase(it); } 
      }
    }  
  }
//----------------------------------------------------------------------------------------------------------------        
    ...
    for (size_t hyperNucIter = 0; hyperNucIter < singleHyperNuclei.size(); hyperNucIter++) {
      ...
      int nCombinations = 1;
        ...
        for (int i = 0; i < nDaughters; i++) {        
          ...
          if (!i) checkSign = daughterTrack.sign();
        ...
        if (passedChecks && rand.Rndm() <= cfgReduce->get((unsigned int)hyperNucIter, 0u)) {                  
          ...
          if (candidate.CheckKfp()) {
            ...
            if (candidate.kfp.GetMass() < cfgPreSelectionsPrimaries->get(hyperNucIter, "minMass") || candidate.kfp.GetMass() > cfgPreSelectionsPrimaries->get(hyperNucIter, "maxMass"))  isPrimCandidate = false;          
            if (candidate.GetDcaTracks() > cfgPreSelectionsPrimaries->get(hyperNucIter, "maxDcaTracks"))  isPrimCandidate = false;
            if (candidate.GetCt(primVtx) < cfgPreSelectionsPrimaries->get(hyperNucIter, "minCt") || candidate.GetCt(primVtx) > cfgPreSelectionsPrimaries->get(hyperNucIter, "maxCt"))  isPrimCandidate = false; 
            if (candidate.GetCpa(primVtx) < cfgPreSelectionsPrimaries->get(hyperNucIter, "minCosPa"))  isPrimCandidate = false; 
            if (candidate.GetDcaMotherToVtxXY(primVtx) > cfgPreSelectionsPrimaries->get(hyperNucIter, "maxDcaMotherToPvXY"))  isPrimCandidate = false;                  
            if (candidate.GetDcaMotherToVtxZ(primVtx) > cfgPreSelectionsPrimaries->get(hyperNucIter, "maxDcaMotherToPvZ"))  isPrimCandidate = false;                  
            if (isPrimCandidate) {
              candidate.isPrimaryCandidate = true;
              collHasCandidate = true;
            }
            if (candidate.kfp.GetMass() < cfgPreSelectionsSecondaries->get(hyperNucIter, "minMass") || candidate.kfp.GetMass() > cfgPreSelectionsSecondaries->get(hyperNucIter, "maxMass"))  isSecCandidate = false;
            if (candidate.GetDcaTracks() > cfgPreSelectionsSecondaries->get(hyperNucIter, "maxDcaTracks"))  isSecCandidate = false;
            if (candidate.GetCt(primVtx) < cfgPreSelectionsSecondaries->get(hyperNucIter, "minCt") || candidate.GetCt(primVtx) > cfgPreSelectionsSecondaries->get(hyperNucIter, "maxCt"))  isSecCandidate = false;
            if (isSecCandidate) {
              candidate.isSecondaryCandidate = true;
            } 
            if (isPrimCandidate || isSecCandidate) singleHyperNucCandidates.at(hyperNucIter).push_back(candidate);                        
          }
        }  
		    it[nDaughters-1]++;
      ...
      it.push_back(vecHypNucDaughers.begin());
        ...
        if (passedChecks && hypNucDaughter->isSecondaryCandidate) {       
          ...
          if (candidate.CheckKfp()) {
            ...
            if (hypNucDaughter->GetCpa(candidate.recoSV) < cfgPreSelectionsSecondaries->get(hyperNuc->daughters.at(0), "minCosPaSv"))  isCandidate = false; 
            if (hypNucDaughter->GetDcaMotherToVtxXY(candidate.recoSV) > cfgPreSelectionsSecondaries->get(hyperNuc->daughters.at(0), "maxDcaMotherToSvXY"))  isCandidate = false;                  
            if (hypNucDaughter->GetDcaMotherToVtxZ(candidate.recoSV) > cfgPreSelectionsSecondaries->get(hyperNuc->daughters.at(0), "maxDcaMotherToSvZ"))  isCandidate = false;                  
            // apply pre selections for cascade
            if (candidate.kfp.GetMass() < cfgPreSelectionsCascades->get(hyperNucIter, "minMass") || candidate.kfp.GetMass() > cfgPreSelectionsCascades->get(hyperNucIter, "maxMass"))  isCandidate = false;          
            if (candidate.GetDcaTracks() > cfgPreSelectionsCascades->get(hyperNucIter, "maxDcaTracks"))  isCandidate = false;
            if (candidate.GetCt(primVtx) < cfgPreSelectionsCascades->get(hyperNucIter, "minCt") || candidate.GetCt(primVtx) > cfgPreSelectionsCascades->get(hyperNucIter, "maxCt"))  isCandidate = false; 
            if (candidate.GetCpa(primVtx) < cfgPreSelectionsCascades->get(hyperNucIter, "minCosPa"))  isCandidate = false; 
            if (candidate.GetDcaMotherToVtxXY(primVtx) > cfgPreSelectionsCascades->get(hyperNucIter, "maxDcaMotherToPvXY"))  isCandidate = false;                  
            if (candidate.GetDcaMotherToVtxZ(primVtx) > cfgPreSelectionsCascades->get(hyperNucIter, "maxDcaMotherToPvZ"))  isCandidate = false;            
      ...
      for (size_t hyperNucIter = 0; hyperNucIter < hypNucVectors.at(vec)->size(); hyperNucIter++) {
        ...
        for (auto& hypCand : candidateVector->at(hyperNucIter)) {
          ...
          if (hypCand.IsCascade()) {
            if (!hypCand.hypNucDaughter->mcTrue) continue;
          ...
          for (auto& daughter : hypCand.daughterTrackIds) {
            ...
            if (!mcLab.has_mcParticle()) continue;
            const auto& mcPart = mcLab.mcParticle_as<aod::McParticles>();
            if (std::abs(mcPart.pdgCode()) != daughterParticles.at(hyperNuc->daughters.at(daughterCount++)).pdgCode) continue;
            if (!mcPart.has_mothers()) continue;
            for (auto& mother : mcPart.mothers_as<aod::McParticles>()) {
              if (mother.pdgCode() == hyperNuc->pdgCode * hypCand.GetSign()) {
                motherIds.push_back(mother.globalIndex());
          ...
          if (motherIds.size() != hyperNuc->daughters.size()) {
            if (cfgSaveOnlyMcTrue) hypCand.isSecondaryCandidate = false;
            continue;
          } 
          hypCand.mcTrue = true;
          for (auto iter = motherIds.begin(); iter != motherIds.end()-1; iter++) if (*iter != *(iter+1)) hypCand.mcTrue = false;  
          if (hypCand.mcTrue) {
            hypCand.mcParticleId = motherIds.front();
            collHasMcTrueCandidate = true;
          } 
          if (!hypCand.mcTrue && cfgSaveOnlyMcTrue) hypCand.isSecondaryCandidate = false;
        } 
      }  
    } 
  }  
//---------------------------------------------------------------------------------------------------------------- 
    ...
    for (int vec = 0; vec < 2; vec++) {
      ...
      for (size_t hyperNucIter = 0; hyperNucIter < hypNucVectors.at(vec)->size(); hyperNucIter++) {
        ...
        if (!hyperNuc->active) continue;   
        for (auto& hypCand : candidateVector->at(hyperNucIter)) {  
          if (!hypCand.isPrimaryCandidate && !hypCand.isUsedSecondary && !hypCand.IsCascade()) continue;
          if (saveOnlyMcTrue && !hypCand.mcTrue) continue;
...
//----------------------------------------------------------------------------------------------------------------    
  ...
  void processMC(CollisionsFullMC const& collisions, aod::McCollisions const& mcColls, TracksFull const& tracks,  aod::BCsWithTimestamps const&, aod::McParticles const& particlesMC, aod::McTrackLabels const& trackLabelsMC, aod::McCollisionLabels const& collLabels, aod::TrackAssoc const& tracksColl) { 
    ...
    for (auto& mcPart : particlesMC) {   
      for (int vec = 0; vec < 2; vec++) {
        for (size_t hyperNucIter = 0; hyperNucIter < hypNucVectors.at(vec)->size(); hyperNucIter++) {
          hyperNucleus *hyperNuc = &(hypNucVectors.at(vec)->at(hyperNucIter));
          if (!hyperNuc->active) continue;
          if (std::abs(mcPart.pdgCode()) != hyperNuc->pdgCode) continue;
          bool isDecayMode = false;
          float svx, svy, svz;
          int daughterPdg;
          if (vec == 0) daughterPdg = daughterParticles.at(hyperNuc->daughters.at(0)).pdgCode;
          else daughterPdg = singleHyperNuclei.at(hyperNuc->daughters.at(0)).pdgCode;
          for (auto& mcDaught : mcPart.daughters_as<aod::McParticles>()) {         
            if (std::abs(mcDaught.pdgCode()) == daughterPdg) {
              isDecayMode = true;
              svx = mcDaught.vx(); svy = mcDaught.vy(); svz = mcDaught.vz();
              break;
            }         
          }
          if (!isDecayMode) continue;
    ...
    for (const auto& collision : collisions) {
      ...
      if (cfgSaveOnlyMcTrue) checkMCTrueTracks(trackLabelsMC, particlesMC);
      ...
      if (!collHasCandidate) continue;
      if (cfgSaveOnlyMcTrue && !collHasMcTrueCandidate) continue;
...
void initCollision(const T& collision) {  
  ...
  if (collPassedEvSel) histos.fill(HIST("histNev"), 1.5);                  
...
//----------------------------------------------------------------------------------------------------------------     
  ...
  void filldedx (T const& track, int species) {
    ...
    if (track.tpcNClsFound() < 100 || track.itsNCls() < 2) return;
...
//----------------------------------------------------------------------------------------------------------------         
  ...
  float getRigidity(T const& track) {
    if (!cfgRigidityCorrection) return track.tpcInnerParam();
...
//---------------------------------------------------------------------------------------------------------------- 
  ...
  int getHypDaughterVec(unsigned int cascade, LabeledArray<std::string> cfg) {
    ...
    if (std::find(hyperNucNames.begin(), hyperNucNames.end(), daughter) == hyperNucNames.end()) return -1;
 ...
 //----------------------------------------------------------------------------------------------------------------    
  std::vector<int> getDaughterVec(unsigned int hypNuc, LabeledArray<std::string> cfg) {
    ...
    for (unsigned int i = 0; i < 4; i++) {
      ...
      if (std::find(particleNames.begin(), particleNames.end(), daughter) == particleNames.end()) break;
