...
struct Lithium4Candidate {
  ...
  uint8_t sharedClustersPion = 0u;
  float chi2TPCPion = -10.f;
  float nSigmaPion = -10.f;
  uint32_t PIDtrkPion = 0xFFFFF; // PID in tracking
  float massTOFPion = -10;
  uint32_t itsClSizePion = 0u;
...
struct pihe3analysis {
  ...
  HistogramRegistry m_qaRegistry{
    "QA",
    {
      ...
      {"h2NsigmaPionTOF", "NsigmaPion TOF distribution; #it{p} (GeV/#it{c}); n#sigma_{TOF}(p)", {HistType::kTH2F, {{20, -5.0f, 5.0f}, {200, -5.0f, 5.0f}}}},
      {"h2NsigmaPionTOF_preselection", "NsigmaPion TOF distribution; #it{p} (GeV/#it{c}); n#sigma_{TOF}(p)", {HistType::kTH2F, {{100, -5.0f, 5.0f}, {200, -10.0f, 10.0f}}}},
    },
  ...
  bool selectionPIDPion(const Ttrack& candidate)
  {
    ...
    if (candidate.hasTOF() && candidate.pt() > setting_cutPtMinTOFPion) {
      if (std::abs(candidate.tpcNSigmaPi() > setting_cutNsigmaTPC)) {
        return false;
      }
      m_qaRegistry.fill(HIST("h2NsigmaPionTOF_preselection"), candidate.p(), candidate.tofNSigmaPi());
      if (std::abs(candidate.tofNSigmaPi()) > setting_cutNsigmaTOF) {
        return false;
      }
      m_qaRegistry.fill(HIST("h2NsigmaPionTPC"), candidate.tpcInnerParam(), candidate.tpcNSigmaPi());
      m_qaRegistry.fill(HIST("h2NsigmaPionTOF"), candidate.p(), candidate.tofNSigmaPi());
      return true;
    } else if (std::abs(candidate.tpcNSigmaPi()) < setting_cutNsigmaTPC) {
      m_qaRegistry.fill(HIST("h2NsigmaPionTPC"), candidate.tpcInnerParam(), candidate.tpcNSigmaPi());
      return true;
    }
    return false;
  ...
  template <typename Ttrack, typename Tcollisions, typename Ttracks>
  bool PionfillCandidateInfo(const Ttrack& trackHe3, const Ttrack& trackPion, const CollBracket& collBracket, const Tcollisions& collisions, Lithium4Candidate& li4cand, const Ttracks& /*trackTable*/, bool isMixedEvent)
  {
    if (!isMixedEvent) {
      auto trackCovHe3 = getTrackParCov(trackHe3);
      auto trackCovPion = getTrackParCov(trackPion);
      int nCand = 0;
      try {
        nCand = m_fitter.process(trackCovHe3, trackCovPion);
      } catch (...) {
        LOG(error) << "Exception caught in DCA fitter process call!";
        return false;
      }
    ...
    for (int i = 0; i < 3; i++)
      li4cand.momHe3[i] = li4cand.momHe3[i] * 2;
    li4cand.momPion = std::array{trackPion.px(), trackPion.py(), trackPion.pz()};

    float invMass = RecoDecay::m(std::array{li4cand.momHe3, li4cand.momPion}, std::array{o2::constants::physics::MassHelium3, o2::constants::physics::MassPiPlus});
    if (setting_cutInvMass > 0 && invMass > setting_cutInvMass) {
      return false;
    }
    ...
    li4cand.DCAxyHe3 = trackHe3.dcaXY();
    li4cand.DCAzHe3 = trackHe3.dcaZ();
    li4cand.DCAxyPion = trackPion.dcaXY();
    li4cand.DCAzPion = trackPion.dcaZ();

    li4cand.tpcSignalHe3 = trackHe3.tpcSignal();
    bool heliumPID = trackHe3.pidForTracking() == o2::track::PID::Helium3 || trackHe3.pidForTracking() == o2::track::PID::Alpha;
    float correctedTPCinnerParamHe3 = (heliumPID && setting_compensatePIDinTracking) ? trackHe3.tpcInnerParam() / 2.f : trackHe3.tpcInnerParam();
    li4cand.momHe3TPC = correctedTPCinnerParamHe3;
    li4cand.tpcSignalPion = trackPion.tpcSignal();
    li4cand.momPionTPC = trackPion.tpcInnerParam();

    li4cand.nTPCClustersHe3 = trackHe3.tpcNClsFound();
    li4cand.nSigmaHe3 = computeNSigmaHe3(trackHe3);
    li4cand.nSigmaPion = trackPion.tpcNSigmaPi();
    li4cand.chi2TPCHe3 = trackHe3.tpcChi2NCl();
    li4cand.chi2TPCPion = trackPion.tpcChi2NCl();

    li4cand.PIDtrkHe3 = trackHe3.pidForTracking();
    li4cand.PIDtrkPion = trackPion.pidForTracking();

    li4cand.itsClSizeHe3 = trackHe3.itsClusterSizes();
    li4cand.itsClSizePion = trackPion.itsClusterSizes();
    ...
    li4cand.trackIDHe3 = trackHe3.globalIndex();
    li4cand.trackIDPion = trackPion.globalIndex();
    ...
    if (trackPion.hasTOF()) {
      float beta = responseBeta.GetBeta(trackPion);
      beta = std::min(1.f - 1.e-6f, std::max(1.e-4f, beta)); /// sometimes beta > 1 or < 0, to be checked
      li4cand.massTOFPion = trackPion.tpcInnerParam() * std::sqrt(1.f / (beta * beta) - 1.f);
    }
  ...
  void pifillTable(const Lithium4Candidate& li4cand, const Tcoll& collision, bool isMC = false)
  {
    m_outputDataTable(
      ...
      li4cand.PIDtrkPion,
      li4cand.itsClSizeHe3,
      li4cand.itsClSizePion,
      li4cand.sharedClustersHe3,
      li4cand.sharedClustersPion,
      li4cand.isBkgUS,
      li4cand.isBkgEM);
    if (isMC) {
      m_outputMCTable(
  ...
  PROCESS_SWITCH(pihe3analysis, processSameEvent, "Process Same event", false);
  ...
  PROCESS_SWITCH(pihe3analysis, processMixedEvent, "Process Mixed event", false);
  ...
  PROCESS_SWITCH(pihe3analysis, processMC, "Process MC", false);
  ...
  PROCESS_SWITCH(pihe3analysis, processSameEventPools, "Process Same event pools", false);
  ...
  PROCESS_SWITCH(pihe3analysis, processMcPools, "Process MC pools", false);
};

WorkflowSpec defineDataProcessing(const ConfigContext& cfgc)
{
  return WorkflowSpec{
    adaptAnalysisTask<pihe3analysis>(cfgc, TaskName{"pihe3analysis"})};
}
