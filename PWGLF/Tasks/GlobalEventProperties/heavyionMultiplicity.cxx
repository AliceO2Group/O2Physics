...
AxisSpec axisTrackType = {kTrackTypeend - 1, +kTrackTypebegin + 0.5,
                          +kTrackTypeend - 0.5, "", "TrackTypeAxis"};
AxisSpec axisGenPtVary = {kGenpTend - 1, +kGenpTbegin + 0.5, +kGenpTend - 0.5,
                          "", "GenpTVaryAxis"};
AxisSpec axisSpecies = {kSpeciesend - 1, +kSpeciesbegin + 0.5,
                        +kSpeciesend - 0.5, "", "SpeciesAxis"};
AxisSpec axisMassK0s = {200, 0.4, 0.6, "K0sMass", "K0sMass"};
AxisSpec axisMassLambda = {200, 1.07, 1.17, "Lambda/AntiLamda Mass",
                           "Lambda/AntiLamda Mass"};
...
struct HeavyionMultiplicity {
  ...
  Configurable<float> dcaZ{"dcaZ", 0.2f,
                           "Custom DCA Z cut (ignored if negative)"};
  ...
  Configurable<float> dcav0daughtercut{"dcav0daughtercut", 1.0f,
                                       "dcav0daughtercut"};
  ...
  Configurable<bool> isApplyCentNGlobal{"isApplyCentNGlobal", false,
                                        "Centrality based on global tracks"};
  ...
  void init(InitContext const&)
  {
    ...
    if (doprocessData) {
      histos.add("CentPercentileHist", "CentPercentileHist", kTH1D, {axisCent},
                 false);
      histos.add("hdatazvtxcent", "hdatazvtxcent", kTH3D,
                 {axisVtxZ, centAxis, axisOccupancy}, false);
    ...
    if (doprocessMonteCarlo || doprocessMCpTefficiency ||
        doprocessMCcheckFakeTracks) {
      histos.add("CentPercentileMCRecHist", "CentPercentileMCRecHist", kTH1D,
                 {axisCent}, false);
      histos.add("hmczvtxcent", "hmczvtxcent", kTH3D,
                 {axisVtxZ, centAxis, axisOccupancy}, false);
    }

    if (doprocessMonteCarlo) {
      histos.add("MCrecPhiVsEtaHist", "MCrecPhiVsEtaHist", kTH2D,
                 {axisPhi2, axisEta}, false);
      histos.add("hmcrecdndeta", "hmcrecdndeta", kTHnSparseD,
                 {axisVtxZ, centAxis, axisOccupancy, axisEta, axisPhi,
    ...
    if (doprocessMCpTefficiency) {
      histos.add("hmcrecdndpt", "hmcrecdndpt", kTHnSparseD,
                 {centAxis, axisOccupancy, axisTrackType, axisPt}, false);
      histos.add("hmcgendndpt", "hmcgendndpt", kTHnSparseD,
                 {centAxis, axisPt, axisGenPtVary}, false);
    }
    ...
    if (doprocessCorrelation) {
      histos.add("GlobalMult_vs_FT0A", "GlobalMult_vs_FT0A", kTH2F,
                 {axisMult, axisFt0aMult}, true);
      histos.add("GlobalMult_vs_FT0C", "GlobalMult_vs_FT0C", kTH2F,
                 {axisMult, axisFt0cMult}, true);
      histos.add("NPVtracks_vs_FT0C", "NPVtracks_vs_FT0C", kTH2F,
                 {axisPV, axisFt0cMult}, true);
      histos.add("GlobalMult_vs_FV0A", "GlobalMult_vs_FV0A", kTH2F,
                 {axisMult, axisFv0aMult}, true);
      histos.add("NPVtracks_vs_GlobalMult", "NPVtracks_vs_GlobalMult", kTH2F,
                 {axisPV, axisMult}, true);
    }

    if (doprocessStrangeYield) {
      histos.add("hzvtxcent", "hzvtxcent", kTH2D, {axisVtxZ, centAxis}, false);
      histos.add("K0sCentEtaMass", "K0sCentEtaMass", kTH3D,
                 {centAxis, axisEta, axisMassK0s}, false);
      histos.add("LambdaCentEtaMass", "LambdaCentEtaMass", kTH3D,
                 {centAxis, axisEta, axisMassLambda}, false);
      histos.add("AntiLambdaCentEtaMass", "AntiLambdaCentEtaMass", kTH3D,
                 {centAxis, axisEta, axisMassLambda}, false);
    }

    if (doprocessppData) {
      histos.add("MultPercentileHist", "MultPercentileHist", kTH1D, {axisCent},
                 false);
      histos.add("hdatazvtxmultpp", "hdatazvtxmultpp", kTH2D,
                 {axisVtxZ, centAxis}, false);
      histos.add("PhiVsEtaHistpp", "PhiVsEtaHistpp", kTH2D, {axisPhi2, axisEta},
                 false);
      histos.add("hdatadndetapp", "hdatadndetapp", kTHnSparseD,
                 {axisVtxZ, centAxis, axisEta, axisPhi, axisTrackType}, false);
    }

    if (doprocessppMonteCarlo) {
      histos.add("MultPercentileMCRecHist", "MultPercentileMCRecHist", kTH1D,
                 {axisCent}, false);
      histos.add("hmczvtxmultpp", "hmczvtxmultpp", kTH2D, {axisVtxZ, centAxis},
                 false);
  ...
  bool isTrackSelected(CheckTrack const& track)
  {
    ...
    if (isApplyExtraPhiCut &&
        ((track.phi() > extraphicut1 && track.phi() < extraphicut2) ||
         track.phi() <= extraphicut3 || track.phi() >= extraphicut4)) {
      return false;
    }
    return true;
  }
                   ...
                   FilTrackDataTable const& tracks)
  {
    ...
    for (const auto& track : tracks) {
      ...
      histos.fill(HIST("hdatadndeta"), cols.posZ(), selColCent(cols),
                  selColOccu(cols), track.eta(), track.phi(), kGlobalplusITS);
      if (track.hasTPC()) {
        histos.fill(HIST("hdatadndeta"), cols.posZ(), selColCent(cols),
                    selColOccu(cols), track.eta(), track.phi(), kGlobalonly);
      } else {
        histos.fill(HIST("hdatadndeta"), cols.posZ(), selColCent(cols),
                    selColOccu(cols), track.eta(), track.phi(), kITSonly);
      }
    }
  }
                          ...
                          FilTrackDataTable const& tracks)
  {
    ...
    histos.fill(HIST("NPVtracks_vs_FT0C"), cols.multNTracksPV(),
                cols.multFT0C());
    histos.fill(HIST("GlobalMult_vs_FV0A"), nchTracks, cols.multFV0A());
    histos.fill(HIST("NPVtracks_vs_GlobalMult"), cols.multNTracksPV(),
                nchTracks);
  }
                         ...
                         FilTrackMCRecTable const& RecTracks)
  {
    for (const auto& RecCol : RecCols) {
      ...
      histos.fill(HIST("hmczvtxcent"), RecCol.posZ(), selColCent(RecCol),
                  selColOccu(RecCol));
      ...
      for (const auto& Rectrack : recTracksPart) {
        ...
        histos.fill(HIST("hmcrecdndeta"), RecCol.posZ(), selColCent(RecCol),
                    selColOccu(RecCol), Rectrack.eta(), Rectrack.phi(),
                    static_cast<double>(kSpAll), kGlobalplusITS);
        if (Rectrack.hasTPC()) {
          histos.fill(HIST("hmcrecdndeta"), RecCol.posZ(), selColCent(RecCol),
                      selColOccu(RecCol), Rectrack.eta(), Rectrack.phi(),
                      static_cast<double>(kSpAll), kGlobalonly);
        } else {
          histos.fill(HIST("hmcrecdndeta"), RecCol.posZ(), selColCent(RecCol),
                      selColOccu(RecCol), Rectrack.eta(), Rectrack.phi(),
                      static_cast<double>(kSpAll), kITSonly);
        }
        ...
        if (Rectrack.has_mcParticle()) {
          ...
          if (find(mclabels.begin(), mclabels.end(), Rectrack.mcParticleId()) !=
              mclabels.end()) {
            pid = kBkg;
          }
          ...
          histos.fill(HIST("hmcrecdndeta"), RecCol.posZ(), selColCent(RecCol),
                      selColOccu(RecCol), Rectrack.eta(), Rectrack.phi(),
                      static_cast<double>(pid), kGlobalplusITS);
        } else {
          histos.fill(HIST("hmcrecdndeta"), RecCol.posZ(), selColCent(RecCol),
                      selColOccu(RecCol), Rectrack.eta(), Rectrack.phi(),
                      static_cast<double>(kBkg), kGlobalplusITS);
        }
      } // track (mcrec) loop
      ...
      for (const auto& particle : GenParticles) {
        ...
        histos.fill(HIST("hmcgendndeta"), RecCol.posZ(), selColCent(RecCol),
                    particle.eta(), particle.phi(), static_cast<double>(kSpAll),
                    kNoGenpTVar);
        if (particle.pt() < kMinpTcut) {
          histos.fill(HIST("hmcgendndeta"), RecCol.posZ(), selColCent(RecCol),
                      particle.eta(), particle.phi(),
                      static_cast<double>(kSpAll), kGenpTup,
                      -10.0 * particle.pt() + 2);
          histos.fill(HIST("hmcgendndeta"), RecCol.posZ(), selColCent(RecCol),
                      particle.eta(), particle.phi(),
                      static_cast<double>(kSpAll), kGenpTdown,
                      5.0 * particle.pt() + 0.5);
        } else {
          histos.fill(HIST("hmcgendndeta"), RecCol.posZ(), selColCent(RecCol),
                      particle.eta(), particle.phi(),
    ...
    for (const auto& RecCol : RecCols) {
      ...
      for (const auto& Rectrack : recTracksPart) {
        ...
        if (Rectrack.has_mcParticle()) {
          ...
          if (mcpart.isPhysicalPrimary()) {
            histos.fill(HIST("hmcrecdndpt"), selColCent(RecCol),
                        selColOccu(RecCol), kGlobalplusITS, mcpart.pt());
            if (Rectrack.hasTPC()) {
              histos.fill(HIST("hmcrecdndpt"), selColCent(RecCol),
                          selColOccu(RecCol), kGlobalonly, mcpart.pt());
            } else {
              histos.fill(HIST("hmcrecdndpt"), selColCent(RecCol),
                          selColOccu(RecCol), kITSonly, mcpart.pt());
            }
          }
        }
      }
      ...
      for (const auto& particle : GenParticles) {
        ...
        histos.fill(HIST("hmcgendndpt"), selColCent(RecCol), particle.pt(),
                    kNoGenpTVar);
        if (particle.pt() < kMinpTcut) {
          histos.fill(HIST("hmcgendndpt"), selColCent(RecCol), particle.pt(),
                      kGenpTup, -10.0 * particle.pt() + 2);
          histos.fill(HIST("hmcgendndpt"), selColCent(RecCol), particle.pt(),
                      kGenpTdown, 5.0 * particle.pt() + 0.5);
        } else {
          histos.fill(HIST("hmcgendndpt"), selColCent(RecCol), particle.pt(),
                      kGenpTup);
          histos.fill(HIST("hmcgendndpt"), selColCent(RecCol), particle.pt(),
                           ...
                           aod::V0Datas const& v0data)
  {
    ...
    for (const auto& v0track : v0data) {
      ...
      if (std::abs(v0track.dcapostopv()) < dcapostopvCut ||
          std::abs(v0track.dcanegtopv()) < dcanegtopvCut ||
          v0track.v0radius() < v0radiusCut || v0track.v0cosPA() < v0cospaCut ||
          std::abs(v0track.dcaV0daughters()) > dcav0daughtercut) {
        continue;
      }
      histos.fill(HIST("K0sCentEtaMass"), selColCent(cols), v0track.eta(),
                  v0track.mK0Short());
      histos.fill(HIST("LambdaCentEtaMass"), selColCent(cols), v0track.eta(),
                  v0track.mLambda());
      histos.fill(HIST("AntiLambdaCentEtaMass"), selColCent(cols),
                  v0track.eta(), v0track.mAntiLambda());
    }
  }
                     ...
                     FilTrackDataTable const& tracks)
  {
    ...
    if (nTrks > 0) {
      ...
      for (const auto& track : tracks) {
        ...
        histos.fill(HIST("hdatadndetapp"), cols.posZ(), cols.centFT0M(),
                    track.eta(), track.phi(), kGlobalplusITS);
        if (track.hasTPC()) {
          histos.fill(HIST("hdatadndetapp"), cols.posZ(), cols.centFT0M(),
                      track.eta(), track.phi(), kGlobalonly);
        } else {
          histos.fill(HIST("hdatadndetapp"), cols.posZ(), cols.centFT0M(),
                      track.eta(), track.phi(), kITSonly);
        }
      } // track loop
    } // nTrks>0
  }
    ...
    for (const auto& RecCol : RecCols) {
      ...
      if (nTrks > 0) {
        ...
        for (const auto& Rectrack : recTracksPart) {
          ...
          histos.fill(HIST("MCrecPhiVsEtaHistpp"), Rectrack.phi(),
                      Rectrack.eta());
          histos.fill(HIST("hmcrecdndetapp"), RecCol.posZ(), RecCol.centFT0M(),
                      Rectrack.eta(), Rectrack.phi(),
                      static_cast<double>(kSpAll), kGlobalplusITS);
          if (Rectrack.hasTPC()) {
            histos.fill(HIST("hmcrecdndetapp"), RecCol.posZ(),
                        RecCol.centFT0M(), Rectrack.eta(), Rectrack.phi(),
                        static_cast<double>(kSpAll), kGlobalonly);
          } else {
            histos.fill(HIST("hmcrecdndetapp"), RecCol.posZ(),
                        RecCol.centFT0M(), Rectrack.eta(), Rectrack.phi(),
          ...
          if (Rectrack.has_mcParticle()) {
            ...
            if (find(mclabels.begin(), mclabels.end(),
                     Rectrack.mcParticleId()) != mclabels.end()) {
              pid = kBkg;
            }
            mclabels.push_back(Rectrack.mcParticleId());
            histos.fill(HIST("hmcrecdndetapp"), RecCol.posZ(),
        ...
        for (const auto& particle : GenParticles) {
          ...
          histos.fill(HIST("hmcgendndetapp"), RecCol.posZ(), RecCol.centFT0M(),
                      particle.eta(), particle.phi(),
                      static_cast<double>(kSpAll), kNoGenpTVar);
          if (particle.pt() < kMinpTcut) {
            histos.fill(HIST("hmcgendndetapp"), RecCol.posZ(),
                        RecCol.centFT0M(), particle.eta(), particle.phi(),
                        static_cast<double>(kSpAll), kGenpTup,
                        -10.0 * particle.pt() + 2);
            histos.fill(HIST("hmcgendndetapp"), RecCol.posZ(),
                        RecCol.centFT0M(), particle.eta(), particle.phi(),
  ...
  PROCESS_SWITCH(HeavyionMultiplicity, processppMonteCarlo, "process pp MC",
                 false);
};
