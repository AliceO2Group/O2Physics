...
struct UccZdc {
    ...
    void processQA(o2::aod::ColEvSels::iterator const& collision, o2::aod::BCsRun3 const& /**/, aod::Zdcs const& /**/, aod::FV0As const& /**/, aod::FT0s const& /**/, TheFilteredTracks const& tracks)
    {
        ...
        double aT0C{-999.};
        if (foundBC.has_ft0()) {
            for (const auto& amplitude : foundBC.ft0().amplitudeA()) { aT0A += amplitude; }
            for (const auto& amplitude : foundBC.ft0().amplitudeC()) { aT0C += amplitude; }
        } else { return; }
        ...
        // TDC cut
        if (isTDCcut) {
            if (std::sqrt(std::pow(tZDCdif, 2.) + std::pow(tZDCsum, 2.)) > tdcCut) { return; }
        ...
        for (const auto& track : tracks) {
            if (track.hasITS() && ((track.eta() > minEta) && (track.eta() < maxEta))) { itsTracks++; }
            // Track Selection
            if (!track.isGlobalTrack()) { continue; }
            if ((track.pt() < minPt) || (track.pt() > maxPt)) { continue; }
            if ((track.eta() < minEta) || (track.eta() > maxEta)) { continue; }
            glbTracks++;
        }
        ...
        if (useMidRapNchSel) {
            ...
            if (!(cfgNch.hMeanNch && cfgNch.hSigmaNch)) return;
            
            TString s1 = TString(detector4Calibration.value);
            double xEval{1.};
            if (s1=="T0M") { xEval = normT0M; }
            if (s1=="V0A") { xEval = normV0A; }
            if (s1=="NPV") { xEval = nPV; }
        ...
        if(useMidRapNchSel && skipEvent) { return; }
        
        double sumpt{0.};
        for (const auto& track : tracks) {
            // Track Selection
            if (!track.isGlobalTrack()) { continue; }
            if ((track.pt() < minPt) || (track.pt() > maxPtSpectra)) { continue; }
            if ((track.eta() < minEta) || (track.eta() > maxEta)) { continue; }
        ...
        if (glbTracks >= minNchSel) registry.fill(HIST("NchVsOneParCorr"), glbTracks, sumpt / glbTracks);
    }
    ...
    void processZdcCollAss(o2::aod::ColEvSels::iterator const& collision, o2::aod::BCsRun3 const& /*bcs*/, aod::Zdcs const& /*zdcs*/, aod::FV0As const& /*fv0as*/, aod::FT0s const& /*ft0s*/, TheFilteredTracks const& tracks)
    {
        ...
        if (sZDC=="ZNA") { sumZNs = znA; }
        else if (sZDC=="ZNC") { sumZNs = znC; }
        else { sumZNs = (znA + znC); }
        ...
        if (applyEff || applyFD) {
            ...
            if (!(cfg.hEfficiency && cfg.hFeedDown)) return;
            ...
            for (const auto& track : tracks) {
                ...
                if (applyFD) fdValue = cfg.hFeedDown->GetBinContent(foundNchBin,foundPtBin);
                
                if ((effValue > 0.) && (fdValue > 0.)) { nchMult += (std::pow(effValue,-1.) * fdValue); }
            }
            
            if (applyEff && !correctNch) { nchMult = glbTracks; }
            
            // Reject low-multiplcicity events
            if (nchMult < minNchSel) { return; }
    ...
    void processMCclosure(aod::McCollisions::iterator const& mccollision,soa::SmallGroups<o2::aod::SimCollisions> const& collisions,o2::aod::BCsRun3 const& /*bcs*/,aod::FT0s const& /*ft0s*/, aod::FV0As const& /*fv0as*/,aod::McParticles const& mcParticles, TheFilteredSimTracks const& simTracks)
    {
        for (const auto& collision : collisions) {
            ...
            if (skipRecoColGTOne && (collisions.size() > kOne)) { continue; }
            ...
            if (rndNum >= kZero && rndNum < evtFracMCcl) {
                ...
                loadCorrections(foundBC.timestamp());
                if (!(cfg.hEfficiency && cfg.hFeedDown)) { continue; }
                ...
                for (const auto& track : groupedTracks) {
                    ...
                    if (pdgParticle != nullptr) { charge = pdgParticle->Charge(); }
                    else { continue; }
                    
                    // Is it a charged particle?
                    if (std::abs(charge) < kMinCharge) { continue; }
                ...
                for (const auto& particle : mcParticles) {
                    if (particle.eta() < minEta || particle.eta() > maxEta) { continue; }
                    if (particle.pt() < minPt || particle.pt() > maxPt) { continue; }
    ...
    void eventSamplingMC(const T& mcParticles, const V& timeStamp)
    {
        ...
        for (int replica = 0; replica < kSizeBootStrapEnsemble; ++replica) vPoisson.emplace_back(rndGen.Poisson(1.));
    ...
    void eventSamplingMCRec(const T& tracks, const V& timeStamp)
    {
        ...
        for (int replica = 0; replica < kSizeBootStrapEnsemble; ++replica) {
            ...
            for (uint64_t evtRep = 0; evtRep < vPoisson.at(replica); ++evtRep) {
                ...
                if (glbTracks < minNchSel) { continue; }
                ...
                if (!applyEff) nchMult = static_cast<double>(glbTracks);
                if (applyEff && !correctNch) nchMult = static_cast<double>(glbTracks);
