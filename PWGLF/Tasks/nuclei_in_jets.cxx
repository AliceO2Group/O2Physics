...
struct nuclei_in_jets {
  ...
  void processData(SelectedCollisions::iterator const& collision, FullTracks const& tracks)
  {
    ...
    do {
      ...
      for (int i = 0; i < (int)particle_ID.size(); i++) {
      ...
      if (nPartAssociated >= ((int)particle_ID.size() - 1))
        exit = 1;
      if (distance_jet_min > distance_bkg_min)
        exit = 2;
    ...
    registryQC.fill(HIST("ue_multiplicity"), (int)ue_particle_ID.size() / 4);

    // Jet Multiplicity
    int jet_Nch = (int)jet_particle_ID.size() - (int)ue_particle_ID.size() / 4;
    registryQC.fill(HIST("jet_multiplicity"), jet_Nch);

    // Loop over particles inside Jet
    for (int i = 0; i < (int)jet_particle_ID.size(); i++) {
    ...
    for (int i = 0; i < (int)ue_particle_ID.size(); i++) {
