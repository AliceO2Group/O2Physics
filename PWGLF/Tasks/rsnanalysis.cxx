...
struct rsn_analysis {
  ...
  //
                 ...
                 kTracksTable const& kTracks)
    {
      ...
      for (auto kCurrentTrack : kTracks) {
        //
        //  Track Selection
          if ( !uIsTrackSelected(kCurrentTrack) ) continue;
        //    PID QA
                                       ...
                                       aod::McParticles const& mcParticles)
    {
      ...
      for (auto kCurrentTrack : kTracks) {
        ...
        //    PID QA
          uFillFullPIDQA(kCurrentTrack);
          //
          if ( !kCurrentTrack.has_mcParticle() ) continue;
          //
        if (uIsKaonSelected(kCurrentTrack) ) {
          if (kCurrentTrack.sign() > 0)
      ...
      for (auto kPosKaon : kPosSelectedKaons) {
        ...
        for (auto kNegKaon : kNegSelectedKaons) {
          ...
          //
            ...
            if ( !kPositiveDecayDaughterMC.has_mothers() || !kNegativeDecayDaughterMC.has_mothers() ) continue;
            //  Check they both have 1 mother
            if ( ( kPositiveDecayDaughterMC.mothersIds().size() != 1 ) || ( kNegativeDecayDaughterMC.mothersIds().size() != 1 ) ) continue;
            //  Check they both have the same mother
            if ( *kPositiveDecayDaughterMC.mothersIds().begin() == *kNegativeDecayDaughterMC.mothersIds().begin() ) continue;
            ...
            if ( kResonanceMCTruthParticle.pdgCode() != 333 ) continue;
            if ( fabs(kResonanceMCTruthParticle.y()) > 0.5 ) continue;
            //
      ...
      for (auto kPosKaon : kPosSelectedKaons) {
        for (auto kNegPion : kNegSelectedPions) {
          ...
          //
            ...
            //  Check this mother is a phi meson within rapidity
            auto kResonanceMCTruthParticle = *kPositiveDecayDaughterMC.mothers_as<aod::McParticles>().begin();
            if ( fabs(kResonanceMCTruthParticle.pdgCode()) != 313 ) continue;
            if ( fabs(kResonanceMCTruthParticle.y()) > 0.5 ) continue;
            //
            lPositiveDecayDaughterMC.SetXYZM(kPositiveDecayDaughterMC.px(),kPositiveDecayDaughterMC.py(),kPositiveDecayDaughterMC.pz(),.493677);
    ...
    void processMCTruth(aod::McCollision const& mcCollision,
        ...
        for ( auto kCurrentParticle : mcParticles ) {
            //
            if ( !kCurrentParticle.producedByGenerator() ) continue;
            if ( fabs(kCurrentParticle.y()) > 0.5 ) continue;
            //
            if ( abs(kCurrentParticle.pdgCode()) == 333 ) {
                ...
                if ( kDaughters.size() == 2 ) {
                    for ( auto kCurrentDaughter : kDaughters ) {
                        if ( kCurrentDaughter.pdgCode() == +321 )    kHasKaonp = true;
                        if ( kCurrentDaughter.pdgCode() == -321 )    kHasKaonm = true;
                    }
                }
                if ( kHasKaonp && kHasKaonm )   uHistograms.fill(HIST("Analysis/Phi/Generated"),
                                                                kCurrentParticle.pt());
            }
            //
            if ( abs(kCurrentParticle.pdgCode()) == 313 ) {
                ...
                if ( kDaughters.size() == 2 ) {
                    for ( auto kCurrentDaughter : kDaughters ) {
                        if ( fabs(kCurrentDaughter.pdgCode()) == 321 )    kHasKaonp = true;
                        if ( fabs(kCurrentDaughter.pdgCode()) == 211 )    kHasKaonm = true;
                    }
                }
                if ( kHasKaonp && kHasKaonm )   uHistograms.fill(HIST("Analysis/Kstar/Generated"),
                                                                kCurrentParticle.pt());
            }
        }
    }
  ...
  bool uIsTrackSelected(kCurrentTrackType const& kCurrentTrack) {
      //
      //    Selection
      if (!kCurrentTrack.isGlobalTrack()) return false;
      //
      //    Custom Selection
