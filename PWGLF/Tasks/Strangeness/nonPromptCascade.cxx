...
std::tuple<bool, bool> isFromHF(auto& particle)
{
...
struct NonPromptCascadeTask {
  ...
  void fillCandidatesVector(auto const& collisions, auto const& trackedCascades)
  {
    using passedCollisionType = decltype(collisions);
    using collisionType = std::remove_cvref_t<passedCollisionType>;
    ...
    for (const auto& trackedCascade : trackedCascades) {
      ...
      candidates.emplace_back(NPCascCandidate{mcParticleID, track.globalIndex(), ITStrack.globalIndex(), trackedCascade.collisionId(), trackedCascade.matchingChi2(), deltaPtITSCascade, trackedCascade.itsClsSize(), hasReassociatedClusters, isGoodMatch, isGoodCascade, pdgCodeMom, itsTrackPDG, std::get<0>(fromHF), std::get<1>(fromHF),
