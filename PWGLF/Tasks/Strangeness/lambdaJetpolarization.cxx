...
struct myAnalysis {
  ...
  int nEvents = 0;
  void processJetV0Analysis(aod::JCollision const& collision, soa::Filtered<soa::Join<aod::ChargedJets, aod::ChargedJetConstituents>> const& chargedjets, soa::Join<aod::JTracks, aod::JTrackPIs> const& tracks, TrackCandidates const&)
  {
    nEvents++;
  }
  PROCESS_SWITCH(myAnalysis, processJetV0Analysis, "processJetV0Analysis", false);
};
...
struct LfMyV0s {
  ...
  void init(InitContext const&)
  {
    ...
    const AxisSpec axisPx{200, -10, 10, "#px (GeV/c)"};
    const AxisSpec axisPy{200, -10, 10, "#py (GeV/c)"};
    const AxisSpec axisPz{200, -10, 10, "#pz (GeV/c)"};
    ...
    registry.add("V0pT", "V0pT", kTH1F, {axisPT});
    ...
    registry.add("V0py", "V0py", kTH1F, {axisPy});
    registry.add("V0pz", "V0pz", kTH1F, {axisPz});

    registry.add("Jetpx", "Jetpx", kTH1F, {axisPx});
    registry.add("Jetpy", "Jetpy", kTH1F, {axisPy});
    registry.add("Jetpz", "Jetpz", kTH1F, {axisPz});
    registry.add("JetpT", "JetpT", kTH1F, {axisPT});
  ...
  void processJetV0Analysis(aod::MyCollision const& collision, aod::MyTable const& myv0s, aod::MyTableJet const& myJets)
  {
    ...
    for (auto& candidate : myv0s) {
      ...
      registry.fill(HIST("V0pT"), candidate.v0pt());
      registry.fill(HIST("hMassVsPtLambda"), candidate.v0pt(), candidate.v0Lambdamass());
      registry.fill(HIST("V0px"), candidate.v0px());
      registry.fill(HIST("V0py"), candidate.v0py());
      registry.fill(HIST("V0pz"), candidate.v0pz());
      registry.fill(HIST("V0protonpx"), candidate.v0protonpx());
      registry.fill(HIST("V0protonpy"), candidate.v0protonpy());
      registry.fill(HIST("V0protonpz"), candidate.v0protonpz());
    }
    for (auto& Jet : myJets) {
      JetNumbers++;
      registry.fill(HIST("Jetpx"), Jet.jetpx());
      registry.fill(HIST("Jetpy"), Jet.jetpy());
      registry.fill(HIST("Jetpz"), Jet.jetpz());
      registry.fill(HIST("JetpT"), Jet.jetpt());
    }
  }
