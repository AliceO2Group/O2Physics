...
struct lambdajetpolarizationions {
    ...
    Configurable<bool> doCompleteTopoQA{"doCompleteTopoQA", false, "do topological variable QA histograms"}; // Includes doPlainTopoQA from derivedlambdakzeroanalysis
    ...
    fastjet::JetAlgorithm mapFJAlgorithm(int algoIdx){
        switch (algoIdx) {
            case 0: return fastjet::kt_algorithm;
            case 1: return fastjet::cambridge_algorithm;
            case 2: return fastjet::antikt_algorithm;
            default:
    ...
    fastjet::RecombinationScheme mapFJRecombScheme(int schemeIdx){
        switch (schemeIdx){
            case 0: return fastjet::E_scheme;
            case 1: return fastjet::pt_scheme;
            case 2: return fastjet::pt2_scheme;
            case 7: return fastjet::WTA_pt_scheme;
            default:
    ...
    void init(InitContext const&){ // (TODO: add all useful histograms here! Add flags for QA plots and the such too)
        ...
        //////////////////////////////////////////////////////////////
        struct CutLabel{std::string label;bool enabled;}; // A method of hiding labels of selections which were not used!
        std::vector<CutLabel> v0LambdaSelectionLabels = {
            {"All V0 candidates",true},
            {"V0 radius (min)",true},{"V0 radius (max)",true},{"V0 cosPA",true},{"DCA_{V0 daughters}",true},{"|y_{#Lambda}|",v0Selections.rapidityCut>0.f},
            {"K^{0}_{S} mass rejection",v0Selections.compMassRejection>=0.f},
            {"ITS clusters (pos)",v0Selections.minITSclusters>0},{"ITS #chi^{2}/N_{cls} (pos)",v0Selections.maxITSchi2PerNcls<1e8},
            {"Reject ITS afterburner (pos)",v0Selections.rejectPosITSafterburner},{"Require ITS afterburner (pos)",v0Selections.requirePosITSafterburnerOnly},
            {"ITS clusters (neg)",v0Selections.minITSclusters>0},{"ITS #chi^{2}/N_{cls} (neg)",v0Selections.maxITSchi2PerNcls<1e8},
            {"Reject ITS afterburner (neg)",v0Selections.rejectNegITSafterburner},{"Require ITS afterburner (neg)",v0Selections.requireNegITSafterburnerOnly},
            {"TPC crossed rows (pos)",v0Selections.minTPCrows>0},{"TPC #chi^{2}/N_{cls} (pos)",v0Selections.maxTPCchi2PerNcls<1e8},
            {"TPC rows / findable (pos)",v0Selections.minTPCrowsOverFindableClusters>=0},{"TPC found / findable (pos)",v0Selections.minTPCfoundOverFindableClusters>=0},
            {"TPC shared clusters (pos)",v0Selections.maxFractionTPCSharedClusters<1e8},{"TPC sector boundary (pos)",v0Selections.rejectTPCsectorBoundary},
            {"TPC crossed rows (neg)",v0Selections.minTPCrows>0},{"TPC #chi^{2}/N_{cls} (neg)",v0Selections.maxTPCchi2PerNcls<1e8},
            {"TPC rows / findable (neg)",v0Selections.minTPCrowsOverFindableClusters>=0},{"TPC found / findable (neg)",v0Selections.minTPCfoundOverFindableClusters>=0},
            {"TPC shared clusters (neg)",v0Selections.maxFractionTPCSharedClusters<1e8},{"TPC sector boundary (neg)",v0Selections.rejectTPCsectorBoundary},
            {"Require ITS-only (pos)",v0Selections.requirePosITSonly},{"Require ITS-only (neg)",v0Selections.requireNegITSonly},
            {"Reject TPC-only (pos)",v0Selections.skipTPConly},{"Reject TPC-only (neg)",v0Selections.skipTPConly},
        ...
        auto addHypothesis = [&](bool isLambda, bool analysisEnabled) {
            if (!analysisEnabled) return; // i.e., don't add these labels if not analyzing said particle type
            std::string p = isLambda ? "#Lambda: " : "#bar{#Lambda}: ";
            v0LambdaSelectionLabels.insert(v0LambdaSelectionLabels.end(), {
                {p + "DCA_{p} to PV", true},
        ...
        for(size_t i=0; i<v0LambdaSelectionLabels.size(); ++i){
            ...
            if(!v0LambdaSelectionLabels[i].enabled) lbl="#color[16]{(off) "+lbl+"}";
    ...
    bool isEventAccepted(TCollision const& collision, float centrality, bool fillHists){ // check whether the collision passes our collision selections
        ...
        if (std::abs(collisionPVz) > eventSelections.maxZVtxPosition) return false;
        if (fillHists) fillEventSelectionQA(++selectionIdx, centrality);
        ...
        if (doPPAnalysis) { // we are in pp
            ...
            const double interactionRate = (eventSelections.minIR >= 0 || eventSelections.maxIR >= 0) ? rateFetcher.fetch(ccdb.service, collision.timestamp(), collision.runNumber(), irSource) * 1.e-3 : -1;
    ...
    bool isCandidateForChargedPseudojetAccepted(JetCandidate const& track){ // (TODO: add an equivalent for photon jets and Z jets, which don't consider charged particles)
    ...
    bool passesGenericV0Cuts(TV0 const& v0){ // (TODO: cache the posTrackExtra and neg track objects outside the V0cuts and hypothesis dependent cuts and then pass them by reference? May be quicker!)
        ...
        // pseudorapidity cuts:
        if (std::fabs(v0.yLambda()) > v0Selections.rapidityCut) return false; // Not using rapidity in selections for now. Just detector acceptance.
        V0SelCounter.fill();
        ...
        const auto posTrackExtra = v0.template posTrackExtra_as<DauTracks>(); // (TODO: is it worth it to cache these transformations outside of the function? They are reused in the Lambda hypothesis checks)
    ...
    bool passesLambdaLambdaBarHypothesis(TV0 const& v0, TCollision const& collision, bool Lambda_hypothesis){
        ...
        if (dcaProtonToPV < v0Selections.dcaProtonToPV) return false;
        V0SelCounter.fill();
        ...
        if (dcaPionToPV < v0Selections.dcaPionToPV) return false;
        V0SelCounter.fill();
    ...
    void processJetsData(SelCollisions::iterator const& collision, PseudoJetTracks const& tracks){
        float centrality = -1.0f; // Just a placeholder
        if (!isEventAccepted(collision, centrality, false)) return; // Uses return instead of continue, as there is no explicit loop here
        ...
        for (auto const& track : tracks){
            ...
            if (!isCandidateForChargedPseudojetAccepted(track)) continue;
        ...
        if (jetConfigurations.bkgSubtraction){
            ...
            for (const auto& jet : jets){
                ...
                if (std::fabs(jet_eta) > (0.9f - jetConfigurations.radiusJet)) continue;
            ...
            if (doJetKinematicsQA){
                ...
                for (const auto& jet : jets) {
                    ...
                    histos.fill(HIST("JetKinematicsQA/h2dJetsPerEventvsJetPt"), selectedJets, jet.pt());
        ...
        else{ // Otherwise, simple jet clustering
