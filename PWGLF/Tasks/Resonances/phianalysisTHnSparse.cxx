...
struct phianalysisTHnSparse {
  ...
  ConfigurableAxis rapidityaxis{"rapidity-axis", {10., -1.0 * (float)cut.y, (float)cut.y}, "Rapidity axis binning."};
  ConfigurableAxis nsigmaaxisPos{"nsigma-pos-axis", {1, 0., (float)cut.tpcnSigmaPos}, "NSigma of positive particle axis binning in THnSparse."};
  ConfigurableAxis nsigmaaxisNeg{"nsigma-neg-axis", {1, 0., (float)cut.tpcnSigmaNeg}, "NSigma of negative particle axis binning in THnSparse."};
  ...
  Filter vtxFilter = (nabs(o2::aod::collision::posZ) < (float)cut.vZ);

  Filter ptFilter = nabs(aod::track::pt) > (float)cut.pt;
  Filter etaFilter = nabs(aod::track::eta) < (float)cut.y;
  Filter dcaFilter = (nabs(o2::aod::track::dcaXY) < (float)cut.dcaXY) && (nabs(o2::aod::track::dcaZ) < (float)cut.dcaZ);
  ...
  Partition<TrackCandidates> positive = (aod::track::signed1Pt > 0.0f) && (nabs(o2::aod::pidtpc::tpcNSigmaKa) < (float)cut.tpcnSigmaPos);
  Partition<TrackCandidates> negative = (aod::track::signed1Pt < 0.0f) && (nabs(o2::aod::pidtpc::tpcNSigmaKa) < (float)cut.tpcnSigmaNeg);

  Partition<TrackCandidatesMC> positiveMC = (aod::track::signed1Pt > 0.0f) && (nabs(o2::aod::pidtpc::tpcNSigmaKa) < (float)cut.tpcnSigmaPos);
  Partition<TrackCandidatesMC> negativeMC = (aod::track::signed1Pt < 0.0f) && (nabs(o2::aod::pidtpc::tpcNSigmaKa) < (float)cut.tpcnSigmaNeg);
  ...
  void init(o2::framework::InitContext&)
  {
    ...
    rsnOutput->init(sparseAxes, allAxes, (bool)produce.True, (bool)produce.eventMixing, (bool)produce.Likesign, &registry);
    ...
    if ((bool)produce.QA) {
      ...
      registry.add("QATrack/unlikepm/TPCPID/h2TracknSigma", "", kTH2F, {{120, -(float)cut.tpcnSigmaPos, (float)cut.tpcnSigmaPos}, {120, -(float)cut.tpcnSigmaNeg, (float)cut.tpcnSigmaNeg}});
  ...
  void processData(EventCandidate const& collision, TrackCandidates const& tracks)
  {
    ...
    if ((bool)verbose.ignorezeroevent && collision.globalIndex() == 0) {
      if ((int)verbose.verboselevel > 0)
    ...
    if ((int)verbose.verboselevel > 0 && (int)verbose.refresh > 0 && collision.globalIndex() % (int)verbose.refresh == (int)verbose.refresh_index)
  ...
  void processTrue(EventCandidatesMC::iterator const& collision, TrackCandidatesMC const& tracks, aod::McParticles const& mcParticles, aod::McCollisions const& mcCollisions)
  {
    ...
    if (std::abs(collision.posZ()) > (float)cut.vZ)
      return;
    ...
    for (auto& [track1, track2] : combinations(o2::soa::CombinationsFullIndexPolicy(posDauthersMC, negDauthersMC))) {
      ...
      for (auto& mothertrack1 : mctrack1.mothers_as<aod::McParticles>()) {
        for (auto& mothertrack2 : mctrack2.mothers_as<aod::McParticles>()) {
          ...
          if (std::abs(mothertrack1.y()) > (float)cut.y)
            continue;

          if (std::abs(mothertrack2.y()) > (float)cut.y)
            continue;
  ...
  void processGen(aod::McCollision const& mcCollision, aod::McParticles const& mcParticles)
  {
    ...
    if (std::abs(mcCollision.posZ()) > (float)cut.vZ)
      return;

    int nuberofPhi = 0;

    for (auto& particle : mcParticles) {
      if (std::abs(particle.y()) > (float)cut.y)
        continue;
      ...
      if (particle.pdgCode() == motherPDG) {
        ...
        pointPair[static_cast<int>(o2::analysis::rsn::PairAxisType::pt)] = mother.Pt();
        pointPair[static_cast<int>(o2::analysis::rsn::PairAxisType::mu)] = multiplicity;
        pointPair[static_cast<int>(o2::analysis::rsn::PairAxisType::ns1)] = std::abs((float)cut.tpcnSigmaPos / 2.0);
        pointPair[static_cast<int>(o2::analysis::rsn::PairAxisType::ns2)] = std::abs((float)cut.tpcnSigmaNeg / 2.0);
        pointPair[static_cast<int>(o2::analysis::rsn::PairAxisType::y)] = mother.Rapidity();
