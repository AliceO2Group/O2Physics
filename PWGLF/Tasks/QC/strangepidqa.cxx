...
// Invariant Mass with TOF selections
  ...
  void processReal(soa::Join<aod::StraCollisions, aod::StraCents>::iterator const& coll, soa::Join<aod::V0Cores, aod::V0CollRefs, aod::V0Extras, aod::V0TOFs, aod::V0TOFPIDs, aod::V0TOFBetas, aod::V0TOFDebugs> const& v0s)
  {
    ...
    if(coll.centFT0C() > maxCentrality || coll.centFT0C() < minCentrality) return;
    
    for (auto& lambda: v0s) { // selecting photons from Sigma0

      if(TMath::Abs(lambda.eta())>0.5) continue;

      if(TMath::Abs(lambda.mLambda()-1.115683)<0.01 && lambda.v0cosPA() > minCosPA){ 
        histos.fill(HIST("hPositiveStatus"), 0.0f);
        if(lambda.posTOFSignal()>1e-5 && lambda.posTOFEventTime()<0.0f) histos.fill(HIST("hPositiveStatus"), 1.0f);
        if(lambda.posTOFSignal()<0.0f && lambda.posTOFEventTime()>1e-5) histos.fill(HIST("hPositiveStatus"), 2.0f);
        if(lambda.posTOFSignal()>1e-5 && lambda.posTOFEventTime()>1e-5) histos.fill(HIST("hPositiveStatus"), 3.0f);

        histos.fill(HIST("hNegativeStatus"), 0.0f);
        if(lambda.negTOFSignal()>1e-5 && lambda.negTOFEventTime()<0.0f) histos.fill(HIST("hNegativeStatus"), 1.0f);
        if(lambda.negTOFSignal()<0.0f && lambda.negTOFEventTime()>1e-5) histos.fill(HIST("hNegativeStatus"), 2.0f);
        if(lambda.negTOFSignal()>1e-5 && lambda.negTOFEventTime()>1e-5) histos.fill(HIST("hNegativeStatus"), 3.0f);

        if(lambda.posTOFLength()>0.0f){
          histos.fill(HIST("hPositiveStatusReachedTOF"), 0.0f);
          if(lambda.posTOFSignal()>1e-5 && lambda.posTOFEventTime()<0.0f) histos.fill(HIST("hPositiveStatusReachedTOF"), 1.0f);
          if(lambda.posTOFSignal()<0.0f && lambda.posTOFEventTime()>1e-5) histos.fill(HIST("hPositiveStatusReachedTOF"), 2.0f);
          if(lambda.posTOFSignal()>1e-5 && lambda.posTOFEventTime()>1e-5) histos.fill(HIST("hPositiveStatusReachedTOF"), 3.0f);
        }

        if(lambda.negTOFLength()>0.0f){
          histos.fill(HIST("hNegativeStatusReachedTOF"), 0.0f);
          if(lambda.negTOFSignal()>1e-5 && lambda.negTOFEventTime()<0.0f) histos.fill(HIST("hNegativeStatusReachedTOF"), 1.0f);
          if(lambda.negTOFSignal()<0.0f && lambda.negTOFEventTime()>1e-5) histos.fill(HIST("hNegativeStatusReachedTOF"), 2.0f);
          if(lambda.negTOFSignal()>1e-5 && lambda.negTOFEventTime()>1e-5) histos.fill(HIST("hNegativeStatusReachedTOF"), 3.0f);
        }
      ...
      if(lambda.v0radius()<minV0Radius) continue;
      if(lambda.posTOFSignal()<0 && requireTOFsignalProton) continue;
      if(lambda.negTOFSignal()<0 && requireTOFsignalPion) continue; 
      ...
      if(lambda.posTOFEventTime()<0 && requireTOFEventTimeProton) continue;
      if(lambda.negTOFEventTime()<0 && requireTOFEventTimePion) continue;
      ...
      if(TMath::Abs(lambda.posTOFDeltaTLaPr())<maxDeltaTimeProton){ 
        ...
        if(lambda.pt()>minPtV0 && lambda.pt() < maxPtV0) histos.fill(HIST("hLambdaMass_ProtonTOF"), lambda.mLambda());
      }
      if(TMath::Abs(lambda.negTOFDeltaTLaPi())<maxDeltaTimeProton){ 
        ...
        if(lambda.pt()>minPtV0 && lambda.pt() < maxPtV0) histos.fill(HIST("hLambdaMass_PionTOF"), lambda.mLambda());
        if(TMath::Abs(lambda.posTOFDeltaTLaPr())<maxDeltaTimeProton){ 
          histos.fill(HIST("h2dLambdaMassVsPt_AllTOF"), lambda.pt(), lambda.mLambda());
          if(lambda.pt()>minPtV0 && lambda.pt() < maxPtV0) histos.fill(HIST("hLambdaMass_AllTOF"), lambda.mLambda());
        }
      }
      ...
      if(TMath::Abs(lambda.posTOFDeltaTLaPr())>maxDeltaTimeProton && TMath::Abs(lambda.posTOFDeltaTLaPr()) < 5e+4){ 
        ...
        if(lambda.pt()>minPtV0 && lambda.pt() < maxPtV0) histos.fill(HIST("hLambdaMass_InvertProtonTOF"), lambda.mLambda());
      }
      if(TMath::Abs(lambda.negTOFDeltaTLaPi())>maxDeltaTimeProton && TMath::Abs(lambda.negTOFDeltaTLaPi()) < 5e+4){ 
        ...
        if(lambda.pt()>minPtV0 && lambda.pt() < maxPtV0) histos.fill(HIST("hLambdaMass_InvertPionTOF"), lambda.mLambda());
        if(TMath::Abs(lambda.posTOFDeltaTLaPr())>maxDeltaTimeProton && TMath::Abs(lambda.posTOFDeltaTLaPr()) < 5e+4){ 
          histos.fill(HIST("h2dLambdaMassVsPt_InvertAllTOF"), lambda.pt(), lambda.mLambda());
          if(lambda.pt()>minPtV0 && lambda.pt() < maxPtV0) histos.fill(HIST("hLambdaMass_InvertAllTOF"), lambda.mLambda());
        }
      }
    ...
    void processSim(aod::StraCollision const& coll, soa::Join<aod::V0Cores, aod::V0CollRefs, aod::V0Extras, aod::V0MCDatas, aod::V0TOFs, aod::V0TOFPIDs, aod::V0TOFBetas> const& v0s)
  {
    for (auto& lambda: v0s) { // selecting photons from Sigma0
      ...
      if (!lambda.isPhysicalPrimary()) continue;
      if (lambda.pdgCodePositive()!=2212) continue;
      if (lambda.pdgCodeNegative()!=-211) continue;
