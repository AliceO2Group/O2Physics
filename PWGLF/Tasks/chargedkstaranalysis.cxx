...
///
/// \code for Charged KStar analysis
...
struct chargedkstaranalysis {
  ...
  void init(InitContext const&)
  {
    ...
    if (QAv0)
      {
	// K0s reconstruction
	histos.add("hMassvsptvsmult", "hMassvsptvsmult", {HistType::kTHnSparseF, {{K0ShortMassAxis}, {ptAxis},{multAxis}}}, true);
    ...
    //CKStar histograms
    histos.add("h3CKSInvMassUnlikeSign", "Invariant mass of CKS meson Unlike Sign", kTHnSparseF, {{200, 0.0, 200.0}, {200, 0.0f, 20.0f}, {90, 0.6, 1.5}}, true);
    histos.add("h3CKSInvMassMixed", "Invariant mass of CKS meson Mixed", kTHnSparseF, {{200, 0.0, 200.0}, {200, 0.0f, 20.0f}, {90, 0.6, 1.5}}, true);
  ...
  bool selectionPID(const T& candidate)
  {
    ...
    if (!candidate.hasTOF() && std::abs(candidate.tpcNSigmaPi()) < nsigmaCutTPC) {
      return true;
    }
    return false;
  }
  ...
  bool SelectionV0(Collision const& collision, V0 const& candidate, float multiplicity)
  {
	...
	histos.fill(HIST("hMassvsptvsmult"), candidate.mK0Short(), candidate.pt(), multiplicity);
  ...
  using BinningTypeCentralityC = ColumnBinningPolicy<aod::collision::PosZ, aod::cent::CentFT0C>;
  ...
  void processSE(EventCandidates::iterator const& collision, TrackCandidates const& tracks, aod::V0Datas const& V0s, aod::BCs const&) 
  {
      ...
      if (QAbefore)
	{
      ...
      if (!selectionPID(track1)) continue; //for primary particle PID
	...
	for (auto ikshort = kshorts.begin(); ikshort != kshorts.end(); ++ikshort) {
	  ...
	  if (TMath::Abs(CKSVector.Rapidity()) < 0.5)
	    {
	      histos.fill(HIST("h3CKSInvMassUnlikeSign"), multiplicity, CKSVector.Pt(), CKSVector.M());
	    }
	}
    ...
    
  {
	...
	if(!(selectionTrack(t1) || selectionPID(t1))) continue;
	if(!SelectionV0(c2,t2, multiplicity)) continue;
	...
	if (TMath::Abs(CKSmix.Rapidity()) < 0.5)
	  {
	    histos.fill(HIST("h3CKSInvMassMixed"), multiplicity, CKSmix.Pt(), CKSmix.M());
	  }
...
{
  return WorkflowSpec{
    adaptAnalysisTask<chargedkstaranalysis>(cfgc)};
}
