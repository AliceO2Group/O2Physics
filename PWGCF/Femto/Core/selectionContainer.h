...
enum LimitType { kUpperLimit,            ///< simple upper limit, e.g. p_T < 1 GeV/c
                 ...
                 kAbsLowerLimit,         ///< lower limit on the absolute value, e.g. |DCA_xyz| > 0.05 cm
...
class SelectionContainer
{
 public:
  ...
  ///        Only kEqualArray is supported; each element is compared against the corresponding threshold.
  ...
  /// \brief Check whether this container uses TF1 functions rather than static threshold values.
  /// \return True if function-based thresholds are configured.
