...
/// \file FemtoFlowObjectSelection.h
/// \brief FemtoFlowObjectSelection - Parent class of all selections
...
/// \class FemtoFlowObjectSelection
...
class FemtoFlowObjectSelection
{
 public:
  /// Destructor
  virtual ~FemtoFlowObjectSelection() = default;
  ...
  void fillSelectionHistogram()
  {
    ...
    auto hist = mHistogramRegistry->get<TH1>(HIST(o2::aod::femtoflowparticle::ParticleTypeName[part]) + HIST("/cuthist"));
  ...
  void setSelection(std::vector<FemtoFlowSelection<selValDataType, selVariable>>& sels)
  {
    ...
    switch (sels.at(0).getSelectionType()) {
      ...
      case (femto_flow_selection::SelectionType::kAbsUpperLimit):
        std::sort(sels.begin(), sels.end(), [](FemtoFlowSelection<selValDataType, selVariable> a, FemtoFlowSelection<selValDataType, selVariable> b) {
          return a.getSelectionValue() > b.getSelectionValue();
        });
        break;
      case (femto_flow_selection::SelectionType::kLowerLimit):
      case (femto_flow_selection::SelectionType::kAbsLowerLimit):
      case (femto_flow_selection::SelectionType::kEqual):
        std::sort(sels.begin(), sels.end(), [](FemtoFlowSelection<selValDataType, selVariable> a, FemtoFlowSelection<selValDataType, selVariable> b) {
          return a.getSelectionValue() < b.getSelectionValue();
        });
        break;
