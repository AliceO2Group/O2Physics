...
/// \file FemtoFlowCutculator.h
/// \brief FemtoFlowCutculator - small class to match bit-wise encoding and actual physics cuts
...
class FemtoFlowCutculator
{
 public:
  ...
  void init(const char* configFile)
  {
    ...
    std::vector<const char*> producerTasks = {"femto-flow-producer-task"};
  ...
  /// passed to the FemtoFlowTrack/V0/../Selection
  ...
  void setTrackSelection(femto_flow_track_selection::TrackSel obs,
                         femto_flow_selection::SelectionType type,
                         const char* prefix)
  {
    auto tmpVec = setSelection(FemtoFlowTrackSelection::getSelectionName(obs, prefix));
    if (tmpVec.size() > 0) {
  ...
  void setTrackSelectionFromFile(const char* prefix)
  {
    for (const auto& sel : mConfigTree) {
      ...
      femto_flow_track_selection::TrackSel obs;
      if (selName.find(prefix) != std::string::npos) {
        int index = FemtoFlowTrackSelection::findSelectionIndex(
          std::string_view(selName), prefix);
        if (index >= 0) {
          obs = femto_flow_track_selection::TrackSel(index);
        } else {
          continue;
        }
        if (obs == femto_flow_track_selection::TrackSel::kPIDnSigmaMax)
          continue; // kPIDnSigmaMax is a special case
        setTrackSelection(obs, FemtoFlowTrackSelection::getSelectionType(obs),
                          prefix);
      }
    }
  }
  ...
  // /// \param type Type of the track selection
  // /// \param prefix Prefix which is added to the name of the Configurable
  // void setV0Selection(femto_flow_v0_selection::V0Sel obs,
  //                     femto_flow_selection::SelectionType type,
  //                     const char* prefix)
  // {
  //   auto tmpVec =
  //     setSelection(FemtoFlowV0Selection::getSelectionName(obs, prefix));
  //   if (tmpVec.size() > 0) {
  ...
  //   for (const auto& sel : mConfigTree) {
  //     std::string selName = sel.first;
  //     femto_flow_v0_selection::V0Sel obs;
  //     if (selName.find(prefix) != std::string::npos) {
  //       int index = FemtoFlowV0Selection::findSelectionIndex(
  //         std::string_view(selName), prefix);
  //       if (index >= 0) {
  //         obs = femto_flow_v0_selection::V0Sel(index);
  //       } else {
  //         continue;
  //       }
  //       setV0Selection(obs, FemtoFlowV0Selection::getSelectionType(obs),
  //                      prefix);
  //     }
  //   }
  // }
  ...
  /// Selection class under investigation (FemtoFlowTrack/V0/../Selection)
  /// \param selectionType Selection type under investigation, as defined in the
  /// selection class
  template <typename T1, typename T2>
  void checkForSelection(aod::femtoflowparticle::CutContainerType& output,
  ...
  /// (FemtoFlowTrack/V0/../Selection) \return the full selection bit-wise
  /// container that will be put to the user task incorporating the user choice
  /// of selections
  template <typename T>
  aod::femtoflowparticle::CutContainerType iterateSelection(T objectSelection, bool SysChecks, float sign)
  {
    aod::femtoflowparticle::CutContainerType output = 0;
    size_t counter = 0;
  ...
  void analyseCuts(std::string choice, bool SysChecks = false, float sign = 1)
  {
    ...
    //   return;
    // }
    std::bitset<8 * sizeof(aod::femtoflowparticle::CutContainerType)>
      bitOutput = output;
    // LOGF(info, "+++++++++++++++++++++++++++++++++");
...
#endif // PWGCF_FEMTOFLOW_CORE_FEMTOFLOWCUTCULATOR_H_ */
