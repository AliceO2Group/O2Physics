...
#include "PWGCF/FemtoDream/Core/femtoDreamResoSelectionK0Short.h"
...
struct femtoDreamProducerTaskResoKshort {
  ...
  Configurable<float> ConfDaughterPTPCThr{"ConfResoPTPCThr", 0.5, "p_T (GeV/c)Threshold for case distinction between TPC/TPCTOF"};
  }Resonance; 
  ...
  void fillCollisionsAndTracksAndV0(CollisionType const& col, TrackType const& tracks, TrackTypeWithItsPid const& tracksWithItsPid, V0Type const& fullV0s)
  {
    ...
    {  
      ...
      for (auto& track1 : SlicePosdaugh){
        if (!resoCuts.DaughterSelectionPos(track1, Resonance.ConfThreshold.value)) continue;          /// loosest cuts for track1
        for (auto& track2 : SliceNegdaugh){
          if(!resoCuts.DaughterSelectionNeg(track2, Resonance.ConfThreshold.value)) continue;         /// loosest cuts for track2
          ...
          if(ResoIsNotAnti){
            ...
            if(!(tempReso.M() > Resonance.ConfResoInvMassLowLimit.value && tempReso.M() < Resonance.ConfResoInvMassUpLimit.value)) continue;            /// MassCut
            ...
            if(!(tempAntiReso.M() > Resonance.ConfResoInvMassLowLimit.value && tempAntiReso.M() < Resonance.ConfResoInvMassUpLimit.value)) continue;    /// MassCut
  ...
  PROCESS_SWITCH(femtoDreamProducerTaskResoKshort, processData,
  ...
  PROCESS_SWITCH(femtoDreamProducerTaskResoKshort, processData_noCentrality,
  ...
  PROCESS_SWITCH(femtoDreamProducerTaskResoKshort, processData_CentPbPb,
  ...
  PROCESS_SWITCH(femtoDreamProducerTaskResoKshort, processMC, "Provide MC data", false);
  ...
  PROCESS_SWITCH(femtoDreamProducerTaskResoKshort, processMC_noCentrality, "Provide MC data without requiring a centrality calibration", false);
  ...
  PROCESS_SWITCH(femtoDreamProducerTaskResoKshort, processMC_CentPbPb, "Provide MC data with centrality information for PbPb collisions", false);
  
};
WorkflowSpec defineDataProcessing(ConfigContext const& cfgc)
{
  WorkflowSpec workflow{adaptAnalysisTask<femtoDreamProducerTaskResoKshort>(cfgc)};
  return workflow;
}
