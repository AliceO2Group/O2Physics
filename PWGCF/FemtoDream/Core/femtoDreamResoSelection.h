 ...
 namespace femtoDreamResoSelection
    ...
    : public FemtoDreamObjectSelection<float, femtoDreamResoSelection::ResoSel>
  {
    ...
    public:
      ...
      {
        switch(pid){
          case (o2::track::PID::Kaon): return o2::constants::physics::MassKPlus;
          case (o2::track::PID::Pion): return o2::constants::physics::MassPiPlus;
          default:  LOG(fatal) << "PID not implemented in femtoDreamResoSelection.getMass";
                    return 0.;
        }
      }
  ...
  {
    if(track1.pt() <= mDaughPTPCThr && track2.pt() <= mDaughPTPCThr){return aod::femtodreamparticle::kPhiPosdaughTPC_NegdaughTPC;} 
    if(track1.pt() <= mDaughPTPCThr && track2.pt() > mDaughPTPCThr){return aod::femtodreamparticle::kPhiPosdaughTPC_NegdaughTOF;}
    if(track1.pt() > mDaughPTPCThr && track2.pt() <= mDaughPTPCThr){return aod::femtodreamparticle::kPhiPosdaughTOF_NegdaughTPC;}
    if(track1.pt() > mDaughPTPCThr && track2.pt() > mDaughPTPCThr){return aod::femtodreamparticle::kPhiPosdaughTOF_NegdaughTOF;} 
