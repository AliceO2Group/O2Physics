...
#ifndef FEMTOUNIVERSEV0SELECTION_H_
#define FEMTOUNIVERSEV0SELECTION_H_
...
class FemtoUniverseV0Selection : public FemtoUniverseObjectSelection<float, femtoUniverseV0Selection::V0Sel>
{
 public:
  FemtoUniverseV0Selection() : nPtV0MinSel(0),
                               nPtV0MaxSel(0),
                               nDCAV0DaughMax(0),
                               nCPAV0Min(0),
                               nTranRadV0Min(0),
  ...
  void setChildCuts(femtoUniverseV0Selection::ChildTrackType child, T1 selVal, T2 selVar, femtoUniverseSelection::SelectionType selType)
  {
  ...
  template <typename T>
  void setChildPIDSpecies(femtoUniverseV0Selection::ChildTrackType child, T& pids)
  {
    if (child == femtoUniverseV0Selection::kPosTrack) {
  ...
  static std::string getSelectionName(femtoUniverseV0Selection::V0Sel iSel, std::string_view prefix = "", std::string_view suffix = "")
  {
  ...
  static femtoUniverseSelection::SelectionType getSelectionType(femtoUniverseV0Selection::V0Sel iSel)
  {
  ...
  /// \param iSel Track selection variable to be examined
  /// \param prefix Additional prefix for the output of the configurable
  static std::string getSelectionHelper(femtoUniverseV0Selection::V0Sel iSel, std::string_view prefix = "")
  {
    std::string outString = static_cast<std::string>(prefix);
...
void FemtoUniverseV0Selection::init(HistogramRegistry* registry)
{
  if (registry) {
    ...
    if (nSelections > 8 * sizeof(cutContainerType)) {
      LOG(fatal) << "FemtoUniverseV0Cuts: Number of selections to large for your container - quitting!";
    }
    ...
    PosDaughTrack.init<aod::femtouniverseparticle::ParticleType::kV0Child, aod::femtouniverseparticle::TrackType::kPosChild, aod::femtouniverseparticle::cutContainerType>(mHistogramRegistry);
    NegDaughTrack.init<aod::femtouniverseparticle::ParticleType::kV0Child, aod::femtouniverseparticle::TrackType::kNegChild, aod::femtouniverseparticle::cutContainerType>(mHistogramRegistry);
  ...
  nPtV0MinSel = getNSelections(femtoUniverseV0Selection::kpTV0Min);
  nPtV0MaxSel = getNSelections(femtoUniverseV0Selection::kpTV0Max);
  nDCAV0DaughMax = getNSelections(femtoUniverseV0Selection::kDCAV0DaughMax);
  nCPAV0Min = getNSelections(femtoUniverseV0Selection::kCPAV0Min);
  nTranRadV0Min = getNSelections(femtoUniverseV0Selection::kTranRadV0Min);
  nTranRadV0Max = getNSelections(femtoUniverseV0Selection::kTranRadV0Max);
  nDecVtxMax = getNSelections(femtoUniverseV0Selection::kDecVtxMax);

  pTV0Min = getMinimalSelection(femtoUniverseV0Selection::kpTV0Min, femtoUniverseSelection::kLowerLimit);
  pTV0Max = getMinimalSelection(femtoUniverseV0Selection::kpTV0Max, femtoUniverseSelection::kUpperLimit);
  DCAV0DaughMax = getMinimalSelection(femtoUniverseV0Selection::kDCAV0DaughMax, femtoUniverseSelection::kUpperLimit);
  CPAV0Min = getMinimalSelection(femtoUniverseV0Selection::kCPAV0Min, femtoUniverseSelection::kLowerLimit);
  TranRadV0Min = getMinimalSelection(femtoUniverseV0Selection::kTranRadV0Min, femtoUniverseSelection::kLowerLimit);
  TranRadV0Max = getMinimalSelection(femtoUniverseV0Selection::kTranRadV0Max, femtoUniverseSelection::kUpperLimit);
  DecVtxMax = getMinimalSelection(femtoUniverseV0Selection::kDecVtxMax, femtoUniverseSelection::kAbsUpperLimit);
}

template <typename C, typename V, typename T>
bool FemtoUniverseV0Selection::isSelectedMinimal(C const& col, V const& v0, T const& posTrack, T const& negTrack)
{
...
template <typename C, typename V, typename T>
void FemtoUniverseV0Selection::fillLambdaQA(C const& col, V const& v0, T const& posTrack, T const& negTrack)
{
  const auto signPos = posTrack.sign();
  const auto signNeg = negTrack.sign();
  if (signPos < 0 || signNeg > 0) {
...
template <typename cutContainerType, typename C, typename V, typename T>
std::array<cutContainerType, 5> FemtoUniverseV0Selection::getCutContainer(C const& col, V const& v0, T const& posTrack, T const& negTrack)
{
  ...
  for (auto& sel : mSelections) {
    ...
    if (selVariable == femtoUniverseV0Selection::kDecVtxMax) {
      for (size_t i = 0; i < decVtx.size(); ++i) {
      ...
      switch (selVariable) {
        case (femtoUniverseV0Selection::kV0Sign):
          observable = sign;
          break;
        case (femtoUniverseV0Selection::kpTV0Min):
        case (femtoUniverseV0Selection::kpTV0Max):
          observable = pT;
          break;
        case (femtoUniverseV0Selection::kDCAV0DaughMax):
          observable = dcaDaughv0;
          break;
        case (femtoUniverseV0Selection::kCPAV0Min):
          observable = cpav0;
          break;
        case (femtoUniverseV0Selection::kTranRadV0Min):
        case (femtoUniverseV0Selection::kTranRadV0Max):
          observable = tranRad;
          break;
        case (femtoUniverseV0Selection::kDecVtxMax):
          break;
      }
...
template <o2::aod::femtouniverseparticle::ParticleType part, o2::aod::femtouniverseparticle::ParticleType daugh, typename C, typename V, typename T>
void FemtoUniverseV0Selection::fillQA(C const& col, V const& v0, T const& posTrack, T const& negTrack)
{
  if (mHistogramRegistry) {
    mHistogramRegistry->fill(HIST(o2::aod::femtouniverseparticle::ParticleTypeName[part]) + HIST("/hPt"), v0.pt());
    mHistogramRegistry->fill(HIST(o2::aod::femtouniverseparticle::ParticleTypeName[part]) + HIST("/hEta"), v0.eta());
    mHistogramRegistry->fill(HIST(o2::aod::femtouniverseparticle::ParticleTypeName[part]) + HIST("/hPhi"), v0.phi());
    mHistogramRegistry->fill(HIST(o2::aod::femtouniverseparticle::ParticleTypeName[part]) + HIST("/hDaughDCA"), v0.dcaV0daughters());
    ...
    mHistogramRegistry->fill(HIST(o2::aod::femtouniverseparticle::ParticleTypeName[part]) + HIST("/hInvMassLambda"), v0.mLambda());
    mHistogramRegistry->fill(HIST(o2::aod::femtouniverseparticle::ParticleTypeName[part]) + HIST("/hInvMassAntiLambda"), v0.mAntiLambda());
    mHistogramRegistry->fill(HIST(o2::aod::femtouniverseparticle::ParticleTypeName[part]) + HIST("/hInvMassLambdaAntiLambda"), v0.mLambda(), v0.mAntiLambda());
  }

  PosDaughTrack.fillQA<aod::femtouniverseparticle::ParticleType::kV0Child, aod::femtouniverseparticle::TrackType::kPosChild>(posTrack);
  NegDaughTrack.fillQA<aod::femtouniverseparticle::ParticleType::kV0Child, aod::femtouniverseparticle::TrackType::kNegChild>(negTrack);
}

} // namespace o2::analysis::femtoUniverse

#endif /* FEMTOUNIVERSEV0SELECTION_H_ */
