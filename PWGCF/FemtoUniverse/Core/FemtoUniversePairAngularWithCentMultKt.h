 ...
 public:
  ...
  void init(HistogramRegistry* registry, t1& kstarbins, t1& centmultbins, t2& phiBins, t2& etaBins, bool processKT)
  {
    ...
    for (int i = 0; i < static_cast<int>(CentMultBins.size() - 1); i++) {
      ...
      std::string HistName = HistFolderMult + "/DeltaEtaDeltaPhi";
      PairWithCentMultKtRegistry->add(HistName.c_str(), HistTitle.c_str(), HistType::kTH2F, {phiAxis, etaAxis});
      if (UseKt) {
        for (int i = 0; i < static_cast<int>(KtBins.size() - 1); i++) {
          ...
          std::string HistNameKt = HistFolderMult + "/DeltaEtaDeltaPhi" + HistSuffix1Kt + "_" + HistSuffix2Kt;
          PairWithCentMultKtRegistry->add(HistNameKt.c_str(), HistTitleKt.c_str(), HistType::kTH2F, {phiAxis, etaAxis});
        }
      }
    }
