      ...
      float, femto_universe_track_selection::TrackSel>
{
 public:
  ...
  template <o2::aod::femtouniverseparticle::ParticleType part,
            o2::aod::femtouniverseparticle::TrackType tracktype,
  ...
  void setPIDSpecies(T& pids)
  {
    std::vector<int> tmpPids =
      pids; /// necessary due to some features of the configurable
  ...
  /// Checks whether the most open combination of all selection criteria is
  /// fulfilled
  /// \tparam T Data type of the track
  /// \param track Track
  /// \return Whether the most open combination of all selection criteria is
  /// fulfilled
  template <typename T>
  ...
  /// \param track Track
  template <o2::aod::femtouniverseparticle::ParticleType part,
            o2::aod::femtouniverseparticle::TrackType tracktype, typename T>
  void fillQA(T const& track);

  /// Helper function to obtain the name of a given selection criterion for
  /// consistent naming of the configurables
  ...
  static std::string
    getSelectionName(femto_universe_track_selection::TrackSel iSel,
  ...
  static femto_universe_selection::SelectionType
    getSelectionType(femto_universe_track_selection::TrackSel iSel)
  {
  ...
  /// \param prefix Additional prefix for the output of the configurable
  static std::string
    getSelectionHelper(femto_universe_track_selection::TrackSel iSel,
                       std::string_view prefix = "")
  {
    std::string outString = static_cast<std::string>(prefix);
  ...
  float getSigmaPIDMax() { return nSigmaPIDMax; }
 ...
 private:
  ...
  std::vector<o2::track::PID>
    kPIDspecies; ///< All the particle species for which the n_sigma values
                 ///< need to be stored
  static constexpr int kNtrackSelection = 15;
  static constexpr std::string_view kSelectionNames[kNtrackSelection] = {
    "Sign", "PtMin", "PtMax", "EtaMax",
    "TPCnClsMin", "TPCfClsMin", "TPCcRowsMin", "TPCsClsMax",
    "TPCfracsClsMax", "ITSnClsMin", "ITSnClsIbMin", "DCAxyMax",
    "DCAzMax", "DCAMin", "PIDnSigmaMax"}; ///< Name of the
                                          ///< different selections

  static constexpr femto_universe_selection::SelectionType
    kSelectionTypes[kNtrackSelection]{
      ...
      femto_universe_selection::kAbsUpperLimit,
      femto_universe_selection::kAbsUpperLimit,
      femto_universe_selection::kAbsUpperLimit}; ///< Map to match a
                                                 ///< variable with its type

  static constexpr std::string_view kSelectionHelper[kNtrackSelection] = {
    "Sign of the track",
    "Minimal pT (GeV/c)",
    "Maximal pT (GeV/c)",
    "Maximal eta",
    ...
    "Maximal DCA_z (cm)",
    "Minimal DCA (cm)",
    "Maximal PID (nSigma)"}; ///< Helper information for the different
                             ///< selections
...
void FemtoUniverseTrackSelection::init(HistogramRegistry* registry)
{
  if (registry) {
    ...
    std::string folderName =
      ...
      "/" +
      static_cast<std::string>(
        o2::aod::femtouniverseparticle::TrackTypeName[tracktype]);

    /// check whether the number of selection exceeds the bitmap size
    unsigned int nSelections =
      getNSelections() -
      getNSelections(femto_universe_track_selection::kPIDnSigmaMax);
    if (nSelections > 8 * sizeof(CutContainerType)) {
      LOG(fatal) << "FemtoUniverseTrackCuts: Number of selections too large "
                    "for your container - quitting!";
    }
    ...
    mHistogramRegistry->add((folderName + "/hPhi").c_str(), "; #phi; Entries",
                            kTH1F, {{200, 0, o2::constants::math::TwoPI}});
    mHistogramRegistry->add((folderName + "/hTPCfindable").c_str(),
                            "; TPC findable clusters; Entries", kTH1F,
                            {{163, -0.5, 162.5}});
    mHistogramRegistry->add((folderName + "/hTPCfound").c_str(),
                            "; TPC found clusters; Entries", kTH1F,
                            {{163, -0.5, 162.5}});
    mHistogramRegistry->add((folderName + "/hTPCcrossedOverFindalbe").c_str(),
                            "; TPC ratio findable; Entries", kTH1F,
                            {{100, 0.5, 1.5}});
    mHistogramRegistry->add((folderName + "/hTPCcrossedRows").c_str(),
                            "; TPC crossed rows; Entries", kTH1F,
                            {{163, 0, 163}});
    mHistogramRegistry->add((folderName + "/hTPCfindableVsCrossed").c_str(),
                            ";TPC findable clusters ; TPC crossed rows;", kTH2F,
                            {{163, 0, 163}, {163, 0, 163}});
    mHistogramRegistry->add((folderName + "/hTPCshared").c_str(),
                            "; TPC shared clusters; Entries", kTH1F,
                            {{163, -0.5, 162.5}});
    mHistogramRegistry->add((folderName + "/hTPCfractionSharedCls").c_str(),
                            "; TPC fraction of shared clusters; Entries", kTH1F,
                            {{100, 0.0, 100.0}});
    mHistogramRegistry->add((folderName + "/hITSclusters").c_str(),
                            "; ITS clusters; Entries", kTH1F,
                            {{10, -0.5, 9.5}});
    mHistogramRegistry->add((folderName + "/hITSclustersIB").c_str(),
                            "; ITS clusters in IB; Entries", kTH1F,
                            {{10, -0.5, 9.5}});
    mHistogramRegistry->add((folderName + "/hDCAxy").c_str(),
                            "; #it{p}_{T} (GeV/#it{c}); DCA_{xy} (cm)", kTH2F,
                            {{100, 0, 10}, {500, -5, 5}});
    mHistogramRegistry->add((folderName + "/hDCAz").c_str(),
                            "; #it{p}_{T} (GeV/#it{c}); DCA_{z} (cm)", kTH2F,
                            {{100, 0, 10}, {500, -5, 5}});
    mHistogramRegistry->add((folderName + "/hDCA").c_str(),
                            "; #it{p}_{T} (GeV/#it{c}); DCA (cm)", kTH2F,
                            {{100, 0, 10}, {301, 0., 1.5}});
    mHistogramRegistry->add((folderName + "/hTPCdEdX").c_str(),
                            "; #it{p} (GeV/#it{c}); TPC Signal", kTH2F,
                            {{100, 0, 10}, {1000, 0, 1000}});
    mHistogramRegistry->add((folderName + "/nSigmaITS_el").c_str(),
                            "; #it{p} (GeV/#it{c}); n#sigma_{ITS}^{e}", kTH2F,
                            {{100, 0, 10}, {200, -4.975, 5.025}});
    mHistogramRegistry->add((folderName + "/nSigmaITS_pi").c_str(),
                            "; #it{p} (GeV/#it{c}); n#sigma_{ITS}^{#pi}", kTH2F,
                            {{100, 0, 10}, {200, -4.975, 5.025}});
    mHistogramRegistry->add((folderName + "/nSigmaITS_K").c_str(),
                            "; #it{p} (GeV/#it{c}); n#sigma_{ITS}^{K}", kTH2F,
                            {{100, 0, 10}, {200, -4.975, 5.025}});
    mHistogramRegistry->add((folderName + "/nSigmaITS_p").c_str(),
                            "; #it{p} (GeV/#it{c}); n#sigma_{ITS}^{p}", kTH2F,
                            {{100, 0, 10}, {200, -4.975, 5.025}});
    mHistogramRegistry->add((folderName + "/nSigmaITS_d").c_str(),
                            "; #it{p} (GeV/#it{c}); n#sigma_{ITS}^{d}", kTH2F,
                            {{100, 0, 10}, {200, -4.975, 5.025}});
    mHistogramRegistry->add((folderName + "/nSigmaTPC_el").c_str(),
                            "; #it{p} (GeV/#it{c}); n#sigma_{TPC}^{e}", kTH2F,
                            {{100, 0, 10}, {200, -4.975, 5.025}});
    mHistogramRegistry->add((folderName + "/nSigmaTPC_pi").c_str(),
                            "; #it{p} (GeV/#it{c}); n#sigma_{TPC}^{#pi}", kTH2F,
                            {{100, 0, 10}, {200, -4.975, 5.025}});
    mHistogramRegistry->add((folderName + "/nSigmaTPC_K").c_str(),
                            "; #it{p} (GeV/#it{c}); n#sigma_{TPC}^{K}", kTH2F,
                            {{100, 0, 10}, {200, -4.975, 5.025}});
    mHistogramRegistry->add((folderName + "/nSigmaTPC_p").c_str(),
                            "; #it{p} (GeV/#it{c}); n#sigma_{TPC}^{p}", kTH2F,
                            {{100, 0, 10}, {200, -4.975, 5.025}});
    mHistogramRegistry->add((folderName + "/nSigmaTPC_d").c_str(),
                            "; #it{p} (GeV/#it{c}); n#sigma_{TPC}^{d}", kTH2F,
                            {{100, 0, 10}, {200, -4.975, 5.025}});
    mHistogramRegistry->add((folderName + "/nSigmaTOF_el").c_str(),
                            "; #it{p} (GeV/#it{c}); n#sigma_{TOF}^{e}", kTH2F,
                            {{100, 0, 10}, {200, -4.975, 5.025}});
    mHistogramRegistry->add((folderName + "/nSigmaTOF_pi").c_str(),
                            "; #it{p} (GeV/#it{c}); n#sigma_{TOF}^{#pi}", kTH2F,
                            {{100, 0, 10}, {200, -4.975, 5.025}});
    mHistogramRegistry->add((folderName + "/nSigmaTOF_K").c_str(),
                            "; #it{p} (GeV/#it{c}); n#sigma_{TOF}^{K}", kTH2F,
                            {{100, 0, 10}, {200, -4.975, 5.025}});
    mHistogramRegistry->add((folderName + "/nSigmaTOF_p").c_str(),
                            "; #it{p} (GeV/#it{c}); n#sigma_{TOF}^{p}", kTH2F,
                            {{100, 0, 10}, {200, -4.975, 5.025}});
    mHistogramRegistry->add((folderName + "/nSigmaTOF_d").c_str(),
                            "; #it{p} (GeV/#it{c}); n#sigma_{TOF}^{d}", kTH2F,
                            {{100, 0, 10}, {200, -4.975, 5.025}});
    mHistogramRegistry->add((folderName + "/nSigmaComb_el").c_str(),
                            "; #it{p} (GeV/#it{c}); n#sigma_{comb}^{e}", kTH2F,
                            {{100, 0, 10}, {200, -4.975, 5.025}});
    mHistogramRegistry->add((folderName + "/nSigmaComb_pi").c_str(),
                            "; #it{p} (GeV/#it{c}); n#sigma_{comb}^{#pi}",
                            kTH2F, {{100, 0, 10}, {200, -4.975, 5.025}});
    mHistogramRegistry->add((folderName + "/nSigmaComb_K").c_str(),
                            "; #it{p} (GeV/#it{c}); n#sigma_{comb}^{K}", kTH2F,
                            {{100, 0, 10}, {200, -4.975, 5.0255}});
    mHistogramRegistry->add((folderName + "/nSigmaComb_p").c_str(),
                            "; #it{p} (GeV/#it{c}); n#sigma_{comb}^{p}", kTH2F,
                            {{100, 0, 10}, {200, -4.975, 5.025}});
    mHistogramRegistry->add((folderName + "/nSigmaComb_d").c_str(),
  ...
  nTPCsFracMaxSel =
    getNSelections(femto_universe_track_selection::kTPCfracsClsMax);
  ...
  pTMin = getMinimalSelection(femto_universe_track_selection::kpTMin,
                              femto_universe_selection::kLowerLimit);
  pTMax = getMinimalSelection(femto_universe_track_selection::kpTMax,
                              femto_universe_selection::kUpperLimit);
  etaMax = getMinimalSelection(femto_universe_track_selection::kEtaMax,
                               femto_universe_selection::kAbsUpperLimit);
  nClsMin = getMinimalSelection(femto_universe_track_selection::kTPCnClsMin,
                                femto_universe_selection::kLowerLimit);
  fClsMin = getMinimalSelection(femto_universe_track_selection::kTPCfClsMin,
                                femto_universe_selection::kLowerLimit);
  cTPCMin = getMinimalSelection(femto_universe_track_selection::kTPCcRowsMin,
                                femto_universe_selection::kLowerLimit);
  sTPCMax = getMinimalSelection(femto_universe_track_selection::kTPCsClsMax,
                                femto_universe_selection::kUpperLimit);
  fracsTPCMax =
    getMinimalSelection(femto_universe_track_selection::kTPCfracsClsMax,
                        femto_universe_selection::kUpperLimit);
  nITSclsMin = getMinimalSelection(femto_universe_track_selection::kITSnClsMin,
                                   femto_universe_selection::kLowerLimit);
  nITSclsIbMin =
    getMinimalSelection(femto_universe_track_selection::kITSnClsIbMin,
                        femto_universe_selection::kLowerLimit);
  dcaXYMax = getMinimalSelection(femto_universe_track_selection::kDCAxyMax,
                                 femto_universe_selection::kAbsUpperLimit);
  dcaZMax = getMinimalSelection(femto_universe_track_selection::kDCAzMax,
                                femto_universe_selection::kAbsUpperLimit);
  dcaMin = getMinimalSelection(femto_universe_track_selection::kDCAMin,
                               femto_universe_selection::kAbsLowerLimit);
  nSigmaPIDMax =
    getMinimalSelection(femto_universe_track_selection::kPIDnSigmaMax,
                        femto_universe_selection::kAbsUpperLimit);
}

template <typename T>
auto FemtoUniverseTrackSelection::getNsigmaTPC(T const& track,
                                               o2::track::PID pid)
{
...
template <typename T>
auto FemtoUniverseTrackSelection::getNsigmaTOF(T const& track,
                                               o2::track::PID pid)
{
...
template <typename T>
auto FemtoUniverseTrackSelection::getNsigmaITS(T const& track,
                                               o2::track::PID pid)
{
...
bool FemtoUniverseTrackSelection::isSelectedMinimal(T const& track)
{
  ...
  const auto dca =
    track.dcaXY(); // Accordingly to FemtoUniverse in AliPhysics  as well as
                   // LF analysis, only dcaXY should be checked; NOT
                   // std::sqrt(pow(dcaXY, 2.) + pow(dcaZ, 2.))
...
std::array<CutContainerType, 2>
  FemtoUniverseTrackSelection::getCutContainer(T const& track)
{
  ...
  if (mHistogramRegistry) {
    mHistogramRegistry->fill(
      HIST(o2::aod::femtouniverseparticle::ParticleTypeName[part]) +
        HIST("/") +
        HIST(o2::aod::femtouniverseparticle::TrackTypeName[tracktype]) +
        HIST("/hPt"),
      track.pt());
    mHistogramRegistry->fill(
      HIST(o2::aod::femtouniverseparticle::ParticleTypeName[part]) +
    ...
    mHistogramRegistry->fill(
      ...
      track.tpcNClsFindable(), track.tpcNClsCrossedRows());
    mHistogramRegistry->fill(
      HIST(o2::aod::femtouniverseparticle::ParticleTypeName[part]) +
        HIST("/") +
        HIST(o2::aod::femtouniverseparticle::TrackTypeName[tracktype]) +
    ...
    mHistogramRegistry->fill(
      HIST(o2::aod::femtouniverseparticle::ParticleTypeName[part]) +
        ...
        HIST("/hDCA"),
      track.pt(),
      std::sqrt(std::pow(track.dcaXY(), 2.) + std::pow(track.dcaZ(), 2.)));
    mHistogramRegistry->fill(
      HIST(o2::aod::femtouniverseparticle::ParticleTypeName[part]) +
        HIST("/") +
        HIST(o2::aod::femtouniverseparticle::TrackTypeName[tracktype]) +
        HIST("/hTPCdEdX"),
      track.p(), track.tpcSignal());
    mHistogramRegistry->fill(
      HIST(o2::aod::femtouniverseparticle::ParticleTypeName[part]) +
        HIST("/") +
        HIST(o2::aod::femtouniverseparticle::TrackTypeName[tracktype]) +
        HIST("/nSigmaTPC_el"),
      track.p(), track.tpcNSigmaEl());
    mHistogramRegistry->fill(
      HIST(o2::aod::femtouniverseparticle::ParticleTypeName[part]) +
        HIST("/") +
        HIST(o2::aod::femtouniverseparticle::TrackTypeName[tracktype]) +
        HIST("/nSigmaTPC_pi"),
      track.p(), track.tpcNSigmaPi());
    mHistogramRegistry->fill(
      HIST(o2::aod::femtouniverseparticle::ParticleTypeName[part]) +
        HIST("/") +
        HIST(o2::aod::femtouniverseparticle::TrackTypeName[tracktype]) +
        HIST("/nSigmaTPC_K"),
      track.p(), track.tpcNSigmaKa());
    mHistogramRegistry->fill(
      HIST(o2::aod::femtouniverseparticle::ParticleTypeName[part]) +
        HIST("/") +
        HIST(o2::aod::femtouniverseparticle::TrackTypeName[tracktype]) +
        HIST("/nSigmaTPC_p"),
      track.p(), track.tpcNSigmaPr());
    mHistogramRegistry->fill(
      HIST(o2::aod::femtouniverseparticle::ParticleTypeName[part]) +
        HIST("/") +
        HIST(o2::aod::femtouniverseparticle::TrackTypeName[tracktype]) +
        HIST("/nSigmaTPC_d"),
      track.p(), track.tpcNSigmaDe());
    mHistogramRegistry->fill(
      HIST(o2::aod::femtouniverseparticle::ParticleTypeName[part]) +
        HIST("/") +
        HIST(o2::aod::femtouniverseparticle::TrackTypeName[tracktype]) +
        HIST("/nSigmaTOF_el"),
      track.p(), track.tofNSigmaEl());
    mHistogramRegistry->fill(
      HIST(o2::aod::femtouniverseparticle::ParticleTypeName[part]) +
        HIST("/") +
        HIST(o2::aod::femtouniverseparticle::TrackTypeName[tracktype]) +
        HIST("/nSigmaTOF_pi"),
      track.p(), track.tofNSigmaPi());
    mHistogramRegistry->fill(
      HIST(o2::aod::femtouniverseparticle::ParticleTypeName[part]) +
        HIST("/") +
        HIST(o2::aod::femtouniverseparticle::TrackTypeName[tracktype]) +
        HIST("/nSigmaTOF_K"),
      track.p(), track.tofNSigmaKa());
    mHistogramRegistry->fill(
      HIST(o2::aod::femtouniverseparticle::ParticleTypeName[part]) +
        HIST("/") +
        HIST(o2::aod::femtouniverseparticle::TrackTypeName[tracktype]) +
        HIST("/nSigmaTOF_p"),
      track.p(), track.tofNSigmaPr());
    mHistogramRegistry->fill(
      HIST(o2::aod::femtouniverseparticle::ParticleTypeName[part]) +
        HIST("/") +
        HIST(o2::aod::femtouniverseparticle::TrackTypeName[tracktype]) +
        HIST("/nSigmaTOF_d"),
      track.p(), track.tofNSigmaDe());
    mHistogramRegistry->fill(
      HIST(o2::aod::femtouniverseparticle::ParticleTypeName[part]) +
        HIST("/") +
        HIST(o2::aod::femtouniverseparticle::TrackTypeName[tracktype]) +
        HIST("/nSigmaComb_el"),
      track.p(),
      std::sqrt(track.tpcNSigmaEl() * track.tpcNSigmaEl() +
                track.tofNSigmaEl() * track.tofNSigmaEl()));
    mHistogramRegistry->fill(
      HIST(o2::aod::femtouniverseparticle::ParticleTypeName[part]) +
        HIST("/") +
        HIST(o2::aod::femtouniverseparticle::TrackTypeName[tracktype]) +
        HIST("/nSigmaComb_pi"),
      track.p(),
      std::sqrt(track.tpcNSigmaPi() * track.tpcNSigmaPi() +
                track.tofNSigmaPi() * track.tofNSigmaPi()));
    mHistogramRegistry->fill(
      HIST(o2::aod::femtouniverseparticle::ParticleTypeName[part]) +
        HIST("/") +
        HIST(o2::aod::femtouniverseparticle::TrackTypeName[tracktype]) +
        HIST("/nSigmaComb_K"),
      track.p(),
      std::sqrt(track.tpcNSigmaKa() * track.tpcNSigmaKa() +
                track.tofNSigmaKa() * track.tofNSigmaKa()));
    mHistogramRegistry->fill(
      HIST(o2::aod::femtouniverseparticle::ParticleTypeName[part]) +
        HIST("/") +
        HIST(o2::aod::femtouniverseparticle::TrackTypeName[tracktype]) +
        HIST("/nSigmaComb_p"),
      track.p(),
      std::sqrt(track.tpcNSigmaPr() * track.tpcNSigmaPr() +
                track.tofNSigmaPr() * track.tofNSigmaPr()));
    mHistogramRegistry->fill(
      HIST(o2::aod::femtouniverseparticle::ParticleTypeName[part]) +
        HIST("/") +
        HIST(o2::aod::femtouniverseparticle::TrackTypeName[tracktype]) +
        HIST("/nSigmaComb_d"),
      track.p(),
      std::sqrt(track.tpcNSigmaDe() * track.tpcNSigmaDe() +
                track.tofNSigmaDe() * track.tofNSigmaDe()));
  }
}
