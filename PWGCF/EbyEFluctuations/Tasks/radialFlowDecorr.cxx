            ...
            for (const auto& mcCollision : mcColl) {
              ...
              for (const auto& col : colSlice) {
                ...
                if (isfinite(sumWiTruth[0][0])) {
                  ...
                  if (!isfinite(meanPtTruth)) LOGF(info, "meanPtTruth = %.3f, num = %.3f, den =%.3f", meanPtTruth, sumWiptiTruth[0][0], sumWiTruth[0][0]);
                  if (!isfinite(meanPtTruth)) continue;
                ...
                if (isfinite(sumWiReco[0][0])) {
                  ...
                  if (!isfinite(meanPtReco)) LOGF(info, "meanPtReco = %.3f, num = %.3f, den =%.3f", meanPtReco, sumWiptiReco[0][0], sumWiReco[0][0]);
                  if (!isfinite(meanPtReco)) continue;
                ...
                if (isfinite(sumWiRecoEffCorr[0][0])) {
                  ...
                  if (!isfinite(meanpTeffcorr)) LOGF(info, "meanPtRecoEffcorr = %.3f, num = %.3f, den =%.3f", meanpTeffcorr, sumWiptiRecoEffCorr[0][0], sumWiRecoEffCorr[0][0]);
                  if (!isfinite(meanpTeffcorr)) continue;
              ...
              for (const auto& col : colSlice) {
                ...
                for (int ieta = 0; ieta < KNEta; ++ieta) {
                  for (int ipt = 0; ipt < KNpT; ++ipt) {
                    ...
                    if (mmptTru != 0.0f) p1kBarTru[ieta][ipt] = meanTru[ieta][ipt] - mmptTru;
                    if (mmptReco != 0.0f) p1kBarReco[ieta][ipt] = meanReco[ieta][ipt] - mmptReco;
                    if (mmptRecoEffCor != 0.0f) p1kBarRecoEffCor[ieta][ipt] = meanRecoEffCor[ieta][ipt] - mmptRecoEffCor;

                    if (mmetTru != 0.0f) p1kBarTruEt[ieta][ipt] = meanTruEt[ieta][ipt] - mmetTru;
                    if (mmetReco != 0.0f) p1kBarRecoEt[ieta][ipt] = meanRecoEt[ieta][ipt] - mmetReco;
                    if (mmetRecoEffCor != 0.0f) p1kBarRecoEffCorEt[ieta][ipt] = meanRecoEffCorEt[ieta][ipt] - mmetRecoEffCor;
                  }
                }
