...
struct RadialFlowDecorr {
  ...
  enum PIDIdx {
    kInclusiveIdx = 0,
    kPiMinusIdx, kPiPlusIdx, kPiAllIdx,
    kKaMinusIdx, kKaPlusIdx, kKaAllIdx,
    kAntiPrIdx, kPrIdx, kAllPrIdx,
    KNsp
  };
  ...
  const std::vector<float> etaLw = {
    ...
    const std::vector<float> etaUp = {
      ...
      const AxisSpec gapAxis{{-1.55, -1.45, -1.35, -1.25, -1.15, -1.05, -0.95, -0.85,
        ...
        0.85,  0.95,  1.05,  1.15,  1.25,  1.35,  1.45,  1.55}, "Gaps"};
          ...
          void processGetEffHists(aod::McCollisions const& mcColl, MyRun3MCCollisions const& collisions, TCs const& tracks, FilteredTCs const& /*filteredTracks*/, aod::McParticles const& mcParticles)
          {
            for (const auto& mcCollision : mcColl) {
              ...
              for (const auto& col : colSlice) {
                ...
                if (trackSlice.size() < 1 || partSlice.size() < 1) continue;

                float cent = getCentrality(col);
                if (cent > KCentMax) continue;
                float multPV = col.multNTracksPV();
                float vz = col.posZ();

                for (const auto& particle : partSlice) {
                  if (!isParticleSelected(particle) || !particle.isPhysicalPrimary()) continue;
                  ...
                  if (isSpecies[kPiMinusIdx]) histos.fill(HIST("h3_AllPrimary_PiMinus"), multPV, pt, eta);
                  else if (isSpecies[kPiPlusIdx]) histos.fill(HIST("h3_AllPrimary_PiPlus"), multPV, pt, eta);
                  if (isSpecies[kPiAllIdx]) histos.fill(HIST("h3_AllPrimary_PiAll"), multPV, pt, eta);

                  if (isSpecies[kKaMinusIdx]) histos.fill(HIST("h3_AllPrimary_KaMinus"), multPV, pt, eta);
                  else if (isSpecies[kKaPlusIdx]) histos.fill(HIST("h3_AllPrimary_KaPlus"), multPV, pt, eta);
                  if (isSpecies[kKaAllIdx]) histos.fill(HIST("h3_AllPrimary_KaAll"), multPV, pt, eta);

                  if (isSpecies[kAntiPrIdx]) histos.fill(HIST("h3_AllPrimary_AntiPr"), multPV, pt, eta);
                  else if (isSpecies[kPrIdx]) histos.fill(HIST("h3_AllPrimary_Pr"), multPV, pt, eta);
                  if (isSpecies[kAllPrIdx]) histos.fill(HIST("h3_AllPrimary_AllPr"), multPV, pt, eta);
                }
