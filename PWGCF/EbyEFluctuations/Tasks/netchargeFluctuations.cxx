...
template <RunType run, typename C>
  bool selCollision(C const& coll, float& cent, float& mult)
  {
   ...
   if (run == kRun3) {
    ...
    if (cfgUseGoodItsLayerAllCut && !(collision.selection_bit(o2::aod::evsel::kIsGoodITSLayersAll))) return false;
...
template <RunType run, typename C, typename T>
  void calculationData(C const& coll, T const& tracks)
  {
   ...
   histogramRegistry.fill(HIST("data/hVtxZ_before"), coll.posZ());
    ...
    int fpos = 0, fneg = 0, posneg = 0, termn = 0, termp = 0; int nch =0, nchCor = 0;
...
template <RunType run, typename C, typename T, typename M, typename P>
  ...
  double weight = 1.0 / eff;
   ...
   if (!mcpart.isPhysicalPrimary())   continue;
      ...
      if (pd != nullptr) {   sign = pd->Charge() / 3.;   }     
      if (sign == 0)   continue;     
      if (std::abs(pid) != kElectron && std::abs(pid) != kMuonMinus && std::abs(pid) != kPiPlus && std::abs(pid) != kKPlus && std::abs(pid) != kProton)    continue;
...
void calculationDeltaEta(C const& coll, T const& tracks, float deta1, float deta2)
{
  ...
  for (const auto& track : tracks) {
    ...
    if (track.sign() == 1) fpos++;
    else if (track.sign() == -1) fneg++;
  }
  termp = fpos * (fpos - 1);
  termn = fneg * (fneg - 1);
  posneg = fpos * fneg;
