...
struct KaonIsospinFluctuations {
  ...
  void init(InitContext const&)
  {
    ...
    const AxisSpec axisPCoarse = {50, 0.0f, 10.0f, "#it{p} (GeV/#it{c})"};
    const AxisSpec axisPtCoarse = {50, 0.0f, 10.0f, "#it{p}_{T} (GeV/#it{c})"};
    const AxisSpec axisTPCInnerParamCoarse = {50, 0.0f, 10.0f, "#it{p}_{tpcInnerParam} (GeV/#it{c})"};
    const AxisSpec axisTOFExpMomCoarse = {50, 0.0f, 10.0f, "#it{p}_{tofExpMom} (GeV/#it{c})"};
    const AxisSpec axisTPCSignalCoarse = {50, -1, 1000, "tpcSignal"};
    const AxisSpec axisTOFBetaCoarse = {40, -2.0, 2.0, "tofBeta"};
    const AxisSpec axisTPCNSigmaCoarse = {50, -10.0, 10.0, "n#sigma_{TPC}"};
    const AxisSpec axisTOFNSigmaCoarse = {50, -10.0, 10.0, "n#sigma_{TOF}"};
    const AxisSpec axisDcaXYCoarse = {50, -5, 5, "dcaXY"};
    const AxisSpec axisDcaZCoarse = {50, -5, 5, "dcaZ"};
    ...
    auto addDaughterQASparse = [axisP, axisPt, axisEta, axisPhi, axisRapidity,
                                axisIsPVContributor, axisIsGlobalTrack, axisSign,
                                axisDcaXY, axisDcaZ, axisDcaXYwide, axisDcaZwide,
                                axisPCoarse, axisPtCoarse, axisTPCInnerParamCoarse, axisTOFExpMomCoarse,
                                axisTPCSignalCoarse, axisTOFBetaCoarse, axisTPCNSigmaCoarse, axisTOFNSigmaCoarse,
                                axisDcaXYCoarse, axisDcaZCoarse](auto& histReg, const std::string& basePath) {
      // fixed-size, full-resolution 1D/2D monitoring (cheap, bounded memory regardless of stats)
      histReg.add((basePath + "h01_p").c_str(), "p", kTH1F, {axisP});
      histReg.add((basePath + "h02_pt").c_str(), "pt", kTH1F, {axisPt});
      histReg.add((basePath + "h05_eta").c_str(), "eta", kTH1F, {axisEta});
      histReg.add((basePath + "h06_phi").c_str(), "phi", kTH1F, {axisPhi});
      ...
      // Sparse #1: momentum/PID correlation group - replaces h21-h29, h31-h39 (18 hists)
      // Axes are strongly correlated with each other (p ~ tpcInnerParam ~ tofExpMom per track),
      // which keeps the number of unique filled bins well below "one per track".
      histReg.add((basePath + "hSparseDauKinePID").c_str(), "p_pt_tpcInnerParam_tofExpMom_tpcSignal_beta_tpcNSigma_tofNSigma", kTHnSparseF,
                  {axisPCoarse, axisPtCoarse, axisTPCInnerParamCoarse, axisTOFExpMomCoarse,
                   axisTPCSignalCoarse, axisTOFBetaCoarse, axisTPCNSigmaCoarse, axisTOFNSigmaCoarse});

      // Sparse #2: DCA correlation group - replaces h41-h48 (8 hists)
      histReg.add((basePath + "hSparseDauDCA").c_str(), "p_pt_dcaXY_dcaZ", kTHnSparseF,
                  {axisPCoarse, axisPtCoarse, axisDcaXYCoarse, axisDcaZCoarse});
    };

    auto addAllV0Histos = [addV0Histos, addDaughterQASparse](auto& histReg, const std::string& basePath, const std::string& posPidPath, const std::string& negPidPath, const bool fillMotherQA, const bool fillDauTrackQA) {
      if (fillMotherQA) {
    ...
    auto addAllPrimVtxParticleHistos = [addPrimVtxParticleQAHistos, addDaughterQASparse](auto& histReg, const std::string& basePath, const std::string& posPidPath, const std::string& negPidPath, const bool fillMotherQA, const bool fillDauTrackQA) {
      if (fillMotherQA) {
    ...
    for (int iPidType = kPrimTrkPhi1020; iPidType <= kPrimTrkRho770; iPidType++) {
      ...
      if (getCfg<bool>(cfgPrimVtxParticleCuts, iPidType, kPrimDoRecoCheck)) {
        if (getCfg<bool>(cfgPrimVtxParticleCuts, iPidType, kPrimFillPreSel)) {
          ...
          addAllPrimVtxParticleHistos(*primVtxRegs[iPidType], basePath + "PreSel/", posPidPath, negPidPath, fillMotherQA, fillDauTrackQA);
          addCountHist(*primVtxRegs[iPidType], commonCountName, commonCountTitle);
          if (iPidType == kPrimTrkPhi1020) {
            addCountLabels(primVtxRegs[iPidType]->get<TH1>(HIST("recoPhi1020/PreSel/nCommonKaOfDifferentPhi")));
          }
          if (iPidType == kPrimTrkJPsiToEE) {
            addCountLabels(primVtxRegs[iPidType]->get<TH1>(HIST("recoJPsiToEE/PreSel/nCommonKaOfDifferentJPsiToEE")));
          }
          if (iPidType == kPrimTrkJPsiToMuMu) {
            addCountLabels(primVtxRegs[iPidType]->get<TH1>(HIST("recoJPsiToMuMu/PreSel/nCommonKaOfDifferentJPsiToMuMu")));
          }
          if (iPidType == kPrimTrkKStar892) {
            addCountLabels(primVtxRegs[iPidType]->get<TH1>(HIST("recoKStar892/PreSel/nCommonKaOfDifferentKStar892")));
          }
          if (iPidType == kPrimTrkKStar892Bar) {
            addCountLabels(primVtxRegs[iPidType]->get<TH1>(HIST("recoKStar892Bar/PreSel/nCommonKaOfDifferentKStar892Bar")));
          }
          if (iPidType == kPrimTrkRho770) {
            addCountLabels(primVtxRegs[iPidType]->get<TH1>(HIST("recoRho770/PreSel/nCommonKaOfDifferentRho770")));
          }
        }
        if (getCfg<bool>(cfgPrimVtxParticleCuts, iPidType, kPrimFillPostSel)) {
          ...
          addAllPrimVtxParticleHistos(*primVtxRegs[iPidType], basePath + "PostSel/", posPidPath, negPidPath, fillMotherQA, fillDauTrackQA);
        }
      }
    }
    ...
    if (getCfg<bool>(cfgPrimVtxParticleCuts, kV0TrkLambda, kV0DoRecoCheck)) {
      fillMotherQA = getCfg<bool>(cfgPrimVtxParticleCuts, kV0TrkK0s, kV0FillPostSel);
      fillDauTrackQA = getCfg<bool>(cfgPrimVtxParticleCuts, kV0TrkK0s, kV0FillPostSelDau);
      addAllV0Histos(recoV0sPostLambdaCheck, "recoLambda/PostSel/", "Pr/", "Pi/", fillMotherQA, fillDauTrackQA);
      recoV0sPostLambdaCheck.add("recoLambda/PostSel/mLambda_vs_cent", "mLambda_vs_cent", kTH2F, {axisCent, axisLambdaMass});
      recoV0sPostLambdaCheck.add("recoLambda/PostSel/mLambda_vs_cent_effcorr", "mLambda_vs_cent_effcorr", kTH2F, {axisCent, axisLambdaMass});
      recoV0sPostLambdaCheck.add("recoLambda/PostSel/mLambda_vs_cent_vs_Pt", "mLambda_vs_cent_vs_Pt", kTH3F, {axisLambdaMass, axisCentBins, axisPtBins});
    }
    ...
    if (getCfg<bool>(cfgPrimVtxParticleCuts, kV0TrkAntiLambda, kV0DoRecoCheck)) {
      ...
      addAllV0Histos(recoV0sPostAntiLambdaCheck, "recoAntiLambda/PostSel/", "Pi/", "Pr/", fillMotherQA, fillDauTrackQA);
      recoV0sPostAntiLambdaCheck.add("recoAntiLambda/PostSel/mAntiLambda_vs_cent", "mAntiLambda_vs_cent", kTH2F, {axisCent, axisLambdaMass});
      recoV0sPostAntiLambdaCheck.add("recoAntiLambda/PostSel/mAntiLambda_vs_cent_effcorr", "mAntiLambda_vs_cent_effcorr", kTH2F, {axisCent, axisLambdaMass});
      recoV0sPostAntiLambdaCheck.add("recoAntiLambda/PostSel/mAntiLambda_vs_cent_vs_Pt", "mAntiLambda_vs_cent_vs_Pt", kTH3F, {axisLambdaMass, axisCentBins, axisPtBins});
    }
    ...
    if (getCfg<bool>(cfgPrimVtxParticleCuts, kV0TrkGamma, kV0DoRecoCheck)) {
      ...
      addAllV0Histos(recoV0sPostGammaCheck, "recoGamma/PostSel/", "El/", "El/", fillMotherQA, fillDauTrackQA);
      recoV0sPostGammaCheck.add("recoGamma/PostSel/mGamma_vs_cent", "mGamma_vs_cent", kTH2F, {axisCent, axisGammaMass});
      recoV0sPostGammaCheck.add("recoGamma/PostSel/mGamma_vs_cent_effcorr", "mGamma_vs_cent_effcorr", kTH2F, {axisCent, axisGammaMass});
      recoV0sPostGammaCheck.add("recoGamma/PostSel/mGamma_vs_cent_vs_Pt", "mGamma_vs_cent_vs_Pt", kTH3F, {axisGammaMass, axisCentBins, axisPtBins});
    }
  ...
  template <int Mode, int fillMode, int pidMode, int signMode, int detMode, typename H, typename T>
  void fillDaughterQASparse(H& histReg, const T& track, const double& particleMass, const float& effWeight)
  {
    float tpcNSigmaVal = -999, tofNSigmaVal = -999;
    switch (pidMode) {
      case kPi:
    ...
    histReg.fill(HIST(HistRegDire[Mode]) + HIST(FillModeDire[fillMode]) + HIST(PidDire[pidMode]) + HIST(SignDire[signMode]) + HIST(DetDire[detMode]) + HIST("h12_dcaZ"), track.dcaZ());
    histReg.fill(HIST(HistRegDire[Mode]) + HIST(FillModeDire[fillMode]) + HIST(PidDire[pidMode]) + HIST(SignDire[signMode]) + HIST(DetDire[detMode]) + HIST("h17_dcaXYwide"), track.dcaXY());
    histReg.fill(HIST(HistRegDire[Mode]) + HIST(FillModeDire[fillMode]) + HIST(PidDire[pidMode]) + HIST(SignDire[signMode]) + HIST(DetDire[detMode]) + HIST("h18_dcaZwide"), track.dcaZ(), effWeight);
    histReg.fill(HIST(HistRegDire[Mode]) + HIST(FillModeDire[fillMode]) + HIST(PidDire[pidMode]) + HIST(SignDire[signMode]) + HIST(DetDire[detMode]) + HIST("h20_pt_eta"), track.pt(), track.eta());

    histReg.fill(HIST(HistRegDire[Mode]) + HIST(FillModeDire[fillMode]) + HIST(PidDire[pidMode]) + HIST(SignDire[signMode]) + HIST(DetDire[detMode]) + HIST("hSparseDauKinePID"),
                 track.p(), track.pt(), track.tpcInnerParam(), track.tofExpMom(), track.tpcSignal(), track.beta(), tpcNSigmaVal, tofNSigmaVal);
    histReg.fill(HIST(HistRegDire[Mode]) + HIST(FillModeDire[fillMode]) + HIST(PidDire[pidMode]) + HIST(SignDire[signMode]) + HIST(DetDire[detMode]) + HIST("hSparseDauDCA"),
                 track.p(), track.pt(), track.dcaXY(), track.dcaZ());
  }
  ...
  void fillV0QA(H histReg, const T& v0, const U& posDaughterTrack, const U& negDaughterTrack, const int& ptEtaBinPosTrk, const int& ptEtaBinNegTrk, const int& v0Tag, const int& v0DauCollisionIndexTag, const int& v0DauBCTag, const int& posDauIdMethod, const int& negDauIdMethod, const float& v0EffWeight, const bool& fillMotherQA, const bool& fillDauTrackQA)
  {
    ...
    if (fillDauTrackQA) {
      ...
      if (posDauIdMethod == kTPCidentified) {
        fillDaughterQASparse<Mode, fillMode, posPidMode, kPos, tpcId>(histReg, posDaughterTrack, posDauMass, posDauEffWeight);
      } else if (posDauIdMethod == kTPCTOFidentified) {
        fillDaughterQASparse<Mode, fillMode, posPidMode, kPos, tpctofId>(histReg, posDaughterTrack, posDauMass, posDauEffWeight);
      } else if (posDauIdMethod == kUnidentified) {
        fillDaughterQASparse<Mode, fillMode, posPidMode, kPos, NoId>(histReg, posDaughterTrack, posDauMass, posDauEffWeight);
      }
      // LOG(info)<<"DEBUG :: Point 03";
      if (negDauIdMethod == kTPCidentified) {
        fillDaughterQASparse<Mode, fillMode, negPidMode, kNeg, tpcId>(histReg, negDaughterTrack, negDauMass, negDauEffWeight);
      } else if (negDauIdMethod == kTPCTOFidentified) {
        fillDaughterQASparse<Mode, fillMode, negPidMode, kNeg, tpctofId>(histReg, negDaughterTrack, negDauMass, negDauEffWeight);
      } else if (negDauIdMethod == kUnidentified) {
        fillDaughterQASparse<Mode, fillMode, negPidMode, kNeg, NoId>(histReg, negDaughterTrack, negDauMass, negDauEffWeight);
      }
    }
  }
                       ...
                       const auto& posTrackPrimVtxMotherFlag, const auto& negTrackPrimVtxMotherFlag, const auto& posTrackV0MotherFlag, const auto& negTrackV0MotherFlag, const int& requiredBit, const auto& mpBit)
  {
    ...
    static const bool fillPostSel = getCfg<bool>(cfgV0ParticleCuts, row, kV0FillPostSel); // DO UPDATE : To be used in future
    ...
    bool countIt = true;
    bool fillMotherQA = fillPostSel;
    bool fillDauTrackQA = fillPostSelDau;
  ...
  void fillPrimVtxCndtPair(H& histReg, const P& mother, const T& posTrack, const T& negTrack, const int& ptEtaBinPosTrk, const int& ptEtaBinNegTrk, const int& idMethodPosDau, const int& idMethodNegDau, int& iNTrkPrim, float& fNTrkPrim, float& effWeightSum, const bool& fillMotherQA, const bool& fillDauTrackQA)
  {
    ...
    if (fillDauTrackQA) {
      ...
      if (idMethodPosDau == kTPCidentified) {
        fillDaughterQASparse<particleMode, fillMode, posPidMode, kPos, tpcId>(histReg, posTrack, posDauMass, posDauEffWeight);
      } else if (idMethodPosDau == kTPCTOFidentified) {
        fillDaughterQASparse<particleMode, fillMode, posPidMode, kPos, tpctofId>(histReg, posTrack, posDauMass, posDauEffWeight);
      } else if (idMethodPosDau == kUnidentified) {
        fillDaughterQASparse<particleMode, fillMode, posPidMode, kPos, NoId>(histReg, posTrack, posDauMass, posDauEffWeight);
      }

      if (idMethodNegDau == kTPCidentified) {
        fillDaughterQASparse<particleMode, fillMode, negPidMode, kNeg, tpcId>(histReg, negTrack, negDauMass, negDauEffWeight);
      } else if (idMethodNegDau == kTPCTOFidentified) {
        fillDaughterQASparse<particleMode, fillMode, negPidMode, kNeg, tpctofId>(histReg, negTrack, negDauMass, negDauEffWeight);
      } else if (idMethodNegDau == kUnidentified) {
        fillDaughterQASparse<particleMode, fillMode, negPidMode, kNeg, NoId>(histReg, negTrack, negDauMass, negDauEffWeight);
      }
    }
  }
                                ...
                                auto& iNTrkDe, auto& fNTrkDe, auto& effWeightDe, auto& effWeightSumDe, auto& iNTrkRejectedDe, auto& fNTrkRejectedDe)
  {
    ...
    executeIdentifiedTrackPart<kPi, IdBitPI>(trackIs[kPi], recoAnalysisPi, recoRejectedPi, track, MassPiPlus, idMethod[kPi],
                                             iNTrkRejectedPi, fNTrkRejectedPi, iNTrkPi, fNTrkPi, effWeightPi, effWeightSumPi,
                                             ptEtaBin, decayDauTagBit);

    executeIdentifiedTrackPart<kKa, IdBitKA>(trackIs[kKa], recoAnalysisKa, recoRejectedKa, track, MassKPlus, idMethod[kKa],
                                             iNTrkRejectedKa, fNTrkRejectedKa, iNTrkKa, fNTrkKa, effWeightKa, effWeightSumKa,
                                             ptEtaBin, decayDauTagBit);

    executeIdentifiedTrackPart<kPr, IdBitPR>(trackIs[kPr], recoAnalysisPr, recoRejectedPr, track, MassProton, idMethod[kPr],
                                             iNTrkRejectedPr, fNTrkRejectedPr, iNTrkPr, fNTrkPr, effWeightPr, effWeightSumPr,
                                             ptEtaBin, decayDauTagBit);

    executeIdentifiedTrackPart<kEl, IdBitEL>(trackIs[kEl], recoAnalysisEl, recoRejectedEl, track, MassElectron, idMethod[kEl],
                                             iNTrkRejectedEl, fNTrkRejectedEl, iNTrkEl, fNTrkEl, effWeightEl, effWeightSumEl,
                                             ptEtaBin, decayDauTagBit);

    executeIdentifiedTrackPart<kDe, IdBitDE>(trackIs[kDe], recoAnalysisDe, recoRejectedDe, track, MassDeuteron, idMethod[kDe],
  ...
  void getTrackMcBitTag(auto& trackTrueMcTag, const auto& mcPart)
  {
    ...
    if (absPdg == kPiPlus) {
      BITSET(trackTrueMcTag, IdBitPI);
    } else if (absPdg == kKPlus) {
      BITSET(trackTrueMcTag, IdBitKA);
    } else if (absPdg == kProton) {
      BITSET(trackTrueMcTag, IdBitPR);
    } else if (absPdg == kElectron) {
      BITSET(trackTrueMcTag, IdBitEL);
    } else if (absPdg == kMuonMinus) {
      BITSET(trackTrueMcTag, IdBitMU);
    } else if (absPdg == kDeuteron) {
      BITSET(trackTrueMcTag, IdBitDE);
    }
  }
  ...
  void executeAnalysis(const C& collisions, const V& V0s, const T& tracks, const P& primVtxCndts)
  {
    ...
    if constexpr (analysisType == doDataProcessing || analysisType == doRecoProcessing || analysisType == doPurityProcessing) {
      ...
      for (const auto& collision : collisions) {
        ...
        executeSortPairDaughters<false>(primVtxCndtDauList[kPrimTrkPhi1020][kPos], primVtxCndtDauList[kPrimTrkPhi1020][kNeg], recoPhi1020.get<TH1>(HIST(HistRegDire[Phi1020]) + HIST(FillModeDire[kFillPreSel]) + HIST("nCommonKaOfDifferentPhi")));
        executeSortPairDaughters<false>(primVtxCndtDauList[kPrimTrkJPsiToEE][kPos], primVtxCndtDauList[kPrimTrkJPsiToEE][kNeg], recoJPsiToEE.get<TH1>(HIST(HistRegDire[JPsiToEE]) + HIST(FillModeDire[kFillPreSel]) + HIST("nCommonKaOfDifferentJPsiToEE")));
        ...
        executeSortPairDaughters<false>(primVtxCndtDauList[kPrimTrkKStar892][kPos], primVtxCndtDauList[kPrimTrkKStar892][kNeg], recoKStar892.get<TH1>(HIST(HistRegDire[KStar892]) + HIST(FillModeDire[kFillPreSel]) + HIST("nCommonKaOfDifferentKStar892")));
        executeSortPairDaughters<false>(primVtxCndtDauList[kPrimTrkKStar892Bar][kPos], primVtxCndtDauList[kPrimTrkKStar892Bar][kNeg], recoKStar892Bar.get<TH1>(HIST(HistRegDire[KStar892Bar]) + HIST(FillModeDire[kFillPreSel]) + HIST("nCommonKaOfDifferentKStar892Bar")));
        executeSortPairDaughters<false>(primVtxCndtDauList[kPrimTrkRho770][kPos], primVtxCndtDauList[kPrimTrkRho770][kNeg], recoRho770.get<TH1>(HIST(HistRegDire[Rho770]) + HIST(FillModeDire[kFillPreSel]) + HIST("nCommonKaOfDifferentRho770")));
      ...
      skipSecondPrimVtxCndtLoop:
        ...
        for (const auto& track : tracksTablePerColl) {
          ...
          if (selPion(track, idMethodTrk[kPi]) && isTrueMcMatch<analysisType>(trackTrueMcTag, IdBitPI)) {
            trackIs[kPi] = true;
            BITSET(trackIdTag, IdBitPI);
          }
          if (selKaon(track, idMethodTrk[kKa]) && isTrueMcMatch<analysisType>(trackTrueMcTag, IdBitKA)) {
            trackIs[kKa] = true;
            BITSET(trackIdTag, IdBitKA);
          }
          if (selProton(track, idMethodTrk[kPr]) && isTrueMcMatch<analysisType>(trackTrueMcTag, IdBitPR)) {
            trackIs[kPr] = true;
            BITSET(trackIdTag, IdBitPR);
          }
          if (selElectron(track, idMethodTrk[kEl]) && isTrueMcMatch<analysisType>(trackTrueMcTag, IdBitEL)) {
            trackIs[kEl] = true;
            BITSET(trackIdTag, IdBitEL);
          }
          if (selMuon(track, idMethodTrk[kMu]) && isTrueMcMatch<analysisType>(trackTrueMcTag, IdBitMU)) {
            trackIs[kMu] = true;
            BITSET(trackIdTag, IdBitMU);
          }
          if (selDeuteron(track, idMethodTrk[kDe]) && isTrueMcMatch<analysisType>(trackTrueMcTag, IdBitDE)) {
            trackIs[kDe] = true;
            BITSET(trackIdTag, IdBitDE);
          }
