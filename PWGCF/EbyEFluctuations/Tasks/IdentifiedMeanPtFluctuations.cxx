...
using MCCollisions = soa::Join<aod::Collisions, aod::EvSels, aod::MultsExtra,  aod::Mults, aod::CentFT0Cs, aod::McCollisionLabels>;

using MCTracks =soa::Join<aod::FullTracks,
                           aod::pidTPCFullEl, aod::pidTPCFullMu, aod::pidTPCFullPi,
                           aod::pidTPCFullKa, aod::pidTPCFullPr, aod::pidTPCFullDe,
                           aod::pidTOFFullEl, aod::pidTOFFullMu, aod::pidTOFFullPi, aod::StoredTracks,
                           aod::pidTOFFullKa, aod::pidTOFFullPr, aod::pidTOFFullDe, aod::pidTOFbeta, aod::TOFSignal,
                           aod::TracksExtra, aod::TracksIU, aod::TracksDCA, aod::TrackSelection, aod::McTrackLabels>;//, aod::StoredMcParticles_000>;
...
void processMCReco(aod::MCCollisions::iterator const& mccoll, aod::MCTracks const& mcrectrack, aod::McParticles const& mcParticles, aod::McCollisions const& mcCollisions)
  {
...
if (!mccoll.selection_bit(o2::aod::evsel::kIsVertexITSTPC)){ // bunch crossing is far from ITS RO Frame border
 ...
 if(!(track1.has_collision())) continue;
 if(!(track1.has_mcParticle())) continue;// *confirm*
 if(!(track1.mcParticle().isPhysicalPrimary())) continue;
// auto mctrack1 = track1.mcParticle();
 if (!track1.isGlobalTrack()) continue;
...
void processMCGen(aod::McCollision const& mcCollision, aod::McParticles& mcParticles)//, const soa::SmallGroups<MCCollisions>& collisions)
 ...
 if (fabs(mcCollision.posZ()) > 10.f) {
    ...
    histos.fill(HIST("MC_hZvtx_after_sel"), mcCollision.posZ());
       ...
       // INITALIZE VARIABLE FOR ADDING IDENTIFIED PT //
 ...
 //tracks passed after DCAxy
  ...
  //    if (!(fabs(track.dcaZ()) < 1.)) continue;//global cut already includes (DCAz< 2.0) cm
      ...
      //tracks passed after pT-cut
       if (!(track.pt() > 0.15 && track.pt() < 2.)) continue;//pt = 0.15
