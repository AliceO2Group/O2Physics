...
{count_rec_p_pdg++;
 ...
 if (abs(track1.tpcNSigmaEl()) < 3.0 &&  abs(track1.tpcNSigmaPi()) > 3. && abs(track1.tpcNSigmaKa()) > 3. && abs(track1.tpcNSigmaPr()) > 3.) continue;
...
void processMCGen(aod::McCollision const& mcCollision, aod::McParticles& mcParticles)
 ...
 if (!(mcgentrack.pt() > 0.15) || !(mcgentrack.pt() < 2.)) continue;
        if (!(mcgentrack.eta() > -0.8) || !(mcgentrack.eta() < 0.8)) continue;
