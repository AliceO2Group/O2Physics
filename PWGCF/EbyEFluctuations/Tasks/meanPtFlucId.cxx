...
struct MeanPtFlucId {
  ...
  Configurable<std::vector<float>> purityValuesPi{"purityValuesPi", {0, 0, 0, 0.99806, 0.999336, 0.999374, 0.999353, 0.999431, 0.999387, 0.999303, 0.998943, 0.998741, 0.999079, 0.99914, 0.999052, 0.998908, 0.998752, 0.998336, 0.996162, 0.996551, 0.990283, 0.989347, 0.981886, 0.980695, 0.973442, 0.972182, 0.961628, 0.964047, 0.95346, 0.944015, 0.945401, 0.938005, 0.924641, 0.918288}, "purity values for Pions"};
  ...
  Configurable<std::vector<float>> purityValuesKa{"purityValuesKa", {0, 0, 0, 0, 0, 0.98435, 0.957291, 0.935329, 0.871784, 0.811303, 0.995, 0.990426, 0.983447, 0.959949, 0.942029, 0.933996, 0.92535, 0.933555, 0.920484, 0.861257, 0.833764, 0.783457, 0.74504, 0.722, 0.701719, 0.667725, 0.682614, 0.670213, 0.685517, 0.685714, 0.656347, 0.610195, 0.630972, 0.574359}, "purity values for Kaons"};
  ...
  Configurable<std::vector<float>> purityValuesPr{"purityValuesPr", {0, 0, 0, 0, 0, 0, 0, 0, 0.998678, 0.997749, 0.989432, 0.982925, 0.957169, 0.945139, 0.937956, 0.934673, 0.924851, 0.996822, 0.997722, 0.993647, 0.988127, 0.987143, 0.990033, 0.982315, 0.968165, 0.965909, 0.954902, 0.953092, 0.96, 0.964286, 0.946629, 0.934911, 0.940063, 0.957627}, "purity values for Protons"};
  ...
  void fillIdParticleQAHistos(T const& track, std::vector<float> effBins, std::vector<float> effValues, std::vector<float> pureValues, std::vector<float> trkEffValues, double rap, double nSigmaTPC, double nSigmaTOF, int nFT0M, int& N, double& Q1, double& Q2, double& Q3, double& Q4)
  {
    double pt = track.pt();
    if (cfgCorrection == true) {
      int binIndex = findBin(pt, effBins);
      auto effVal = static_cast<std::vector<float>>(effValues);
      auto pureVal = static_cast<std::vector<float>>(pureValues);
      auto trkEffVal = static_cast<std::vector<float>>(trkEffValues);
      if (binIndex != -1) {
        float efficiency = effVal[binIndex];
        float purity = pureVal[binIndex];
        float trkEff = trkEffVal[binIndex];
        if (efficiency > 0) {
          float weight = 1.0 / (/* trkEff * */ efficiency);
          N += weight;
      ...
      for (const auto& track : tracks) {
          ...
          if (cfgCorrection == true) {
            ...
            if (binIndex != -1) {
              ...
              if (efficiency > 0) {
                ...
                // LOGF(info, "pt: %f, eff: %f, binIndex: %d, weight: %f",  ptCh, efficiency, binIndex, weight);
              }
            }
          } else {
            nCh++;
        ...
        if (cfgSelOR == true && cfgSelAND == false) {
          if (selLowPi(track) == cfgSelLow || selHighPi(track) == cfgSelHigh) {
            fillIdParticleQAHistos<QA_Pion>(track, effBinsPi, effValuesPi, purityValuesPi, effValuesCh, rapPi, nSigmaTPCPi, nSigmaTOFPi, nFT0M, nPi, q1Pi, q2Pi, q3Pi, q4Pi);
          }
        ...
        if (cfgSelOR == true && cfgSelAND == false) {
          if (selLowKa(track) == cfgSelLow || selHighKa(track) == cfgSelHigh) {
            fillIdParticleQAHistos<QA_Kaon>(track, effBinsKa, effValuesKa, purityValuesKa, effValuesCh, rapKa, nSigmaTPCKa, nSigmaTOFKa, nFT0M, nKa, q1Ka, q2Ka, q3Ka, q4Ka);
          }
        } else if (cfgSelOR == false && cfgSelAND == true) {
          if (selLowKa(track) == cfgSelLow && selHighKa(track) == cfgSelHigh) {
            fillIdParticleQAHistos<QA_Kaon>(track, effBinsKa, effValuesKa, purityValuesKa, effValuesCh, rapKa, nSigmaTPCKa, nSigmaTOFKa, nFT0M, nKa, q1Ka, q2Ka, q3Ka, q4Ka);
        ...
        if (cfgSelOR == true && cfgSelAND == false) {
          if (selLowPr(track) == cfgSelLow && selHighPr(track) == cfgSelHigh) {
            fillIdParticleQAHistos<QA_Proton>(track, effBinsPr, effValuesPr, purityValuesPr, effValuesCh, rapPr, nSigmaTPCPr, nSigmaTOFPr, nFT0M, nPr, q1Pr, q2Pr, q3Pr, q4Pr);
          }
        } else if (cfgSelOR == false && cfgSelAND == true) {
          if (selLowPr(track) == cfgSelLow && selHighPr(track) == cfgSelHigh) {
            fillIdParticleQAHistos<QA_Proton>(track, effBinsPr, effValuesPr, purityValuesPr, effValuesCh, rapPr, nSigmaTPCPr, nSigmaTOFPr, nFT0M, nPr, q1Pr, q2Pr, q3Pr, q4Pr);
      ...
      for (const auto& track : tracks) {
        ...
        if (selTrack(track)) {
          if (std::abs(track.eta()) < 0.8) {
            double pt = track.pt();
            if (cfgCorrection == true) {
              int binIndex = findBin(pt, effBinsCh);
              auto effValCh = static_cast<std::vector<float>>(effValuesCh);
              if (binIndex != -1) {
                float efficiency = effValCh[binIndex];
                if (efficiency > 0) {
