...
// \file multiparticle-correlations-mei.cxx
...
struct MultiparticleCorrelationsMei  // this name is used in lower-case format to name the TDirectoryFile in AnalysisResults.root
{
    ...
    // templates
    template <ERecSim rs, typename T1> bool EventCuts(T1 const& collision) 
    {
    ...
    template <ERecSim rs, ECuts cuts, typename T1, typename T2> void EventHistFill(T1 const& collision, T2 const& tracks) 
    {
    ...
    template <ERecSim rs, typename T> bool ParticlECuts(T const& track)
    {
        if constexpr (rs == ERec || rs == ERecAndSim) 
        {
    ...
    template <ERecSim rs, ECuts cuts, typename T1> void ParticlEHistFill(T1 const& track)
    {
        if constexpr (rs == ERec || rs == ERecAndSim) 
        {
            if constexpr (cuts == ENoCuts)
    ...
    template <ERecSim rs, typename T1> void qaFill(T1 const& collision) 
    {
        auto thisCent = collision.centFT0C(); // use auto to determine the type
        switch (centralityEstimator) 
        {
