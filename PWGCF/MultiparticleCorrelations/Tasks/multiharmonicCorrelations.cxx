...
struct MultiharmonicCorrelations { // this name is used in lower-case format to name the TDirectoryFile in AnalysisResults.root
  ...
  Configurable<std::vector<float>> cf_centr_bins{"cf_centr_bins", {100, 0., 100.}, "nCentrBins, centrMin, centrMax"};
  ...
  Configurable<std::vector<float>> cf_tpcncls_bins{"cf_tpcncls_bins", {100, 0., 1000.}, "ntpcnclsBins, tpnclsMin, tpcnclsMax"};
  Configurable<std::vector<float>> cf_dcaxy_bins{"cf_dcaxy_bins", {1000, -20., 20.}, "ndcaxyBins, dcaxyMin, dcaxyMax"};
  Configurable<std::vector<float>> cf_dcaz_bins{"cf_dcaz_bins", {1000, -10., 10.}, "ndcazBins, dcazMin, dcazMax"};
  Configurable<std::vector<float>> cf_ncontr_bins{"cf_ncontr_bins", {100, 0., 1000}, "nNContrBins, NContrMin, NContrMax"};
  ...
  void Steer(T1 const& collision, T2 const& tracks)
  {
    ...
    four32 = Four(3, 2, -3, -2).Re() / Four(0, 0, 0, 0).Re();
    four42 = Four(4, 2, -4, -2).Re() / Four(0, 0, 0, 0).Re();
    v22 = (Q(2).Rho2() - M) / (M * (M - 1.));
    v32 = (Q(3).Rho2() - M) / (M * (M - 1.));
    v42 = (Q(4).Rho2() - M) / (M * (M - 1.));
  ...
  void init(InitContext&)
  {
    ...
    vector<float> l_tpcncls_bins = cf_tpcncls_bins.value;
    vector<float> l_dcaxy_bins = cf_dcaxy_bins.value;
    vector<float> l_dcaz_bins = cf_dcaz_bins.value;
    vector<float> l_ncontr_bins = cf_ncontr_bins.value;
    ...
    cor.pv22_centr = new TProfile("pv22", "profile of v2^2", 9, quantiles);
    cor.pv32_centr = new TProfile("pv32", "profile of v3^2", 9, quantiles);
    cor.pv42_centr = new TProfile("pv42", "profile of v4^2", 9, quantiles);
    cor.pfour32_centr = new TProfile("pfour32", "profile of v2^2*v3^2", 9, quantiles);
    cor.pfour42_centr = new TProfile("pfour32", "profile of v2^2*v4^2", 9, quantiles);
    cor.pv22_centr->GetXaxis()->SetTitle("v_{2}^{2}");
    cor.pv32_centr->GetXaxis()->SetTitle("v_{3}^{2}");
    cor.pv42_centr->GetXaxis()->SetTitle("v_{4}^{2}");
    cor.pfour32_centr->GetXaxis()->SetTitle("v_{2}^{2}v_{3}^{2}");
    cor.pfour42_centr->GetXaxis()->SetTitle("v_{2}^{2}v_{4}^{2}");
