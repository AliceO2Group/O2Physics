...
struct MultiparticleCumulants { // this name is used in lower-case format to name the TDirectoryFile in AnalysisResults.root
    ...
    template <EnRecSim rs, typename T1> bool EventCuts(T1 const& collision){
        return (collision.posZ() < tc.fVertexZ[1] && collision.posZ() > tc.fVertexZ[0]);
    }

    template <EnRecSim rs, typename T1> bool ParticleCuts(T1 const& track){
    ...
    std::vector<TH1F*> getHistogramsWithWeights(const char* filePath, const char* runNumber)
    {
        ...
        if (filePathStr(0, 17) == "/alice/data/CCDB/" || filePathStr(0, 15) == "/alice/cern.ch/") {bFileIsInAliEn = true;}
        else if (filePathStr(0, 20) == "/alice-ccdb.cern.ch/") {bFileIsInCCDB = true;}
    ...
    template <EnRecSim rs, typename T1, typename T2> void Steer(T1 const& collision, T2 const& tracks) {
        ...
        if (tc.fCentEstm == "FT0M") rlCollisionCent = collision.centFT0M();
        else if (tc.fCentEstm == "FV0A") rlCollisionCent = collision.centFV0A();
        else if (tc.fCentEstm == "NTPV") rlCollisionCent = collision.centNTPV();
        else if (tc.fCentEstm == "FT0C") rlCollisionCent = collision.centFT0C();
        ...
        if (tc.fMultEstm == "FT0A") rlCollisionMult = collision.multFT0A();
        else if (tc.fMultEstm == "FT0C") rlCollisionMult = collision.multFT0C();
        else if (tc.fMultEstm == "FT0M") rlCollisionMult = collision.multFT0M();
        else if (tc.fMultEstm == "FV0A") rlCollisionMult = collision.multFV0A();
        else if (tc.fMultEstm == "FV0C") rlCollisionMult = collision.multFV0C();
        else if (tc.fMultEstm == "NTracksPV") rlCollisionMult = collision.multNTracksPV();
        ...
        if constexpr (rs == eRec || rs == eRecAndSim) {
            ...
            if constexpr (rs == eRecAndSim){
                ...
                if(qa.fQASwitch) {qa.fQAHistograms[eQACent][0]->Fill(rlCollisionCent, mcCollisionCent);}
            }
        }
    ...
    template <EnParticleHistograms histType, typename T1> void BookParticleHistograms(T1 const& lPcBins, ParticleHistograms& pc){
    ...
    template <EnEventHistograms histType, typename T1> void BookEventHistograms(T1 const& lEvBins, EventHistograms& ev){
