...
FlowPtContainer::FlowPtContainer(const char* name):
    TNamed(name,name),
    fCMTermList(0),
    fCorrList(0),
    fCovList(0),
    ...
    corrDen()
{};
FlowPtContainer::FlowPtContainer(const char* name, const char* title, int nbinsx, double* xbins, const int &m, const GFWCorrConfigs &configs):
    TNamed(name,title),
    fCMTermList(0),
    fCorrList(0),
    fCovList(0),
...
FlowPtContainer::FlowPtContainer(const char* name, const char* title, int nbinsx, double xlow, double xhigh, const int &m, const GFWCorrConfigs &configs):
    TNamed(name,title),
    fCMTermList(0),
    fCorrList(0),
    fCovList(0),
...
void FlowPtContainer::Initialise(const o2::framework::AxisSpec axis, const int &m, const GFWCorrConfigs &configs, const int &nsub) {
    ...
    if(fCovList) delete fCovList;
    fCovList = new TList();
    fCovList->SetOwner(kTRUE);
    for(int m = 0;m < mpar; ++m) fCorrList->Add(new BootstrapProfile(Form("mpt%i",m+1),Form("corr_%ipar",m+1),nMultiBins, &multiBins[0]));
    for(int m = 0;m < 4; ++m) {
        for(int i=0;i<=m;++i)
    ...
    for(int i = 0; i < configs.GetSize(); ++i){
        for(auto m(1); m <= mpar; ++m){
            if(!(configs.GetpTCorrMasks()[i]&(1<<(m-1)))) continue;
...
void FlowPtContainer::Initialise(int nbinsx, double* xbins, const int &m, const GFWCorrConfigs &configs, const int &nsub) {
    ...
    fCorrList = new TList();
    fCorrList->SetOwner(kTRUE);
    for(int m=0;m<mpar;++m) fCorrList->Add(new BootstrapProfile(Form("mpt%i",m+1),Form("mpt%i",m+1),nbinsx,xbins));
    for(int m=0;m<4;++m) {
        for(int i=0;i<=m;++i)
...
void FlowPtContainer::Initialise(int nbinsx, double xlow, double xhigh, const int &m, const GFWCorrConfigs &configs, const int &nsub) {
    ...
    fCorrList = new TList();
    fCorrList->SetOwner(kTRUE);
    for(int m=0;m<mpar;++m) fCorrList->Add(new BootstrapProfile(Form("mpt%i",m+1),Form("mpt%i",m+1),nbinsx,xlow,xhigh));
    for(int m=0;m<4;++m) {
        for(int i=0;i<=m;++i)
...
void FlowPtContainer::CalculateCorrelations() {
  corrNum.clear(); corrNum.resize(mpar+1,0); corrNum[0] = 1.0;
  corrDen.clear(); corrDen.resize(mpar+1,0); corrDen[0] = 1.0;
  double sumNum = 0;
  double sumDenum = 0;
...
void FlowPtContainer::FillPtProfiles(const double &centmult, const double &rn) {
    for(int m=1;m<=mpar;++m)
    {
        if(corrDen[m]!=0) dynamic_cast<BootstrapProfile*>(fCorrList->At(m-1))->FillProfile(centmult,corrNum[m]/corrDen[m],(fEventWeight==kEventWeight::kUnity)?1.0:corrDen[m],rn);
    }
    return;
}
void FlowPtContainer::FillVnPtProfiles(const double &centmult, const double &flowval, const double &flowtuples,  const double &rn, uint8_t mask){
  ...
  for(auto m(1); m <= mpar; ++m){
    ...
    if(corrDen[m] != 0) dynamic_cast<BootstrapProfile*>(fCovList->At(fillCounter))->FillProfile(centmult,flowval*corrNum[m]/corrDen[m],(fEventWeight==kUnity)?1.0:flowtuples*corrDen[m],rn);
    ++fillCounter;
  }
  return;
}
void FlowPtContainer::FillCMProfiles(const double &centmult, const double &rn) {
    if(sumP[GetVectorIndex(0,0)]==0) return;
    ...
    if(mpar<1 || sumP[GetVectorIndex(1,0)] == 0) return;
    ...
    if(mpar<2 || sumP[GetVectorIndex(2,0)] == 0 || weight1 == 0 ) return;
    ...
    if(mpar<3 || sumP[GetVectorIndex(3,0)] == 0 || weight2 == 0) return;
    ...
    if(mpar<4 || sumP[GetVectorIndex(4,0)] == 0 || weight3 == 0) return;
...
void FlowPtContainer::RebinMulti(Int_t nbins) {
    if(fCMTermList) for(Int_t i=0;i<fCMTermList->GetEntries();i++) dynamic_cast<BootstrapProfile*>(fCMTermList->At(i))->RebinMulti(nbins);
    if(fCorrList) for(Int_t i=0;i<fCorrList->GetEntries();i++) dynamic_cast<BootstrapProfile*>(fCorrList->At(i))->RebinMulti(nbins);
    if(fCovList) for(Int_t i=0;i<fCovList->GetEntries();i++) dynamic_cast<BootstrapProfile*>(fCovList->At(i))->RebinMulti(nbins);
    return;
}
void FlowPtContainer::RebinMulti(Int_t nbins, Double_t *binedges) {
    if(fCMTermList) for(Int_t i=0;i<fCMTermList->GetEntries();i++) dynamic_cast<BootstrapProfile*>(fCMTermList->At(i))->RebinMulti(nbins,binedges);
    if(fCorrList) for(Int_t i=0;i<fCorrList->GetEntries();i++) dynamic_cast<BootstrapProfile*>(fCorrList->At(i))->RebinMulti(nbins,binedges);
    if(fCovList) for(Int_t i=0;i<fCovList->GetEntries();i++) dynamic_cast<BootstrapProfile*>(fCovList->At(i))->RebinMulti(nbins,binedges);
    return;
}
...
TH1* FlowPtContainer::getCentralMomentHist(int ind, int m) {
  ...
  if(ind+1<fCentralMomentList->GetEntries()) return dynamic_cast<TH1*>(fCentralMomentList->FindObject(Form("cm%i_%i",m+1,ind)));
  return 0;
}
...
TH1* FlowPtContainer::getCumulantHist(int ind, int m) {
    ...
    if(ind+1<fCumulantList->GetEntries()) return dynamic_cast<TH1*>(fCumulantList->At((ind+1)*mpar+m-1));
}
...
Long64_t FlowPtContainer::Merge(TCollection *collist) {
  if(!fCorrList || !fCMTermList) return 0;
  Long64_t nmerged=0;
  ...
  while ((l_PTC = dynamic_cast<FlowPtContainer*>(all_PTC()))) {
      ...
      if(t_CMTerm) {
        if(!fCMTermList) fCMTermList = dynamic_cast<TList*>(t_CMTerm->Clone());
        else MergeBSLists(fCMTermList,t_CMTerm);
        nmerged++;
      };
      if(t_Corr) {
          if(!fCorrList) fCorrList = dynamic_cast<TList*>(t_Corr->Clone());
          else MergeBSLists(fCorrList,t_Corr);
      };
      if(t_Cum) {
          if(!fCumulantList) fCumulantList = dynamic_cast<TList*>(t_Cum->Clone());
          else MergeBSLists(fCumulantList,t_Cum);
      };
      if(t_CM) {
          if(!fCentralMomentList) fCentralMomentList = dynamic_cast<TList*>(t_CM->Clone());
