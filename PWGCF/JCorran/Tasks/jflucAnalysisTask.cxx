  ...
  void processLoadWeights(aod::JCollision const& collision, aod::JTracks const& tracks){
	...
	for(auto &track : tracks){
		if(pf){
			...
			if(m == nuaCache.end()){
				TH1 *ph = (TH1*)pf->Get(Form("PhiWeights_%u_%02u", collision.runNumber(), centBin));
				if(ph){
...
WorkflowSpec defineDataProcessing(ConfigContext const& cfgc)
{
	...
	}else return WorkflowSpec{adaptAnalysisTask<jflucAnalysisTask>(cfgc)};
}
