...
/// \file LambdaSpinCorrelation.cxx
/// \brief Spin Correlation of Lambda baryons.
...
struct LambdaTableProducer {
  ...
  template <RunType run, typename C, typename V, typename T>
  ...
  {
    fillLambdaRecoTables<kRun3>(collision, V0s, tracks);
  }
  ...
  {
    fillLambdaRecoTables<kRun2>(collision, V0s, tracks);
  }
...
struct LambdaSpinCorrelation {
  ...
  void init(InitContext const&)
  {
    ...
    // Create Histograms.
    // Event
    histos.add("Event/h1f_collision_posz", "V_{Z} Distribution", kTH1F, {axisPosZ});
    histos.add("Event/h1f_ft0m_mult_percentile", "FT0M (%)", kTH1F, {axisCent});
    histos.add("Event/h2f_Mult_vs_Centrality", "N_{ch} vs FT0M(%)", kTH2F, {axisCent, axisChMult});
    histos.add("Event/h2f_lambda_mult", "#Lambda - Multiplicity", kTH2F, {axisCent, axisMult});
    histos.add("Event/h2f_antilambda_mult", "#bar{#Lambda} - Multiplicity", kTH2F, {axisCent, axisMult});

    // Single and Two Particle Densities
    // 1D Histograms
    histos.add("Reco/h3f_n1_centmasspt_LaP", "#rho_{1}^{#Lambda}", kTH3F, {axisCent, axisMass, axisPt});
    histos.add("Reco/h3f_n1_centmasspt_LaM", "#rho_{1}^{#bar{#Lambda}}", kTH3F, {axisCent, axisMass, axisPt});
    histos.add("Reco/h2f_n1_pt_LaP", "#rho_{1}^{#Lambda}", kTH2F, {axisCent, axisPt});
    histos.add("Reco/h2f_n1_pt_LaM", "#rho_{1}^{#bar{#Lambda}}", kTH2F, {axisCent, axisPt});
    histos.add("Reco/h2f_n1_eta_LaP", "#rho_{1}^{#Lambda}", kTH2F, {axisCent, axisEta});
    histos.add("Reco/h2f_n1_eta_LaM", "#rho_{1}^{#bar{#Lambda}}", kTH2F, {axisCent, axisEta});
    histos.add("Reco/h2f_n1_rap_LaP", "#rho_{1}^{#Lambda}", kTH2F, {axisCent, axisRap});
    histos.add("Reco/h2f_n1_rap_LaM", "#rho_{1}^{#bar{#Lambda}}", kTH2F, {axisCent, axisRap});
    histos.add("Reco/h2f_n1_phi_LaP", "#rho_{1}^{#Lambda}", kTH2F, {axisCent, axisPhi});
    histos.add("Reco/h2f_n1_phi_LaM", "#rho_{1}^{#bar{#Lambda}}", kTH2F, {axisCent, axisPhi});
  ...
  void analyzeSingles(T const& tracks)
  {
    ...
    for (auto const& track : tracks) {
      ...
      // Fill histograms
      histos.fill(HIST("Reco/h3f_n1_centmasspt_") + HIST(SubDirHist[part]), cent, track.mass(), track.pt());
      histos.fill(HIST("Reco/h2f_n1_pt_") + HIST(SubDirHist[part]), cent, track.pt());
      histos.fill(HIST("Reco/h2f_n1_eta_") + HIST(SubDirHist[part]), cent, track.eta());
      histos.fill(HIST("Reco/h2f_n1_phi_") + HIST(SubDirHist[part]), cent, track.phi());
      histos.fill(HIST("Reco/h2f_n1_rap_") + HIST(SubDirHist[part]), cent, track.rap());
    ...
    if (ntrk != 0) {
      if (part == kLambda) {
        histos.fill(HIST("Event/h2f_lambda_mult"), cent, ntrk);
      } else {
        histos.fill(HIST("Event/h2f_antilambda_mult"), cent, ntrk);
      }
    }
  }
  ...
  {
    histos.fill(HIST("Event/h1f_collision_posz"), collision.posZ());
    histos.fill(HIST("Event/h1f_ft0m_mult_percentile"), collision.cent());
    histos.fill(HIST("Event/h2f_Mult_vs_Centrality"), collision.cent(), collision.mult());
