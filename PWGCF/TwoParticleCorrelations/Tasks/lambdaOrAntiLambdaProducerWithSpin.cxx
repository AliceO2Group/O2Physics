...
/*void customize(std::vector<ConfigParamSpec>& workflowOptions)
{
  workflowOptions.push_back(
    {"isMC", VariantType::Bool, false, {"Run the Monte Carlo analysis workflow"}});
}*/
...
DECLARE_SOA_COLUMN(IsAntiLambdaHypothesis, isAntiLambdaHypothesis, bool);
...
DECLARE_SOA_TABLE(LambdaOrAntiLambdas,
                  "AOD",
                  "LAMORANTILAM",
                  o2::soa::Index<>,
...
DECLARE_SOA_COLUMN(ProTPCNSigma, proTPCNSigma, float);
...
DECLARE_SOA_COLUMN(PieTPCNSigma, pieTPCNSigma, float);

} // namespace mcrecolambda

DECLARE_SOA_TABLE(McRecoLambdaCandidates,
                  "AOD",
                  "MCRECOLAMBDA",
...
DECLARE_SOA_TABLE(EveSelPassMcGenLambdaCandidates,
                  "AOD",
                  "MCGENLAMBDA",
...
struct LambdaOrAntiLambdaProducerWithSpin {
  ...
  ConfigurableAxis axisLambdaAntiLambdaPt{"axisLambdaAntiLambdaPt", {200, 0.f, 10.f}, "LambdaAntiLambda transverse-momentum axis"};
  ...
  void init(InitContext const&)
  {
    ...
    rLambdaOrAntiLambda.add("Candidate/hMassVsPt",
                            "#Lambda invariant mass versus p_{T};"
                            "p_{T} (GeV/c);"
                            "m_{#Lambda} (GeV/c^{2})",
                            HistType::kTH2F, {axisLambdaAntiLambdaPt, axisInvariantMass});
...
struct LambdaAntiLambdaSelector {
  ...
  void addSelectedHistograms(std::string const& prefix)
  {
    ...
    rSelected.add((prefix + "/Candidate/hMassVsPt").c_str(),
                  "#Candidate invariant mass versus p_{T};"
                  "p_{T} (GeV/c);"
                  "m_{#Lambda} (GeV/c^{2})",
                  HistType::kTH2F, {axisPt, axisInvariantMass});
...
struct LambdaAntiLambdaMcRecoTableProducer {
    ...
    V0CandidatesMC const& V0s, DaughterTracks const&, aod::McParticles const&)
  {
    ...
    for (const auto& v0 : V0s) {
      ...
      const bool correctSpecies =
        (isLambda && pdgCode == PDG_t::kLambda0) ||
        (isAntiLambda && pdgCode == PDG_t::kLambda0Bar);

      if (!correctSpecies) {
        continue;
      }

      const bool isTrueAntiLambda = (pdgCode == PDG_t::kLambda0Bar);
...
struct LambdaAntiLambdaEfficiencyPlots {
  ...
  ConfigurableAxis axisGenPt{"axisGenPt", {200, 0.f, 10.f}, "McGen transverse-momentum axis"};
  ConfigurableAxis axisDeltaPt{"axisDeltaPt", {200, 0.f, 2.f}, "|pt_{Reco} - pT_{Gen}|"};
  ConfigurableAxis axisMcRecoEta{"axisMcRecoEta", {100, -2.f, 2.f}, "McReco eta axis"};
  ConfigurableAxis axisMcGenEta{"axisMcGenEta", {100, -2.f, 2.f}, "McGen eta axis"};
  ...
  Filter mcRecoDaughterFilter = aod::mcrecolambda::proPt > minDaughterPt&& aod::mcrecolambda::piePt > minDaughterPt&&
                                                                                                        nabs(aod::mcrecolambda::proEta) < maxDaughterEta&& nabs(aod::mcrecolambda::pieEta) < maxDaughterEta&& aod::mcrecolambda::proTPCCrossedRows
                                >= minTPCCrossedRows&& aod::mcrecolambda::pieTPCCrossedRows >= minTPCCrossedRows;
  ...
  PROCESS_SWITCH(LambdaAntiLambdaEfficiencyPlots, processEfficiencyMcReco, "Fill MC reconstructed efficiency histograms", false);
  ...
  void processEfficiencyMcGen(SelectedMcGenEvents::iterator const& selectedMcEvent, aod::McParticles const& allMcParticles)
  {
    ...
    for (const auto& particle : particlesFromThisMcCollision) {
      ...
      if (particle.pt() <= minLambdaPt || particle.pt() >= maxLambdaPt) {
        continue;
      }
      if (std::abs(particle.y()) >= maxLambdaRapidity) {
        continue;
      }
      // Lambda or anti-Lambda

      if (particle.pdgCode() == PDG_t::kLambda0) {
...
struct LambdaAntiLambdaSelectionCutFlow {
  ...
  // Daughter selection
  Configurable<float> minDaughterPt{"minDaughterPt", 0.15f, "Minimum daughter pT"};
  Configurable<float> maxDaughterEta{"maxDaughterEta", 0.8f, "Maximum daughter absolute eta"};
  Configurable<int> minTPCCrossedRows{"minTPCCrossedRows", 70, "Minimum TPC crossed rows"};

  // Topology
  Configurable<float> maxDcaV0Daughters{"maxDcaV0Daughters", 1.f, "Maximum daughter DCA"};
  Configurable<float> maxDcaV0ToPV{"maxDcaV0ToPV", 0.1f, "Maximum V0 DCA to PV"};
  Configurable<float> minDcaProtonToPV{"minDcaProtonToPV", 0.02f, "Minimum proton DCA to PV"};
  Configurable<float> minDcaPionToPV{"minDcaPionToPV", 0.06f, "Minimum pion DCA to PV"};
  Configurable<float> minV0Radius{"minV0Radius", 0.5f, "Minimum V0 radius"};
  Configurable<float> maxV0CTau{"maxV0CTau", 30.f, "Maximum Lambda c-tau"};
  Configurable<float> minV0CosPA{"minV0CosPA", 0.995f, "Minimum cosine of pointing angle"};
  ...
  void processLambdaCutFlow(aod::McRecoLambdaCandidates const& candidates)
  {
    for (const auto& candidate : candidates) {
      ...
      if (std::abs(candidate.vertexZ()) >= cutZVertex) {
        continue;
      }
      fillStage(2.f);

      if (!candidate.isPhysicalPrimary()) {
        continue;
      }
      fillStage(3.f);
      ...
      if (candidate.proPt() <= minDaughterPt || candidate.piePt() <= minDaughterPt ||
          ...
          std::abs(candidate.pieEta()) >= maxDaughterEta) {
        continue;
      }
      fillStage(4.f);

      if (candidate.proTPCCrossedRows() < minTPCCrossedRows ||
          candidate.pieTPCCrossedRows() < minTPCCrossedRows) {
        continue;
      }
      fillStage(5.f);
      ...
      if (candidate.dcaDaughters() >= maxDcaV0Daughters || std::abs(candidate.dcaV0ToPV()) >= maxDcaV0ToPV ||
          ...
          candidate.cosPA() <= minV0CosPA) {
        continue;
      }
      fillStage(6.f);

      if (candidate.recoPt() <= minLambdaPt || candidate.recoPt() >= maxLambdaPt ||
          std::abs(candidate.recoRapidity()) >= maxLambdaRapidity) {
        continue;
      }
      fillStage(7.f);
    }
...
struct LambdaAntiLambdaPairAnalysis {
  ...
  Configurable<float> sameEventShortRangePairMaxDeltaPhi{"sameEventShortRangePairMaxDeltaPhi", 1.047f, "ShortRangePai Maximum absolute wrapped phi difference for same-event pairs"}; // Pi/3 =1.047

  void addHistograms(TString const& candidate1, TString const& candidate2)
  {
    //~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~Same-Event~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
    // EventConter histogram
    rSpinAnalysis.add(candidate1 + candidate2 + "/hSameEvent/EventCount", "Same-event" + candidate1 + candidate2 +
                                                                            "pairs;"
                                                                            "SameEvent",
    ...
    rSpinAnalysis.add(candidate1 + candidate2 + "/hSameEvent/DeltaEtaDelatPhi", "Same-event" + candidate1 + candidate2 +
                                                                                  "pairs;"
                                                                                  "#Delta#eta;"
                                                                                  "#Delta#phi",
                      HistType::kTH2F, {axisDeltaEta, axisDeltaPhi});
    rSpinAnalysis.add(candidate1 + candidate2 + "/hSameEvent/DeltaPtDelatEta", "Same-event" + candidate1 + candidate2 +
                                                                                 "pairs;"
                                                                                 "#Delta#p_{T};"
                                                                                 "#Delta#eta",
                      HistType::kTH2F, {axisDeltaPt, axisDeltaEta});
    rSpinAnalysis.add(candidate1 + candidate2 + "/hSameEvent/DeltaPtDelatPhi", "Same-event" + candidate1 + candidate2 +
                                                                                 "pairs;"
                                                                                 "#Delta#p_{T};"
                                                                                 "#Delta#phi",
                      HistType::kTH2F, {axisDeltaPt, axisDeltaPhi});
    // Invarient mass and Cent histos
    rSpinAnalysis.add(candidate1 + candidate2 + "/hSameEvent/DeltaMassDeltaCent", "Same-event" + candidate1 + candidate2 +
                                                                                    "pairs;"
                                                                                    "#Delta m_{#pi#p} ;"
                                                                                    "DeltaCent",
                      HistType::kTH2F, {axisDeltaMass, axisDeltaCent});
    rSpinAnalysis.add(candidate1 + candidate2 + "/hSameEvent/DeltaMassDeltaPt", "Same-event" + candidate1 + candidate2 +
                                                                                  "pairs;"
                                                                                  "#Delta m_{#pi#p} ;"
                                                                                  "Deltap_{T}",
                      HistType::kTH2F, {axisDeltaMass, axisDeltaPt});
    rSpinAnalysis.add(candidate1 + candidate2 + "/hSameEvent/DeltaCentDeltaMult", "Same-event" + candidate1 + candidate2 +
                                                                                    "pairs;"
                                                                                    "DeltaCent ;"
                                                                                    "DeltaMult",
                      HistType::kTH2F, {axisDeltaCent, axisDeltaMult});
    rSpinAnalysis.add(candidate1 + candidate2 + "/hSameEvent/Mass", "Same-event" + candidate1 + candidate2 +
                                                                      "pairs;"
                                                                      "Mass-Lambda ;"
                                                                      "Mass-AntiLambda",
                      HistType::kTH2F, {axisInvariantMass, axisInvariantMass});

    // ================================================================
    // Same-event short-range spin histograms
    // |Delta y| < 0.5 and |Delta phi| < pi/3
    // ================================================================

    rSpinAnalysis.add(candidate1 + candidate2 + "/hSameEvent/ShortRange/CosMult", "Same-event short-range " + candidate1 + candidate2 + " pairs;cos#theta;Mult", HistType::kTH2F, {axisCos, axisMult});
    rSpinAnalysis.add(candidate1 + candidate2 + "/hSameEvent/ShortRange/CosCent", "Same-event short-range " + candidate1 + candidate2 + " pairs;cos#theta;Cent", HistType::kTH2F, {axisCos, axisCent});
    rSpinAnalysis.add(candidate1 + candidate2 + "/hSameEvent/ShortRange/Cos", "Same-event short-range " + candidate1 + candidate2 + " pairs;cos#theta", HistType::kTH1F, {axisCos});
    rSpinAnalysis.add(candidate1 + candidate2 + "/hSameEvent/ShortRange/NumberOfPairs", "Same-event short-range pair count;Counter;Accepted pairs", HistType::kTH1F, {{1, -0.5, 0.5}});
    rSpinAnalysis.add(candidate1 + candidate2 + "/hSameEvent/ShortRange/EventCount", "Events containing at least one short-range pair;Counter;Events", HistType::kTH1F, {{1, -0.5, 0.5}});

    // ================================================================
    // Same-event long-range spin histograms
    // Complement of the short-range selection
    // ================================================================

    rSpinAnalysis.add(candidate1 + candidate2 + "/hSameEvent/LongRange/CosMult", "Same-event long-range " + candidate1 + candidate2 + " pairs;cos#theta;Mult", HistType::kTH2F, {axisCos, axisMult});
    rSpinAnalysis.add(candidate1 + candidate2 + "/hSameEvent/LongRange/CosCent", "Same-event long-range " + candidate1 + candidate2 + " pairs;cos#theta;Cent", HistType::kTH2F, {axisCos, axisCent});
    rSpinAnalysis.add(candidate1 + candidate2 + "/hSameEvent/LongRange/Cos", "Same-event long-range " + candidate1 + candidate2 + " pairs;cos#theta", HistType::kTH1F, {axisCos});
    rSpinAnalysis.add(candidate1 + candidate2 + "/hSameEvent/LongRange/NumberOfPairs", "Same-event long-range pair count;Counter;Accepted pairs", HistType::kTH1F, {{1, -0.5, 0.5}});
    rSpinAnalysis.add(candidate1 + candidate2 + "/hSameEvent/LongRange/EventCount", "Events containing at least one long-range pair;Counter;Events", HistType::kTH1F, {{1, -0.5, 0.5}});

    //~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~Mixed-Event~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
    // EventConter histogram
    rSpinAnalysis.add(candidate1 + candidate2 + "/hMixedEvent/EventCount", "Mixed-event" + candidate1 + candidate2 +
                                                                             "pairs;"
                                                                             "MixedEvent",
                      HistType::kTH1F, {{1, -0.5, 0.5}});
    ...
    rSpinAnalysis.add(candidate1 + candidate2 + "/hMixedEvent/DeltaEtaDelatPhi", "Mixed-event" + candidate1 + candidate2 +
                                                                                   ...
                                                                                   "#Delta#phi",
                      HistType::kTH2F, {axisDeltaEta, axisDeltaPhi});
    rSpinAnalysis.add(candidate1 + candidate2 + "/hMixedEvent/DeltaPtDelatEta", "Mixed-event" + candidate1 + candidate2 +
                                                                                  "pairs;"
                                                                                  "#Delta#p_{T};"
                                                                                  "#Delta#eta",
                      HistType::kTH2F, {axisDeltaPt, axisDeltaEta});
    rSpinAnalysis.add(candidate1 + candidate2 + "/hMixedEvent/DeltaPtDelatPhi", "Mixed-event" + candidate1 + candidate2 +
                                                                                  "pairs;"
                                                                                  "#Delta#p_{T};"
                                                                                  "#Delta#phi",
                      HistType::kTH2F, {axisDeltaPt, axisDeltaPhi});
    // Invarient mass and Cent histos
    rSpinAnalysis.add(candidate1 + candidate2 + "/hMixedEvent/DeltaMassDeltaCent", "Mixed-event" + candidate1 + candidate2 +
                                                                                     "pairs;"
                                                                                     "#Delta m_{#pi#p} ;"
                                                                                     "DeltaCent",
                      HistType::kTH2F, {axisDeltaMass, axisDeltaCent});
    rSpinAnalysis.add(candidate1 + candidate2 + "/hMixedEvent/DeltaMassDeltaPt", "Mixed-event" + candidate1 + candidate2 +
                                                                                   "pairs;"
                                                                                   "#Delta m_{#pi#p} ;"
                                                                                   "Deltap_{T}",
                      HistType::kTH2F, {axisDeltaMass, axisDeltaPt});
    rSpinAnalysis.add(candidate1 + candidate2 + "/hMixedEvent/DeltaCentDeltaMult", "Mixed-event" + candidate1 + candidate2 +
                                                                                     "pairs;"
                                                                                     "DeltaCent ;"
                                                                                     "DeltaMult",
                      HistType::kTH2F, {axisDeltaCent, axisDeltaMult});
    rSpinAnalysis.add(candidate1 + candidate2 + "/hMixedEvent/Mass", "Mixed-event" + candidate1 + candidate2 +
                                                                       "pairs;"
                                                                       "Mass-Lambda ;"
                                                                       "Mass-AntiLambda",
                      HistType::kTH2F, {axisInvariantMass, axisInvariantMass});
    ...
    // Spin histos
    // 2d
    rSpinAnalysis.add(candidate1 + candidate2 + "/hMixedEvent/CosMult", "Mixed-event" + candidate1 + candidate2 +
                                                                          "pairs;"
                                                                          "cos#theta;"
                                                                          "Mult" +
                                                                          candidate1 +
  ...
  template <typename Candidate1, typename Candidate2>
  bool isSameEventShortRange(Candidate1 const& candidate1, Candidate2 const& candidate2)
  {
    const float deltaRapidity = std::abs(candidate1.rapidity() - candidate2.rapidity());
  ...
  void fillLambdaAntiLambdaSameEvent(LambdaSlice const& lambdas, AntiLambdaSlice const& antiLambdas)
  {
    ...
    bool hasShortRangePair = false;
    bool hasLongRangePair = false;
    ...
    for (auto const& [lambda, antiLambda] : combinations(CombinationsFullIndexPolicy(lambdas, antiLambdas))) {
      ...
      if (isShortRange) {
        ...
        rSpinAnalysis.fill(HIST("LambdaAntiLambda/hSameEvent/ShortRange/NumberOfPairs"), 0);
        ...
        rSpinAnalysis.fill(HIST("LambdaAntiLambda/hSameEvent/LongRange/Cos"), cosDeltaThetaStar);
        rSpinAnalysis.fill(HIST("LambdaAntiLambda/hSameEvent/LongRange/CosMult"), cosDeltaThetaStar, lambda.multiplicity());
        rSpinAnalysis.fill(HIST("LambdaAntiLambda/hSameEvent/LongRange/CosCent"), cosDeltaThetaStar, lambda.centrality());
      }
    }
    if (hasShortRangePair) {
      rSpinAnalysis.fill(HIST("LambdaAntiLambda/hSameEvent/ShortRange/EventCount"), 0);
    }
    if (hasLongRangePair) {
      rSpinAnalysis.fill(HIST("LambdaAntiLambda/hSameEvent/LongRange/EventCount"), 0);
    }
  }
  ...
  void fillLambdaLambdaSameEvent(LambdaSlice const& lambdas)
  {
    ...
    for (auto const& [lambda1, lambda2] : combinations(CombinationsStrictlyUpperIndexPolicy(lambdas, lambdas))) {
      ...
      if (isShortRange) {
        ...
        rSpinAnalysis.fill(HIST("LambdaLambda/hSameEvent/ShortRange/NumberOfPairs"), 0);
        ...
        rSpinAnalysis.fill(HIST("LambdaLambda/hSameEvent/LongRange/Cos"), cosDeltaThetaStar);
        rSpinAnalysis.fill(HIST("LambdaLambda/hSameEvent/LongRange/CosMult"), cosDeltaThetaStar, lambda1.multiplicity());
        rSpinAnalysis.fill(HIST("LambdaLambda/hSameEvent/LongRange/CosCent"), cosDeltaThetaStar, lambda1.centrality());
      }
    }
    if (hasShortRangePair) {
      rSpinAnalysis.fill(HIST("LambdaLambda/hSameEvent/ShortRange/EventCount"), 0);
    }
    if (hasLongRangePair) {
      rSpinAnalysis.fill(HIST("LambdaLambda/hSameEvent/LongRange/EventCount"), 0);
    }
  }
  ...
  void fillAntiLambdaAntiLambdaSameEvent(AntiLambdaSlice const& antiLambdas)
  {
    ...
    for (auto const& [antilambda1, antilambda2] : combinations(CombinationsStrictlyUpperIndexPolicy(antiLambdas, antiLambdas))) {
      ...
      if (isShortRange) {
        ...
        rSpinAnalysis.fill(HIST("AntiLambdaAntiLambda/hSameEvent/ShortRange/NumberOfPairs"), 0);
        ...
        rSpinAnalysis.fill(HIST("AntiLambdaAntiLambda/hSameEvent/LongRange/Cos"), cosDeltaThetaStar);
        rSpinAnalysis.fill(HIST("AntiLambdaAntiLambda/hSameEvent/LongRange/CosMult"), cosDeltaThetaStar, antilambda1.multiplicity());
        rSpinAnalysis.fill(HIST("AntiLambdaAntiLambda/hSameEvent/LongRange/CosCent"), cosDeltaThetaStar, antilambda1.centrality());
      }
    }
    if (hasShortRangePair) {
      rSpinAnalysis.fill(HIST("AntiLambdaAntiLambda/hSameEvent/ShortRange/EventCount"), 0);
    }
    if (hasLongRangePair) {
      rSpinAnalysis.fill(HIST("AntiLambdaAntiLambda/hSameEvent/LongRange/EventCount"), 0);
    }
  }
  ...
  using CollisionsWithMultiplicity = soa::Join<aod::Collisions, aod::Mults>;
...
{
  return WorkflowSpec{
    adaptAnalysisTask<LambdaOrAntiLambdaProducerWithSpin>(cfgc),
