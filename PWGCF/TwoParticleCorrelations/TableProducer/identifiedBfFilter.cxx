...
struct IdentifiedBfFilterTracks {
  ...
  template<typename TrackObject>
  float calcBeta(TrackObject const& track);
  template<typename TrackObject>
...
template <typename TrackObject>
inline float IdentifiedBfFilterTracks::calcBeta(TrackObject const& track)
{
  //LOGF(info,"Top of calcBeta");
  float beta;
  if(track.hasTOF()){
    if constexpr (framework::has_type_v<aod::pidtofsignal::TOFSignal, typename TrackObject::all_columns>){

      if((track.tofSignal()) > 0){
        float tmpLength = track.length()/1000.; //Convert from mm to m
        float tmpTime = (track.tofSignal())/1000000000.; //Convert from ns to s
        beta = tmpLength/tmpTime;
        return(beta/TMath::C());
      }
    }
  }
...
inline MatchRecoGenSpecies IdentifiedBfFilterTracks::identifyTrack(TrackObject const& track)
{
  ...
  if (minNSigma < acceptRange[spMinNSigma][1] && minNSigma > acceptRange[spMinNSigma][0]) {           // Check that current nsigma is in accpetance range
    ...
    if (doublematch) { // if double match true
      ...
      if constexpr (framework::has_type_v<aod::pidtofsignal::TOFSignal, typename TrackObject::all_columns>){
        fhTrackTimeA[kIdBfNoOfSpecies]->Fill(track.p(), (track.tofSignal()));
        fhTrackTimeIPA[kIdBfNoOfSpecies]->Fill(track.tpcInnerParam(), (track.tofSignal()));
      }
  ...
  if constexpr (framework::has_type_v<aod::mctracklabel::McParticleId, typename TrackObject::all_columns>) {
    ...
    if(isPrimary(track.template mcParticle_as<aod::McParticles>())){
      ...
      if(track.hasTOF() && track.p() > tofCut[0]){
        ...
        if constexpr (framework::has_type_v<aod::pidtofsignal::TOFSignal, typename TrackObject::all_columns>){
          fhPrimaryTrackTimeB->Fill(track.p(), (track.tofSignal()));
          fhPrimaryTrackTimeIPB->Fill(track.tpcInnerParam(), (track.tofSignal()));
        }
        if constexpr (framework::has_type_v<aod::pidtofbeta::Beta, typename TrackObject::all_columns>){
  ...
  if constexpr (framework::has_type_v<aod::mctracklabel::McParticleId, typename TrackObject::all_columns>) {
    ...
    if(!(realPID < 0)){
      ...
      if(track.hasTOF() && track.p() > tofCut[realPID]){
        ...
        if constexpr (framework::has_type_v<aod::pidtofsignal::TOFSignal, typename TrackObject::all_columns>){
          fhTrueTrackTimeA[realPID]->Fill(track.p(), (track.tofSignal()));
          fhTrueTrackTimeIPA[realPID]->Fill(track.tpcInnerParam(), (track.tofSignal()));
        }
        if constexpr (framework::has_type_v<aod::pidtofbeta::Beta, typename TrackObject::all_columns>){
    ...
    if(isPrimary(track.template mcParticle_as<aod::McParticles>())){
      ...
      if(track.hasTOF() && track.p() > tofCut[sp]){
        ...
        if constexpr (framework::has_type_v<aod::pidtofsignal::TOFSignal, typename TrackObject::all_columns>){
          fhPrimaryTrackTimeA[sp]->Fill(track.p(), (track.tofSignal()));
          fhPrimaryTrackTimeIPA[sp]->Fill(track.tpcInnerParam(), (track.tofSignal()));
        }
        if constexpr (framework::has_type_v<aod::pidtofbeta::Beta, typename TrackObject::all_columns>){
      ...
      if(sp == realPID){
        ...
        if(track.hasTOF() && track.p() > tofCut[realPID]){
          ...
          if constexpr (framework::has_type_v<aod::pidtofsignal::TOFSignal, typename TrackObject::all_columns>){
            fhPureTrackTimeA[realPID]->Fill(track.p(), (track.tofSignal()));
            fhPureTrackTimeIPA[realPID]->Fill(track.tpcInnerParam(), (track.tofSignal()));
          }
          if constexpr (framework::has_type_v<aod::pidtofbeta::Beta, typename TrackObject::all_columns>){
...
void IdentifiedBfFilterTracks::fillTrackHistosAfterSelection(TrackObject const& track, MatchRecoGenSpecies sp)
{
  ...
  if(track.hasTOF() && track.p() > tofCut[sp]){
    ...
    if constexpr (framework::has_type_v<aod::pidtofsignal::TOFSignal, typename TrackObject::all_columns>){
      fhTrackTimeA[sp]->Fill(track.p(), (track.tofSignal()));
      fhTrackTimeIPA[sp]->Fill(track.tpcInnerParam(), (track.tofSignal()));
    }
    if constexpr (framework::has_type_v<aod::pidtofbeta::Beta, typename TrackObject::all_columns>){
