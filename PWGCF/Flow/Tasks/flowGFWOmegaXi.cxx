...
struct FlowGFWOmegaXi {
  ...
  O2_DEFINE_CONFIGURABLE(cfgCutPtPOIMin, float, 0.2f,
                         "Minimal pT for poi tracks")
  O2_DEFINE_CONFIGURABLE(cfgCutPtPOIMax, float, 10.0f,
                         "Maximal pT for poi tracks")
  O2_DEFINE_CONFIGURABLE(cfgCutPtMin, float, 0.2f, "Minimal pT for ref tracks")
  O2_DEFINE_CONFIGURABLE(cfgCutPtMax, float, 3.0f, "Maximal pT for ref tracks")
  O2_DEFINE_CONFIGURABLE(cfgCutEta, float, 0.8f, "Eta range for tracks")
  O2_DEFINE_CONFIGURABLE(cfgCutChi2prTPCcls, float, 2.5,
  ...
  O2_DEFINE_CONFIGURABLE(cfgAcceptance, std::string, "",
  ...
  Filter trackFilter = (nabs(aod::track::eta) < cfgCutEta) &&
                       (aod::track::pt > cfgCutPtPOIMin) &&
                       (aod::track::pt < cfgCutPtPOIMax) &&
  ...
  void init(InitContext const &) // Initialization
  {
    ...
    fMultPVCutHigh = new TF1("fMultPVCutHigh",
                             ...
                             "3.5*([5]+[6]*x+[7]*x*x+[8]*x*x*x+[9]*x*x*x*x)",
                             0, 100);
    fMultPVCutHigh->SetParameters(3257.29, -121.848, 1.98492, -0.0172128,
                                  6.47528e-05, 154.756, -1.86072, -0.0274713,
                                  0.000633499, -3.37757e-06);
    fMultCutLow = new TF1("fMultCutLow",
                          "[0]+[1]*x+[2]*x*x+[3]*x*x*x - "
                          "2.*([4]+[5]*x+[6]*x*x+[7]*x*x*x+[8]*x*x*x*x)",
                          0, 100);
    fMultCutLow->SetParameters(1654.46, -47.2379, 0.449833, -0.0014125, 150.773,
                               -3.67334, 0.0530503, -0.000614061, 3.15956e-06);
    fMultCutHigh = new TF1("fMultCutHigh",
                           "[0]+[1]*x+[2]*x*x+[3]*x*x*x + "
                           "3.*([4]+[5]*x+[6]*x*x+[7]*x*x*x+[8]*x*x*x*x)",
                           0, 100);
    fMultCutHigh->SetParameters(1654.46, -47.2379, 0.449833, -0.0014125,
                                150.773, -3.67334, 0.0530503, -0.000614061,
                                3.15956e-06);
    ...
    for (int massbin = 1; massbin <= nMassBins; massbin++) {
      ...
      if (TMath::Abs(val) < 1) {
        registry.fill(tarName, fPtAxis->GetBinCenter(ptbin),
                      fMass->GetBinCenter(massbin), val, dnx);
      }
    }
    return;
  }
  ...
  bool eventSelected(TCollision collision, const int multTrk,
    ...
    // V0A T0A 5 sigma cut
    if (abs(collision.multFV0A() - fT0AV0AMean->Eval(collision.multFT0A())) >
        5 * fT0AV0ASigma->Eval(collision.multFT0A()))
      return 0;
  ...
  void process(aodCollisions::iterator const &collision,
    ...
    for (auto &track : tracks) {
      ...
      if ((track.pt() > cfgCutPtMin) && (track.pt() < cfgCutPtMax)) {
        fGFW->Fill(track.eta(), ptbin, track.phi(), wacc * weff,
                   1); //(eta, ptbin, phi, wacc*weff, bitmask)
      }
    }
    ...
    for (auto &casc : Cascades) {
      ...
      // check TPC
      if (cfgcheckDauTPC &&
          (!posdau.hasTPC() || !negdau.hasTPC() || !bachelor.hasTPC())) {
        continue;
      }
      if (TMath::Abs(casc.yOmega()) < cfgCasc_rapidity &&
          TMath::Abs(bachelor.tpcNSigmaKa()) < cfgNSigmaCascKaon &&
          TMath::Abs(posdau.tpcNSigmaPr()) < cfgNSigmaCascProton &&
          TMath::Abs(negdau.tpcNSigmaPi()) < cfgNSigmaCascPion) {
        registry.fill(HIST("InvMassOmegaMinus"), casc.pt(), casc.mOmega(),
                      casc.eta());
        if ((casc.pt() < cfgCutPtPOIMax) && (casc.pt() > cfgCutPtPOIMin) &&
      ...
      if (TMath::Abs(casc.yXi()) < cfgCasc_rapidity &&
          TMath::Abs(bachelor.tpcNSigmaKa()) < cfgNSigmaCascKaon &&
          TMath::Abs(posdau.tpcNSigmaPr()) < cfgNSigmaCascProton &&
          TMath::Abs(negdau.tpcNSigmaPi()) < cfgNSigmaCascPion) {
        registry.fill(HIST("InvMassXiMinus"), casc.pt(), casc.mXi(),
                      casc.eta());
        if ((casc.pt() < cfgCutPtPOIMax) && (casc.pt() > cfgCutPtPOIMin) &&
