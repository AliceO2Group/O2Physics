...
bool DoesV0LegMatchWithCluster(TV0Leg const& v0leg, TCluster const& cluster, const float max_deta, const float max_dphi)
{
  ...
  return (pow(deta / max_deta, 2) + pow(dphi / max_dphi, 2) < 1) & (abs(Ep - 1) < 0.5);
}
//_______________________________________________________________________
} // namespace photonpair
} // namespace o2::aod
