...
struct PhotonTableProducer {
  Produces<o2::aod::EMReducedEvents> event;
  Produces<o2::aod::V0Photons> v0photon;
  Produces<o2::aod::V0Legs> v0leg;
  // Produces<o2::aod::PHOSPhotons> phosphoton;
  // Produces<o2::aod::EMCPhotons> emcphoton;
  ...
  void process(MyFilteredTracks const& tracks, soa::Join<aod::Collisions, aod::EvSels> const& collisions, aod::BCsWithTimestamps const&)
  {
    ...
    for (auto& collision : collisions) {
      ...
      event(collision.globalIndex(), tag, bc.runNumber(), collision.sel8(), collision.posX(), collision.posY(), collision.posZ(), collision.numContrib(), collision.collisionTime(), collision.collisionTimeRes());
      ...
      for (auto& [ele, pos] : combinations(CombinationsFullIndexPolicy(groupEle, groupPos))) {
        ...
        v0photon(collision.globalIndex(), event.lastIndex(), v0leg.lastIndex() + 1, v0leg.lastIndex() + 2,
                 svpos[0], svpos[1], svpos[2],
                 pvec0[0], pvec0[1], pvec0[2],
                 pvec1[0], pvec1[1], pvec1[2], v0CosinePA, v0dca); // if v0leg is empty, lastIndex = -1.
        v0leg(collision.globalIndex(), event.lastIndex(),
              pos.globalIndex(), pos.sign(), pos.isAmbiguousTrack(),
              // pos.numBC(), pos.ambiguousCollisionIndicesIds().size(),
        ...
        v0leg(collision.globalIndex(), event.lastIndex(),
              ...
              // ele.numBC(), ele.ambiguousCollisionIndicesIds().size(),
...
WorkflowSpec defineDataProcessing(ConfigContext const& cfgc)
{
  return WorkflowSpec{
    ...
    adaptAnalysisTask<PhotonTableProducer>(cfgc, TaskName{"produce-photon-table"}),
  };
}
