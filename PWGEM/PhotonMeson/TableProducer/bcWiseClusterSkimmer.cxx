...
struct bcWiseClusterSkimmer {
  ...
  void processEventProperties(const auto& bc, const auto& collisionsInBC, const auto& cellsInBC, const bool hasMCCollision = false)
  {
    ...
    mHistManager.fill(HIST("BCHasFT0FoundFT0"), (float)FT0ID, (float)foundFT0ID);
    mHistManager.fill(HIST("BCFoundFT0Diff"), (float)foundFT0ID - (float)FT0ID);

    auto bcIDInOrbit = bc.globalBC() % 3564;

    if (hasMCCollision) {
      mHistManager.fill(HIST("BCIDOfMCColl"), (float)bcIDInOrbit);
      if (!hasFoundFT0) {
        mHistManager.fill(HIST("BCIDOfMCCollButNoFT0"), (float)bcIDInOrbit);
      }
    }
