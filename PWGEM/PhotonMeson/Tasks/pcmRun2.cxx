...
struct PcmRun2 {
  ...
  bool checkLegs(V0Iter const& v0)
  {
    ...
    if (pidcuts.doPionRejection.value && posLeg.p() > 0.4) {
      if (posLeg.p() < 3.5 && posLeg.tpcNSigmaPi() <= pidcuts.minNSigmaPiLowP.value) {
        registry.fill(HIST("hCuts"), 2);
        return false;
      } else if (posLeg.p() >= 3.5 && posLeg.tpcNSigmaPi() <= pidcuts.minNSigmaPiHighP.value) {
        registry.fill(HIST("hCuts"), 2);
        return false;
      }
    }
    ...
    if (pidcuts.doPionRejection.value && negLeg.p() > 0.4) {
      if (negLeg.p() < 3.5 && negLeg.tpcNSigmaPi() <= pidcuts.minNSigmaPiLowP.value) {
        registry.fill(HIST("hCuts"), 2);
        return false;
      } else if (negLeg.p() >= 3.5 && negLeg.tpcNSigmaPi() <= pidcuts.minNSigmaPiHighP.value) {
        registry.fill(HIST("hCuts"), 2);
        return false;
      }
    }
  ...
  void process(o2::aod::Collisions const& collisions, o2::aod::Run2OTFV0s const& v0s, TracksWithPID const& /*tracks*/)
  {
    ...
    for (const auto& collision : collisions) {
      ...
      for (const auto& [g1, g2] : combinations(CombinationsStrictlyUpperIndexPolicy(photonsPerCollision, photonsPerCollision))) {
        ...
        if (cR1 < 5.f || (cR1 > 55.f && cR1 < 72.f) || cR1 > 180.f) {
        ...
        if (cR2 < 5.f || (cR2 > 55.f && cR2 < 72.f) || cR2 > 280.f) {
          registry.fill(HIST("hCutsPair"), 2);
          continue;
        }
