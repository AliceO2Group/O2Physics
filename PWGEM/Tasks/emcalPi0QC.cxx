...
/// \brief Class for photon/cluster objects
struct Photon {
  /// \brief Constructor taking eta, phi and energy of the cluster as well as cluster id
  Photon(float eta_tmp, float phi_tmp, float energy_tmp, int clusid = 0)
  {
    eta = eta_tmp;
    phi = phi_tmp;
    energy = energy_tmp;
    // transform eta and phi to px, py and pz
...
/// \brief Class for meson objects
struct Meson {
  /// \brief Constructor taking two photons
...
struct Pi0QCTask {
  ...
  /// \brief Process EMCAL clusters that are matched to a collisions
  void ProcessClusters(collisionEvSelIt const& theCollision, selectedClusters const& clusters, o2::aod::BCs const& bcs)
  {
    // clear photon vector
    mPhotons.clear();
  ...
  void ProcessMesons(collisionEvSelIt const& theCollision, selectedClusters const& clusters, o2::aod::BCs const& bcs)
  {
...
{
  return WorkflowSpec{
    adaptAnalysisTask<Pi0QCTask>(cfgc)};
}