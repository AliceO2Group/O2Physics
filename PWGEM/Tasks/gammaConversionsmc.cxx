...
struct GammaConversionsmc {
  ...
  std::string fFullNameIsPhotonSelectedHisto{fPrefixReconstructedInfoHistos + "afterCuts/v0/IsPhotonSelected"};
  ...
  // resolution histos
  std::vector<MyHistogramSpec> fV0ResolutionHistoDefinitions{ // only in mc
    ...
    // resolution histos
  std::vector<MyHistogramSpec> fV0MCInfoOnlyHistoDefinitions{ // only in mc
    {"hTruePhotonPt", {HistType::kTH1F, {{800, -0.0f,  25.f}}}},
  };
  ...
  void init(InitContext const&)
  {
    // reconstructed data histograms
    {
      for (auto bac : std::vector<std::string>{"beforeCuts/", "afterCuts/"}){
        std::string lPath(fPrefixReconstructedInfoHistos + bac);
        
        // track histograms
        for (auto& tHisto : fTrackHistoDefinitions){ 
          std::string lFullName(lPath + "track/" + tHisto.name);
        ...
        for (auto tHisto : fV0HistoDefinitions){ 
          ...
          std::string lFullName(lPath + "v0/" + tHisto.name);
    ...
    { 
      for (auto bac : std::vector<std::string>{"beforeCuts/", "afterCuts/"}){
        std::string lPath(fPrefixMCInfoNeededHistos + bac);
        
        // v0 Info only histos
        for (auto tHisto : fV0MCInfoOnlyHistoDefinitions){ 
          std::string lFullName(lPath + "MCinformationOnly/" + tHisto.name);
        ...
        for (auto tHisto : fV0ResolutionHistoDefinitions){ 
          std::string lFullName(lPath + "v0Resolutions/" + tHisto.name);
  ...
  bool processPhoton(TCOLL const& theCollision, const TV0& theV0, const TTRACK& theTrackPos, const TTRACK& theTrackNeg)
  {
   ...
   fillHistograms(fPrefixReconstructedInfoHistos + "beforeCuts/", 
                  theV0, 
                  theTrackPos, 
                  theTrackNeg, 
                  lV0CosinePA);
    ...
    fillHistograms(fPrefixReconstructedInfoHistos + "afterCuts/", 
                   theV0, 
                   theTrackPos, 
                   theTrackNeg, 
                   lV0CosinePA);
  ...
  void processTruePhoton(std::string thePath, const TV0& theV0, const TTRACK& theTrackPos, const TTRACK& theTrackNeg, const TMC& theMcParticles)
  {
    ...
    */
    ...
    {
      if (lMothers.size()==2){
        if (lMothers[0]==lMothers[1]) {LOGF(info, "size2: 01");lSame = 1;}
      }
      
      if (lMothers.size()==3){
        if (lMothers[0]==lMothers[1]) {LOGF(info, "size3: 01");lSame = 2;}
        if (lMothers[0]==lMothers[2]) {LOGF(info, "size2: 02");lSame = 3;}
        if (lMothers[1]==lMothers[2]) {LOGF(info, "size2: 12");lSame = 4;}
      }
      
      if (lMothers.size()==4){
        if (lMothers[0]==lMothers[2]) {LOGF(info, "size4 02");lSame = 4;}
        if (lMothers[1]==lMothers[3]) {LOGF(info, "size4 13");lSame = 5;}
        if (lMothers[0]==lMothers[3]) {LOGF(info, "size4 03");lSame = 6;}
        if (lMothers[1]==lMothers[2]) {LOGF(info, "size4 12");lSame = 7;}
        if (lMothers[0]==lMothers[1]) {LOGF(info, "size4 01");lSame = 8;}
        if (lMothers[2]==lMothers[3]) {LOGF(info, "size4 23");lSame = 9;}
      }
    }
    ...
    if (lSame) {
      ...
      for (auto& lMother : lMcNeg.template mothers_as<aod::McParticles_001>()) {
        ...
        if (lMother.pdgCode() == 22) {
          ...
          fillTH1(fV0MCInfoOnlyHistos, thePath + "MCinformationOnly/hTruePhotonPt", lMother.pt());
          
          fillTH1(fV0ResolutionHistos, thePath + "v0Resolutions/hPtRes",  theV0.pt() - lMother.pt());
          fillTH1(fV0ResolutionHistos, thePath + "v0Resolutions/hEtaRes",  theV0.eta() - lMother.eta());
          fillTH1(fV0ResolutionHistos, thePath + "v0Resolutions/hPhiRes",  theV0.phi() - lMother.phi());
          fillTH1(fV0ResolutionHistos, thePath + "v0Resolutions/hConvPointRRes",  lConvPointRec.Perp() - lConvPointMC.Perp());
          fillTH1(fV0ResolutionHistos, thePath + "v0Resolutions/hConvPointAbsoluteDistanceRes", TVector3(lConvPointRec - lConvPointMC).Mag());
        }
      }
    }
  }
  ...
  {     
    for (auto& lV0 : theV0s) {
      ...
      processTruePhoton(fPrefixMCInfoNeededHistos + "beforeCuts/", 
                        lV0, 
                        lTrackPos, 
                        lTrackNeg, 
                        theMcParticles);
      ...
      processTruePhoton(fPrefixMCInfoNeededHistos + "afterCuts/",
                        lV0, 
                        lTrackPos, 
                        lTrackNeg, 
                        theMcParticles);
  ...
  void fillHistograms(std::string theDMC, const TV0& theV0, const TTRACK& theTrackPos, const TTRACK& theTrackNeg, float theV0CosinePA)
  {
    fillTrackHistograms(theDMC + "track/", theTrackPos, theTrackNeg);
    fillV0Histograms(theDMC + "v0/", theV0, theV0CosinePA);
    
    if (theDMC == fPrefixReconstructedInfoHistos + "beforeCuts/"){
      fillTH1(fV0Histos, fFullNameIsPhotonSelectedHisto, getPhotonCutIndex("kPhotonIn"));
    } else if (theDMC == fPrefixReconstructedInfoHistos + "afterCuts/"){
