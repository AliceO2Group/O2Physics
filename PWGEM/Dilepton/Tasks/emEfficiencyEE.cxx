	...
	if ((*sig).CheckSignal(true, groupedMCTracks, mctrack)) {
	  if(mctrack.pdgCode() > 0) {
	    ...
	    dynamic_cast<TH3D*>(fHistGenPosPart.at(isig))->Fill(mctrack.pt(), mctrack.eta(), mctrack.phi());
          ...
          if (fConfigQA) fHistManQA->FillHistClass(Form("MCTruthGen_%s", (*sig).GetName()), VarManager::fgValues);
        }
      }
    }
  }
	...
	for (unsigned int j = 0; j < fTrackCuts.size(); j++) {
	  if (filterMap & (uint8_t(1) << j)) {
		...
		mcpdg = mctrack.pdgCode();
	      ...
	      if(mcpt>0.) deltaptoverpt = (mcpt-track.pt())/mcpt;
	    ...
	    if(fConfigQA) fHistManQA->FillHistClass(fHistNamesMCMatchedQA[j][i].Data(), VarManager::fgValues);
	  }
	} // end loop over cuts
...
struct AnalysisSameEventPairing {
  ...
  void runMCGenPair(TTracksMC const& groupedMCTracks)
  {
    ...
    for (auto& [t1, t2] : combinations(groupedMCTracks, groupedMCTracks)) {
      ...
      if((abs(t1.pdgCode()) != 11) || (abs(t2.pdgCode()) != 11)) continue;
      if(t1.pdgCode()*t2.pdgCode() > 0) continue; // ULS only
      ...
      if((t1.eta()> fConfigMaxEta) || (t2.eta() > fConfigMaxEta) || (t1.eta() < fConfigMinEta) || (t2.eta() < fConfigMinEta) || (t1.pt()> fConfigMaxPt) || (t2.pt() > fConfigMaxPt) || (t1.pt() < fConfigMinPt) || (t2.pt() < fConfigMinPt)) genfidcut = kFALSE;
	...
	if ((*sig).CheckSignal(true, groupedMCTracks, t1, t2)) {
	  ...
	  // not smeared
	  if(genfidcut) dynamic_cast<TH2D*>(fHistGenPair.at(isig))->Fill(mass,pairpt);
