               ...
               aod::V0Datas const& fullV0s, PIDTracks const& tracks)
  {
    ...
    for (auto& v0 : fullV0s) {
      ...
      if (passedK0Selection(v0, negTrack, posTrack, collision)) {
        float eposPi = posTrack.energy(piMass);
        float enegPi = negTrack.energy(piMass);
        ...
        if (std::abs(invMass * 1000 - k0sMass) > 10) {
          continue;
        }
      ...
      if (passedLambdaSelection(v0, negTrack, posTrack, collision)) {
        float eposPr = posTrack.energy(protonMass);
        float enegPi = negTrack.energy(piMass);
        ...
        if (std::abs(invMass - lambdaMass) > 0.01) {
          continue;
        }
      ...
      if (passedAntiLambdaSelection(v0, negTrack, posTrack, collision)) {
        ...
        float eposPi = posTrack.energy(piMass);
        float enegPr = negTrack.energy(protonMass);
      ...
      if (passedGammaSelection(v0, negTrack, posTrack, collision)) {
        ...
        float eposEl = posTrack.energy(eMass);
        float enegEl = negTrack.energy(eMass);
        ...
        if (std::abs(invMass - gammaMass) > 10) {
          continue;
        }

        registryDeDx.fill(HIST(kInvMass[3]), invMass);
