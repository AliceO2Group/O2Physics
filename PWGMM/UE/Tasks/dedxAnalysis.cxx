...
struct dedxAnalysis {
  ...
  // Histograms names
  static constexpr std::string_view hArmenteros[5] = {"Armenteros", "Armenteros_K0S", "Armenteros_Lambda", "Armenteros_AntiLambda", "Armenteros_Gamma"};
  ...
  static constexpr std::string_view hdEdxvsMomentum[2] = {"dEdx_vs_Momentum_all_beforeCalibration", "dEdx_vs_Momentum_AfterCalibration"};
  static constexpr std::string_view hdEdxvsMomentumV0[3] = {"dEdx_vs_Momentum_Pi_v0", "dEdx_vs_Momentum_Pr_v0", "dEdx_vs_Momentum_El_v0"};
  ...
  void init(InitContext const&)
  {
    ...
    for (int i = 0; i < 5; ++i) {
      registryDeDx.add(hArmenteros[i].data(), kQtvsAlpha[i].data(), HistType::kTH2F,
                       {{100, -1., 1., "#alpha (a. u.)"}, {100, 0.0, 0.3, "q_T (GeV/c)"}});
    }
    for (int i = 0; i < 2; ++i) {
      registryDeDx.add(hdEdxvsMomentum[i].data(), "dE/dx", HistType::kTH3F,
    ...
    for (int i = 0; i < 3; ++i) {
      ...
      registryDeDx.add(hdEdxvsMomentumV0[i].data(), "dE/dx", HistType::kTH3F,
                       {{100, -20.0, 20.0, "#it{p}/Z (GeV/c)"}, {100, 0.0, 600.0, "dE/dx (a. u.)"}, {100, -0.8, 0.8, "#eta"}});
    }
               ...
               aod::V0Datas const& fullV0s, PIDTracks const& tracks)
  {
    ...
    for (auto& trk : tracks) {
      ...
      registryDeDx.fill(HIST(hdEdxvsMomentum[0]), signedP, trk.tpcSignal(), trk.eta());
      ...
      for (int i = 0; i < 8; ++i) {
        if (trk.eta() > EtaCut[i] && trk.eta() < EtaCut[i + 1]) {
          registryDeDx.fill(HIST(hdEdxvsMomentum[1]), signedP, trk.tpcSignal() * 50 / Correction[i], trk.eta());
        }
      }
    }
    ...
    for (auto& v0 : fullV0s) {
      ...
      float pyV0 = v0.py();
      float pzV0 = v0.pz();
      float pV0 = sqrt(pxV0 * pxV0 + pyV0 * pyV0 + pzV0 * pzV0);
      ...
      float pPos = sqrt(pxPos * pxPos + pyPos * pyPos + pzPos * pzPos);
      float qt = sqrt(pPos * pPos - plPos * plPos);

      registryDeDx.fill(HIST(hArmenteros[0]), alpha, qt);
      ...
      if (passedK0Selection(v0, negTrack, posTrack, collision)) {
        ...
        float InvMass = sqrt((EnegPi + EposPi) * (EnegPi + EposPi) - ((pxNeg + pxPos) * (pxNeg + pxPos) + (pyNeg + pyPos) * (pyNeg + pyPos) + (pzNeg + pzPos) * (pzNeg + pzPos)));

        if (std::abs(InvMass * 1000 - K0S_MASS) > 10) {
          continue;
        }

        registryDeDx.fill(HIST(hInvMass[0]), InvMass * 1000);
        registryDeDx.fill(HIST(hArmenteros[1]), alpha, qt);

        for (int i = 0; i < 8; ++i) {
          if (negTrack.eta() > EtaCut[i] && negTrack.eta() < EtaCut[i + 1]) {

            registryDeDx.fill(HIST(hdEdxvsMomentumV0[0]), signedPneg, negTrack.tpcSignal() * 50 / Correction[i], negTrack.eta());
          }
          if (posTrack.eta() > EtaCut[i] && posTrack.eta() < EtaCut[i + 1]) {

            registryDeDx.fill(HIST(hdEdxvsMomentumV0[0]), signedPpos, posTrack.tpcSignal() * 50 / Correction[i], posTrack.eta());
          }
        }
      }
      ...
      if (passedLambdaSelection(v0, negTrack, posTrack, collision)) {
        ...
        registryDeDx.fill(HIST(hArmenteros[2]), alpha, qt);
        ...
        for (int i = 0; i < 8; ++i) {
          ...
          if (posTrack.eta() > EtaCut[i] && posTrack.eta() < EtaCut[i + 1]) {
            ...
            registryDeDx.fill(HIST(hdEdxvsMomentumV0[1]), signedPpos, posTrack.tpcSignal() * 50 / Correction[i], posTrack.eta());
          }
        }
      }
      ...
      if (passedAntiLambdaSelection(v0, negTrack, posTrack, collision)) {
        ...
        registryDeDx.fill(HIST(hArmenteros[3]), alpha, qt);

        for (int i = 0; i < 8; ++i) {
          if (negTrack.eta() > EtaCut[i] && negTrack.eta() < EtaCut[i + 1]) {

            registryDeDx.fill(HIST(hdEdxvsMomentumV0[1]), signedPneg, negTrack.tpcSignal() * 50 / Correction[i], negTrack.eta());
          }
          if (posTrack.eta() > EtaCut[i] && posTrack.eta() < EtaCut[i + 1]) {
      ...
      if (passedGammaSelection(v0, negTrack, posTrack, collision)) {
        ...
        registryDeDx.fill(HIST(hArmenteros[4]), alpha, qt);

        for (int i = 0; i < 8; ++i) {
          if (negTrack.eta() > EtaCut[i] && negTrack.eta() < EtaCut[i + 1]) {

            registryDeDx.fill(HIST(hdEdxvsMomentumV0[2]), signedPneg, negTrack.tpcSignal() * 50 / Correction[i], negTrack.eta());
          }
          if (posTrack.eta() > EtaCut[i] && posTrack.eta() < EtaCut[i + 1]) {

            registryDeDx.fill(HIST(hdEdxvsMomentumV0[2]), signedPpos, posTrack.tpcSignal() * 50 / Correction[i], posTrack.eta());
          }
        }
      }
    }
