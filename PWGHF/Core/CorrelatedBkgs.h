...
namespace o2::hf_corrbkg
{
      ...
      for (const auto& [chn, finalState] : finalStates3Prongs) {
        ...
        if (indexRec > -1) {
          ...
          if (arrResoDaughIndex.size() == NDaughtersResonant) {
            ...
            LOG(info) << "[matchFinalStateDMeson] Flag set to: " << static_cast<int>(*flag) << " sign: " << static_cast<int>(*sign) << " for channel: " << channel;
            break;
          }
    ...
    int matchFinalStateCorrBkgsGen(int pdgParticle, aod::McParticles const& mcParticles, const U& candidate, int8_t* sign, int depth, int8_t* flag, int8_t* channel) {
      ...
      //   auto motherI = particleI.template mothers_first_as<aod::McParticles>();
      //   auto pdgI = particleI.pdgCode();
      //   auto pdgParticleI = motherI.pdgCode();
      //   auto pdgParticleII = -1;
      //   LOG(info) << "Daughter " << iProng << " PDG: " << pdgI << " motherI: " << pdgParticleI;
      // }
      
      // auto finalStates = finalStates3Prongs;
      // if (std::abs(pdgParticle) == Pdg::kD0) {
      //   finalStates = finalStates2Prongs;
      // }
      ...
      for (const auto& [chn, finalState] : finalStates3Prongs) {
        if (finalState.size() == 4) {                     // Partly Reco 3-prong decays
          std::array<int, 4> finalStateParts = std::array{finalState[0], finalState[1], finalState[2], finalState[3]};
          // LOG(info) << "Matching final state: " << finalState[0] << " " << finalState[1] << " " << finalState[2] << " " << finalState[3];
          // matched = RecoDecay::isMatchedMCGen(mcParticles, candidate, pdgParticle, std::array{+kPiPlus, -kKPlus, +kPiPlus}, true, sign, depth);
          matched = RecoDecay::isMatchedMCGen(mcParticles, candidate, pdgParticle, finalStateParts, true, sign, depth);
          // RecoDecay::isMatchedMCGen(mcParticles, particle, Pdg::kDPlus, std::array{+kPiPlus, -kKPlus, +kPiPlus}, true, &sign, 2)) {
          } else if (finalState.size() == 3) {              // Fully Reco 3-prong decays
            // LOG(info) << "Matching final state: " << finalState[0] << " " << finalState[1] << " " << finalState[2];
            std::array<int, 3> finalStateParts = std::array{finalState[0], finalState[1], finalState[2]};
            matched = RecoDecay::isMatchedMCGen(mcParticles, candidate, pdgParticle, finalStateParts, true, sign, depth);
          } else if (finalState.size() == 2) {              // Fully Reco 2-prong decays
            // LOG(info) << "Matching final state: " << finalState[0] << " " << finalState[1];
          std::array<int, 2> finalStateParts = std::array{finalState[0], finalState[1]};
          matched = RecoDecay::isMatchedMCGen(mcParticles, candidate, pdgParticle, finalStateParts, true, sign, depth);
        } else {
          // LOG(info) << "Final state size not supported: " << finalState.size();
        ...
        if (matched) {
          ...
          if (arrResoDaughIndex.size() == NDaughtersResonant) {
            ...
            LOG(info) << "[matchFinalStateDMeson] Flag set to: " << static_cast<int>(*flag) << " sign: " << static_cast<int>(*sign) << " for channel: " <<  static_cast<int>(*channel);
            break;
          }
          break; // Exit loop if a match is found
        }
