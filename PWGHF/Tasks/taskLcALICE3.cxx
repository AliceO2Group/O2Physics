...
struct TaskLcALICE3MC {
  HistogramRegistry registry{
    ...
    {{"hMassGen", "3-prong candidates (generated); #it{p}_{T}; #it{y}", {HistType::kTH2F, {{150, 0., 30.}, {8, 0, 4.0}}}},
     ...
     {"hMassSigBkgLcRICHPID", "3-prong candidates (not checked);#it{m}_{inv} (GeV/#it{c}^{2}); #it{p}_{T}; #it{y}", {HistType::kTH3F, {{500, 1.6, 3.1}, {150, 0., 30.}, {8, 0, 4.0}}}},
     {"hMassSigLcRICHPID", "3-prong candidates (matched);#it{m}_{inv} (GeV/#it{c}^{2}); #it{p}_{T}; #it{y}", {HistType::kTH3F, {{500, 1.6, 3.1}, {150, 0., 30.}, {8, 0., 4.}}}},
     {"hMassBkgLcRICHPID", "3-prong candidates (checked);#it{m}_{inv} (GeV/#it{c}^{2}); #it{p}_{T}; #it{y}", {HistType::kTH3F, {{500, 1.6, 3.1}, {150, 0., 30.}, {8, 0., 4.}}}},
     {"hMassReflBkgLcRICHPID", "3-prong candidates (checked);#it{m}_{inv} (GeV/#it{c}^{2}); #it{p}_{T}; #it{y}", {HistType::kTH3F, {{500, 1.6, 3.1}, {150, 0., 30.}, {8, 0., 4.}}}},
  ...
  Filter filterSelectCandidates = (aod::hf_selcandidate_lc_ALICE3::isSelHFFlag >= 1);
               ...
               soa::Join<aod::McParticles, aod::HfCandProng3MCGen> const& particlesMC, aod::BigTracksMC const& tracks)
  {
    for (auto& candidate : candidates) {
      ...
      auto massLcbar = InvMassLcpiKp(candidate);
      auto ptCandidate = candidate.pt();
      auto rapidityCandidate = std::abs(YLc(candidate));

      if (candidate.isSelLcNoPID() >= 1) {
      ...
      if (candidate.isSelLcRICHPID() >= 1) {
        registry.fill(HIST("hMassSigBkgLcRICHPID"), massLc, ptCandidate, rapidityCandidate);
        if (candidate.flagMCMatchRec() == (1 << DecayType::LcToPKPi)) {
          registry.fill(HIST("hMassSigLcRICHPID"), massLc, ptCandidate, rapidityCandidate);
        } else {
          if (candidate.flagMCMatchRec() == -(1 << DecayType::LcToPKPi)) {
            registry.fill(HIST("hMassReflBkgLcRICHPID"), massLc, ptCandidate, rapidityCandidate);
          }
          registry.fill(HIST("hMassBkgLcRICHPID"), massLc, ptCandidate, rapidityCandidate);
        }
      }
      if (candidate.isSelLcTOFplusRICHPID() >= 1) {
      ...
      if (candidate.isSelLcPerfectPID() >= 1) {
