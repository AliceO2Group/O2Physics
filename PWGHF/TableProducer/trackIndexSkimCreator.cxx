...
struct HfTrackIndexSkimCreatorTagSelTracks {
  ...
  void init(InitContext const&)
  {
    ...
    for (auto iChannel{0u}; iChannel < ChannelsProtonPid::NChannelsProtonPid; ++iChannel) {
      selectorProton[iChannel].setRangePtTpc(selectionsPid->get(iChannel, 0u), selectionsPid->get(iChannel, 1u));
    ...
    selectorKaon.setRangePtTpc(selectionsPid->get(channelKaonPid, 0u), selectionsPid->get(channelKaonPid, 1u));
    selectorKaon.setRangePtTof(selectionsPid->get(channelKaonPid, 3u), selectionsPid->get(channelKaonPid, 4u));
    selectorKaon.setRangeNSigmaTpc(-selectionsPid->get(channelKaonPid, 2u), selectionsPid->get(channelKaonPid, 2u));
    selectorKaon.setRangeNSigmaTof(-selectionsPid->get(channelKaonPid, 5u), selectionsPid->get(channelKaonPid, 5u));
  }
...
struct HfTrackIndexSkimCreator {
  ...
  Configurable<bool> applyKaonPidIn3Prongs{"applyKaonPidIn3Prongs", false, "Apply kaon PID for opposite-sign track in 3-prong decays"};
    ...
    if (debug || isSelected > 0) {
      ...
      for (int iDecay3P = 0; iDecay3P < kN3ProngDecays; iDecay3P++) {
        ...
        if ((debug || TESTBIT(isSelected, iDecay3P))) {
          auto cpa = RecoDecay::cpa(primVtx, secVtx, pVecCand);
          if (cpa < cut3Prong[iDecay3P].get(pTBin, 2u)) {
            CLRBIT(isSelected, iDecay3P);
            if (debug) {
        ...
        if ((debug || TESTBIT(isSelected, iDecay3P))) {
          ...
          if (decayLength < cut3Prong[iDecay3P].get(pTBin, 3u)) {
            CLRBIT(isSelected, iDecay3P);
            if (debug) {
