                 ...
                 aod::McCollisionLabels const&)
  {
    ...
    for (const auto& candidate : candidates) {
      ...
      if (matchOmegacMc) {
        ...
        if (indexRec > -1) {
          // Omega- → kaon pi p
          indexRec = RecoDecay::getMatchedMCRec(mcParticles, arrayDaughtersCasc, static_cast<int>(kOmegaMinus), std::array{static_cast<int>(kKMinus), static_cast<int>(kProton), static_cast<int>(kPiMinus)}, true, &sign, 2);
          if (indexRec == -1) {
            debug = 2;
          }
          if (indexRec > -1) {
      ...
      if (matchOmegacMc) {
        ...
        if (RecoDecay::isMatchedMCGen(mcParticles, particle, static_cast<int>(kOmegaC0), std::array{static_cast<int>(kOmegaMinus), static_cast<int>(kPiPlus)}, true, &sign)) {
          ...
          if (RecoDecay::isMatchedMCGen(mcParticles, cascMC, static_cast<int>(kOmegaMinus), std::array{static_cast<int>(kLambda0), static_cast<int>(kKMinus)}, true)) {
            ...
            if (RecoDecay::isMatchedMCGen(mcParticles, v0MC, static_cast<int>(kLambda0), std::array{static_cast<int>(kProton), static_cast<int>(kPiMinus)}, true)) {
              debugGenLambda = 1;
              flag = sign * (1 << aod::hf_cand_omegac::DecayType::OmegaczeroToOmegaPi);
            }
          }
