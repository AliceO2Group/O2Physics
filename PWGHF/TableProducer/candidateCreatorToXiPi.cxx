...
struct HfCandidateCreatorToXiPi {
  ...
  using MyTrkCascTable = soa::Join<aod::TraCascDatas, aod::CascCovs>;
  ...
  {
    if (doprocessWithCascades == true && doprocessWithTrackedCascades == true) {
      LOGF(fatal, "Cannot enable processWithCascades and processWithTrackedCascades at the same time. Please choose one.");
    }
    if (doprocessWithCascades == false && doprocessWithTrackedCascades == false) {
      LOGF(fatal, "Please enable at least one process function, either processWithCascades or processWithTrackedCascades.");
    }
                                  ...
                                  MySkimIdx const& candidates)
  {
    ...
    for (const auto& cand : candidates) {
      ...
      if (!cascAodElement.has_cascData())
        continue;
      auto casc = cascAodElement.cascData_as<MyTrkCascTable>();
      hCascadesCounter->Fill(1);
      auto trackXiDauCharged = casc.bachelor_as<TracksWCovDca>(); // pion <- xi track
      auto trackV0Dau0 = casc.posTrack_as<TracksWCovDca>();       // V0 positive daughter track
      auto trackV0Dau1 = casc.negTrack_as<TracksWCovDca>();       // V0 negative daughter track
