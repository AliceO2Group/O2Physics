...
/// \author Ana Marin ana.marin@cern.ch  , adding the converted photons part
               ...
               aod::PHOTONs const& photons)
  {
    ...
    for (auto& jpsiCand : jpsiCands) {
      ...
      for (auto& photon : photons) {
        ...
        array<float, 3> pvecGamma{(float)photon.px(), (float)photon.py(), (float)photon.pz()};
        ...
        if (TESTBIT(jpsiCand.hfflag(), hf_cand_prong2::DecayType::JpsiToMuMu)) {
          SETBIT(hfFlag, hf_cand_chicPCM::DecayType::ChicToJpsiToMuMuGamma); // dimuon channel
        }
...
WorkflowSpec defineDataProcessing(ConfigContext const& cfgc)
{
  WorkflowSpec workflow{
    adaptAnalysisTask<HFCandidateCreatorChicPCM>(cfgc, TaskName{"hf-cand-creator-chicPCM"}),
