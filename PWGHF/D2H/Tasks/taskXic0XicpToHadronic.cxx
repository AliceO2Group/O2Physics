...
struct HfTaskXic0XicpToHadronic {
  ...
  void init(InitContext const&)
  {
    ...
    std::array<bool, 4> doprocess{doprocessXic0WithDCAFitter,
    ...
    if ((doprocess[0] || doprocess[2]) && (doprocess[1] || doprocess[3])) {
    ...
    if (doprocess[0] || doprocess[1]) {
      ...
      if (doprocess[1]) { // If KF specific histograms
  ...
  void fillHistograms(TCandTable const& candidates)
  {
    for (const auto& candidate : candidates) {
      ...
      auto yCandXic0Xicp = candidate.y(o2::constants::physics::MassXiC0);
