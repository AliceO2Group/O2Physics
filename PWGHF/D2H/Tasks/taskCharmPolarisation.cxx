  ...
  void runPolarisationAnalysis(Cand const& candidate, int bkgRotationId = 0)
  {
    ...
    for (uint8_t iMass = 0u; iMass < nMassHypos; iMass++) {
      ...
      if constexpr (channel == charm_polarisation::DecayChannel::DstarToDzeroPi) {
        ...
        std::array<float, 3> threeVecSoftPi{candidate.pxSoftPi() * std::cos(bkgRotAngle) - candidate.pySoftPi() * std::sin(bkgRotAngle), candidate.pxSoftPi() * std::cos(bkgRotAngle) + candidate.pySoftPi() * std::sin(bkgRotAngle), candidate.pzSoftPi()}; // we rotate the soft pion
