...
struct HfTaskXic {
  ...
  Configurable<float> trackDcaXYCut{"trackDcaXYCut", 0.0025, "DCAxy cuts for tracks"};
  Configurable<float> trackDcaZCut{"trackDcaZCut", 0.0025, "DCAz cuts for tracks"};
  ...
  void process(aod::Collision const& collision, soa::Join<aod::Tracks, aod::TracksDCA> const& tracks, soa::Filtered<soa::Join<aod::HfCand3Prong, aod::HfSelXicToPKPi>> const& candidates, aod::BigTracksPID const&)
  {
    ...
    if (collision.numContrib() > 1) {
      for (auto const& track : tracks) {
        if (std::abs(track.eta()) > trackEtaCut) { // hard coded cut TO BE REFINED
          continue;
        }
        if (std::abs(track.dcaXY()) > trackDcaXYCut || std::abs(track.dcaZ()) > trackDcaZCut) { // hardcoded cut TO BE REFINED
          continue;
        }
        nTracks++;
      }
    ...
    for (auto const& candidate : candidates) {
      ...
      auto prong0Momentum = trackProng0.p();
      auto prong1Momentum = trackProng1.p();
      auto prong2Momentum = trackProng2.p();

      // TPC nSigma histograms
      registry.fill(HIST("Data/hPVsTPCNSigmaPr_Prong0"), prong0Momentum, trackProng0.tpcNSigmaPr());
      registry.fill(HIST("Data/hPVsTPCNSigmaPi_Prong0"), prong0Momentum, trackProng0.tpcNSigmaPi());
      registry.fill(HIST("Data/hPVsTPCNSigmaKa_Prong0"), prong0Momentum, trackProng0.tpcNSigmaKa());

      registry.fill(HIST("Data/hPVsTPCNSigmaPr_Prong1"), prong1Momentum, trackProng1.tpcNSigmaPr());
      registry.fill(HIST("Data/hPVsTPCNSigmaPi_Prong1"), prong1Momentum, trackProng1.tpcNSigmaPi());
      registry.fill(HIST("Data/hPVsTPCNSigmaKa_Prong1"), prong1Momentum, trackProng1.tpcNSigmaKa());

      registry.fill(HIST("Data/hPVsTPCNSigmaPr_Prong2"), prong2Momentum, trackProng2.tpcNSigmaPr());
      registry.fill(HIST("Data/hPVsTPCNSigmaPi_Prong2"), prong2Momentum, trackProng2.tpcNSigmaPi());
      registry.fill(HIST("Data/hPVsTPCNSigmaKa_Prong2"), prong2Momentum, trackProng2.tpcNSigmaKa());

      // TOF nSigma histograms
      registry.fill(HIST("Data/hPVsTOFNSigmaPr_Prong0"), prong0Momentum, trackProng0.tofNSigmaPr());
      registry.fill(HIST("Data/hPVsTOFNSigmaPi_Prong0"), prong0Momentum, trackProng0.tofNSigmaPi());
      registry.fill(HIST("Data/hPVsTOFNSigmaKa_Prong0"), prong0Momentum, trackProng0.tofNSigmaKa());

      registry.fill(HIST("Data/hPVsTOFNSigmaPr_Prong1"), prong1Momentum, trackProng1.tofNSigmaPr());
      registry.fill(HIST("Data/hPVsTOFNSigmaPi_Prong1"), prong1Momentum, trackProng1.tofNSigmaPi());
      registry.fill(HIST("Data/hPVsTOFNSigmaKa_Prong1"), prong1Momentum, trackProng1.tofNSigmaKa());

      registry.fill(HIST("Data/hPVsTOFNSigmaPr_Prong2"), prong2Momentum, trackProng2.tofNSigmaPr());
      registry.fill(HIST("Data/hPVsTOFNSigmaPi_Prong2"), prong2Momentum, trackProng2.tofNSigmaPi());
      registry.fill(HIST("Data/hPVsTOFNSigmaKa_Prong2"), prong2Momentum, trackProng2.tofNSigmaKa());
    }
  }
  // Fill MC histograms
                 ...
                 soa::Join<aod::McParticles, aod::HfCand3ProngMcGen> const& particlesMC, aod::BigTracksMC const& /*tracks*/)
  {
    ...
    for (auto const& candidate : candidates) {
      ...
      if (std::abs(candidate.flagMcMatchRec()) == 1 << DecayType::XicToPKPi) {
        ...
        if (massXicToPKPi != 0.) {
          registry.fill(HIST("MC/reconstructed/signal/hMassSig"), massXicToPKPi, candidatePt);
        }
        if (massXicToPiKP != 0.) {
          registry.fill(HIST("MC/reconstructed/signal/hMassSig"), massXicToPiKP, candidatePt);
        }
        ...
        if (massXicToPKPi != 0.) {
          registry.fill(HIST("MC/reconstructed/background/hMassBg"), massXicToPKPi, candidatePt);
        }
        if (massXicToPiKP != 0.) {
          registry.fill(HIST("MC/reconstructed/background/hMassBg"), massXicToPiKP, candidatePt);
        }
