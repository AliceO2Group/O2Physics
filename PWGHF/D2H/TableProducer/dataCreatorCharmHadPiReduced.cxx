...
struct HfDataCreatorCharmHadPiReduced {
                      ...
                      const int64_t indexCollisionMaxNumContrib)
  {
    ...
    if constexpr (decChannel == DecayChannel::B0ToDminusPi) {
      ...
      // Lb → Lc+ π- → (p K- π+) π-
      auto indexRec = RecoDecay::getMatchedMCRec<true, false, false, true, true>(particlesMc, std::array{vecDaughtersB[0], vecDaughtersB[1], vecDaughtersB[2], vecDaughtersB[3]}, Pdg::kLambdaB0, std::array{+kProton, -kKPlus, +kPiPlus, -kPiPlus}, true, &sign, 3);
      if (indexRec > -1) {
        // Lc+ → p K- π+
      ...
      if (checkDecayTypeMc) {
        // Lb → Lc+ K- →  (p K- π+) K-
        if (!flag) {
          indexRec = RecoDecay::getMatchedMCRec<true, false, false, true, true>(particlesMc, std::array{vecDaughtersB[0], vecDaughtersB[1], vecDaughtersB[2], vecDaughtersB[3]}, Pdg::kLambdaB0, std::array{+kProton, -kKPlus, +kPiPlus, -kKPlus}, true, &sign, 3);
          if (indexRec > -1) {
            //  Lc+ → p K- π+
            indexRec = RecoDecay::getMatchedMCRec<false, false, false, true, true>(particlesMc, std::array{vecDaughtersB[0], vecDaughtersB[1], vecDaughtersB[2]}, -Pdg::kLambdaCPlus, std::array{+kProton, -kKPlus, +kPiPlus}, true, &sign, 2);
            if (indexRec > -1) {
