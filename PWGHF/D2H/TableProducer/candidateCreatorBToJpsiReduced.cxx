...
struct HfCandidateCreatorBToJpsiReduced {
  ...
  Produces<aod::HfCandB0JPBase> rowCandidateB0Base;        // table defined in CandidateReconstructionTables.h
  Produces<aod::HfRedB02JpsiDaus> rowCandidateB0Prongs;    // table defined in ReducedDataModel.h
  ...
  void init(InitContext const&)
  {
    ...
    registry.add("hMassB0ToJpsiK0Star", "2-prong candidates;inv. mass (B^{0} #rightarrow J/ψ K^{*0}) (GeV/#it{c}^{2});entries", {HistType::kTH1F, {{500, 4., 6.}}});
    registry.add("hMassBsToJpsiPhi", "2-prong candidates;inv. mass (B^{0}_{s} #rightarrow J/ψ #phi) (GeV/#it{c}^{2});entries", {HistType::kTH1F, {{500, 4., 6.}}});
      ...
      for (const auto& trackLf0 : tracksLfDau0ThisCollision) {
        ...
        if constexpr (DecChannel == DecayChannel::BplusToJpsiK) {
          ...
          for (const auto& trackLf1 : tracksLfDau1ThisCollision) {
            ...
            // ---------------------------------
            // reconstruct the 4-prong B0 vertex
            hCandidatesB->Fill(SVFitting::BeforeFit);
            try {
            ...
            hCandidatesB->Fill(SVFitting::FitOk);
            // passed B0 reconstruction

            // calculate relevant properties
            const auto& secondaryVertexB0 = df4.getPCACandidate();
            auto chi2PCA = df4.getChi2AtPCACandidate();
            auto covMatrixPCA = df4.calcPCACovMatrixFlat();
            registry.fill(HIST("hCovSVXX"), covMatrixPCA[0]);
            registry.fill(HIST("hCovPVXX"), covMatrixPV[0]);

            // get JPsi daughters and K tracks (propagated to the B0 vertex if propagateToPCA==true)
            // track.getPxPyPzGlo(pVec) modifies pVec of track
            df4.getTrack(0).getPxPyPzGlo(pVecDauPos);   // momentum of Jpsi at the B0 vertex
            df4.getTrack(1).getPxPyPzGlo(pVecDauNeg);   // momentum of Jpsi at the B0 vertex
            df4.getTrack(2).getPxPyPzGlo(pVecTrackLf0); // momentum of K at the B0 vertex
            df4.getTrack(3).getPxPyPzGlo(pVecTrackLf1); // momentum of K at the B0 vertex

            // compute invariant mass square and apply selection
            auto invMass2JpsiPiK = RecoDecay::m2(std::array{pVecDauPos, pVecDauNeg, pVecTrackLf0, pVecTrackLf1}, std::array{o2::constants::physics::MassMuon, o2::constants::physics::MassMuon, o2::constants::physics::MassPiPlus, o2::constants::physics::MassKPlus});
            auto invMass2JpsiKPi = RecoDecay::m2(std::array{pVecDauPos, pVecDauNeg, pVecTrackLf0, pVecTrackLf1}, std::array{o2::constants::physics::MassMuon, o2::constants::physics::MassMuon, o2::constants::physics::MassKPlus, o2::constants::physics::MassPiPlus});
            bool isK0StarPiK = invMass2JpsiPiK > invMass2JpsiHadMin && invMass2JpsiPiK < invMass2JpsiHadMax;
            bool isK0StarKPi = invMass2JpsiKPi > invMass2JpsiHadMin && invMass2JpsiKPi < invMass2JpsiHadMax;

            if (!isK0StarPiK && !isK0StarKPi) {
              continue;
            }
            registry.fill(HIST("hMassB0ToJpsiK0Star"), isK0StarPiK ? std::sqrt(invMass2JpsiPiK) : std::sqrt(invMass2JpsiKPi));

            // compute impact parameters of Jpsi and K*0 daughters
            ...
            rowCandidateB0Base(collision.globalIndex(),
                               collision.posX(), collision.posY(), collision.posZ(),
            ...
            rowCandidateB0Prongs(candJpsi.globalIndex(), trackLf0.globalIndex(), trackLf1.globalIndex());
          }
          ...
          for (const auto& trackLf1 : tracksLfDau1ThisCollision) {
            ...
            // track.getPxPyPzGlo(pVec) modifies pVec of track
            df4.getTrack(0).getPxPyPzGlo(pVecDauPos);   // momentum of Jpsi at the Bs vertex
            df4.getTrack(1).getPxPyPzGlo(pVecDauNeg);   // momentum of Jpsi at the Bs vertex
            df4.getTrack(2).getPxPyPzGlo(pVecTrackLf0); // momentum of K at the Bs vertex
            df4.getTrack(3).getPxPyPzGlo(pVecTrackLf1); // momentum of K at the Bs vertex
            ...
            registry.fill(HIST("hMassBsToJpsiPhi"), std::sqrt(invMass2JpsiPhi));
  ...
  void processDataB0(HfRedCollisionsWithExtras const& collisions,
                     soa::Join<aod::HfRedJpsis, aod::HfRedJpsiCov> const& candsJpsi,
                     ...
                     aod::HfOrigColCounts const& collisionsCounter,
                     aod::HfCfgB0ToJpsis const& configs)
  {
    // Jpsi K*0 invariant-mass window cut
    for (const auto& config : configs) {
      myInvMassWindowJpsiK0Star = config.myInvMassWindowJpsiK0Star();
    }
    // invMassWindowJpsiHadTolerance is used to apply a slightly tighter cut than in JpsiK pair preselection
    // to avoid accepting JpsiK pairs that were not formed in JpsiK pair creator
    double const invMass2JpsiKMin = (massB0 - myInvMassWindowJpsiK0Star + invMassWindowJpsiHadTolerance) * (massB0 - myInvMassWindowJpsiK0Star + invMassWindowJpsiHadTolerance);
    double const invMass2JpsiKMax = (massB0 + myInvMassWindowJpsiK0Star - invMassWindowJpsiHadTolerance) * (massB0 + myInvMassWindowJpsiK0Star - invMassWindowJpsiHadTolerance);
    ...
    for (const auto& collision : collisions) {
      ...
      runCandidateCreation<DecayChannel::B0ToJpsiK0Star>(collision, candsJpsiThisColl, tracksLf0ThisCollision, tracksLf1ThisCollision, invMass2JpsiKMin, invMass2JpsiKMax);
    }
  } // processDataB0
  PROCESS_SWITCH(HfCandidateCreatorBToJpsiReduced, processDataB0, "Process data for B0", false);
  ...
  {
    // Jpsi phi invariant-mass window cut
    for (const auto& config : configs) {
...
struct HfCandidateCreatorBToJpsiReducedExpressions {
  ...
  Spawns<aod::HfCandB0JPExt> rowCandidateB0;
  ...
  void fillMcRec(McRec const& rowsJpsiHadMcRec, CCands const& candsBIds)
  {
    for (const auto& candB : candsBIds) {
      ...
      if constexpr (DecChannel == DecayChannel::BplusToJpsiK) {
        ...
        for (const auto& rowJpsiHadMcRec : rowsJpsiHadMcRec) {
          if ((rowJpsiHadMcRec.jpsiId() != candB.jpsiId()) || (rowJpsiHadMcRec.prong0K0StarId() != candB.prong0K0StarId()) || (rowJpsiHadMcRec.prong1K0StarId() != candB.prong1K0StarId())) {
            continue;
          }
          rowB0McRec(rowJpsiHadMcRec.flagMcMatchRec(), rowJpsiHadMcRec.flagMcDecayChanRec(), rowJpsiHadMcRec.flagWrongCollision(), rowJpsiHadMcRec.debugMcRec(), rowJpsiHadMcRec.ptMother());
          filledMcInfo = true;
          break;
        }
        if (!filledMcInfo) { // protection to get same size tables in case something went wrong: we created a candidate that was not preselected in the Jpsi-K creator
  ...
  PROCESS_SWITCH(HfCandidateCreatorBToJpsiReducedExpressions, processMcBPlus, "Process MC for B+", false);

  void processMcB0(HfMcRecRedJPK0ss const& rowsJpsiK0StarMcRec, HfRedB02JpsiDaus const& b0)
  {
    fillMcRec<DecayChannel::B0ToJpsiK0Star>(rowsJpsiK0StarMcRec, b0);
  }
  PROCESS_SWITCH(HfCandidateCreatorBToJpsiReducedExpressions, processMcB0, "Process MC for B0", false);
  ...
  PROCESS_SWITCH(HfCandidateCreatorBToJpsiReducedExpressions, processMcBs, "Process MC for Bs", false);
};
