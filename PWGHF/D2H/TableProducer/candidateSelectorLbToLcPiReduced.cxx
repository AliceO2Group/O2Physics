...
struct HfCandidateSelectorLbToLcPiReduced {
  ...
  // Enable PID
  Configurable<int> pionPidMethod{"pionPidMethod", 1, "PID selection method for the bachelor pion (PidMethod::NoPid: none, PidMethod::TpcOrTof: TPC or TOF, PidMethod::TpcAndTof: TPC and TOF)"};
                    ...
                    HfCandLbConfigs const&)
  {
    for (const auto& hfCandLb : hfCandsLb) {
      ...
      if (applyLbMl) {
        ...
        SETBIT(statusLbToLcPi, SelectionStep::RecoMl); // RecoML = 3 --> statusLbToLcPi = 15 if pionPidMethod, 11 otherwise
        if (activateQA) {
