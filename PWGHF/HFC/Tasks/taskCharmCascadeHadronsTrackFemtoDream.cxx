...
/// \file taskCharmCascadeHadronsTrackFemtoDream.cxx
/// \brief FemtoDream task for Ξc± → Ξ∓ π± π± + associated track (e.g. π)
...
struct HfTaskCharmCascadeHadronsTrackFemtoDream {
  ...
  enum DecayChannel { XicToXiPiPi = 0 };
  ...
  struct : ConfigurableGroup {
    ...
    Configurable<int> charmHadCandSel{"charmHadCandSel", 1, "candidate selection for charm hadron (bit mask, 1 for XicToXiPiPi)"};
    Configurable<int> charmHadMcSel{"charmHadMcSel", 1, "MC charm hadron selection: 1 << XicToXiPiPi = 1"};
    ...
    Configurable<float> charmHadMaxInvMass{"charmHadMaxInvMass", 2.55, "Maximum invariant mass of Ξc± (particle 2)"};
    Configurable<float> charmHadMinInvMass{"charmHadMinInvMass", 2.35, "Minimum invariant mass of Ξc± (particle 2)"};
    ...
    Configurable<int> charmHadPDGCode{"charmHadPDGCode", 4232, "PDG code of particle 2 (Ξc+)"};
  ...
  struct : ConfigurableGroup {
    ...
    Configurable<int> pdgCodeTrack1{"pdgCodeTrack1", 211, "PDG code of Particle 1 (Track)"};
  ...
  using FilteredCharmCand3Prongs = soa::Filtered<soa::Join<aod::FDHfCand3Prong, aod::FDHfCand3ProngXic>>;
  using FilteredCharmCand3Prong = FilteredCharmCand3Prongs::iterator;

  using FilteredCharmMcCand3Prongs = soa::Filtered<soa::Join<aod::FDHfCand3Prong, aod::FDHfCand3ProngXic, aod::FDHfCandMC>>;
  ...
  ConfigurableAxis bin4Dkstar{"bin4Dkstar", {1500, 0., 6.}, "binning kstar for the 4Dimensional plot: k* vs multiplicity vs multiplicity percentil vs mT (set <<Confuse4D>> to true in order to use)"};
  ...
  ConfigurableAxis binInvMass{"binInvMass", {400, 2.10, 2.80}, "InvMass binning for Ξc±"};
  ...
  {
    std::array<bool, 2> processes = {doprocessDataXicTrk, doprocessMcXicTrk};
    ...
    for (auto const& part : sliceTrk1) {
      ...
      switch (trackSel.pdgCodeTrack1.value) {
        ...
        case kDeuteron:
          ...
          break;
        default:
          LOG(fatal) << "Invalid PDG code for track PID QA: " << trackSel.pdgCodeTrack1.value;
      }
    ...
    if (mixSetting.doMixEvent) {
      ...
      switch (mixSetting.mixingBinPolicy) {
        case femtodreamcollision::kMult:
          doMixedEvent<false, DecayChannel::XicToXiPiPi, FilteredCollisions>(cols, partitionCharmHadron3Prong, *partitionTrk1Selected, parts, colBinningMult);
          break;
        case femtodreamcollision::kMultPercentile:
          doMixedEvent<false, DecayChannel::XicToXiPiPi, FilteredCollisions>(cols, partitionCharmHadron3Prong, *partitionTrk1Selected, parts, colBinningMultPercentile);
          break;
        case femtodreamcollision::kMultMultPercentile:
          doMixedEvent<false, DecayChannel::XicToXiPiPi, FilteredCollisions>(cols, partitionCharmHadron3Prong, *partitionTrk1Selected, parts, colBinningMultMultPercentile);
          break;
        default:
  ...
  PROCESS_SWITCH(HfTaskCharmCascadeHadronsTrackFemtoDream, processDataXicTrk, "Enable processing XicToXiPiPi and Tracks correlation", false);
                       ...
                       FilteredCharmMcCand3Prongs const&)
  {
    ...
    switch (mixSetting.mixingBinPolicy) {
      case femtodreamcollision::kMult:
        doMixedEvent<true, DecayChannel::XicToXiPiPi, FilteredMcColisions>(cols, partitionMcCharmHadron3Prong, *partitionTrk1Selected, parts, colBinningMult);
        break;
      case femtodreamcollision::kMultPercentile:
        doMixedEvent<true, DecayChannel::XicToXiPiPi, FilteredMcColisions>(cols, partitionMcCharmHadron3Prong, *partitionTrk1Selected, parts, colBinningMultPercentile);
        break;
      case femtodreamcollision::kMultMultPercentile:
        doMixedEvent<true, DecayChannel::XicToXiPiPi, FilteredMcColisions>(cols, partitionMcCharmHadron3Prong, *partitionTrk1Selected, parts, colBinningMultMultPercentile);
        break;
      default:
        LOG(fatal) << "Invalid binning policiy specifed. Breaking...";
    }
  }
  PROCESS_SWITCH(HfTaskCharmCascadeHadronsTrackFemtoDream, processMcXicTrk, "Enable processing XicToXiPiPi and Tracks correlation for Monte Carlo", false);
};

WorkflowSpec defineDataProcessing(ConfigContext const& cfgc)
{
  return WorkflowSpec{adaptAnalysisTask<HfTaskCharmCascadeHadronsTrackFemtoDream>(cfgc)};
}
