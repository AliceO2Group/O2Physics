...
struct HfTaskCharmHadronsCharmFemtoDream {
  ...
  // Keep charge-conjugate channels separate so they can also be combined
  // offline.
  enum PairChannel : int {
    D0D0LikeSign = 0,
    D0barD0barLikeSign = 1,
    D0D0barUnlikeSign = 2,
    D0DstarPlusLikeSign = 3,
    D0barDstarMinusLikeSign = 4,
    D0DstarMinusUnlikeSign = 5,
  ...
  void init(InitContext const&)
  {
    ...
    registry.add("ME/D0D0", "Mixed event", kTHnSparseF, {kstar, massD0, massD0, pt, pt, mult, channel});
    registry.add("SE/D0Dstar", "Same event", kTHnSparseF, {kstar, massD0, deltaMass, pt, ptStar, mult, channel});
    registry.add("ME/D0Dstar", "Mixed event", kTHnSparseF, {kstar, massD0, deltaMass, pt, ptStar, mult, channel});
    registry.add("QA/events", ";0=in mixing range,1=outside;events", kTH1F, {{2, -0.5, 1.5}});
    ...
    registry.add("QA/sharedDaughters", ";0=D0D0,1=D0Dstar;rejected pairs", kTH1F, {{2, -0.5, 1.5}});
  ...
  {
    if constexpr (!IsDstar) {
      return d0.charge() != other.charge() ? D0D0barUnlikeSign : (d0.charge() > 0 ? D0D0LikeSign : D0barD0barLikeSign);
    }
    return d0.charge() > 0 ? (other.charge() > 0 ? D0DstarPlusLikeSign : D0DstarMinusUnlikeSign) : (other.charge() < 0 ? D0barDstarMinusLikeSign : D0barDstarPlusUnlikeSign);
  }
  ...
  static bool reverseD0Order(FirstRow const& first, SecondRow const& second)
  {
    ...
    return first.pt() < second.pt(); // leading pT first for equal flavours
  }

  template <typename CandsType>
  void validateMlScores(CandsType const& candidates)
  {
    if (!useMl) {
      return;
    }
    for (const auto& row : candidates) {
      if (!std::isfinite(row.bdtBkg()) || !std::isfinite(row.bdtPrompt()) || row.bdtBkg() < 0.f || row.bdtPrompt() < 0.f) {
  ...
  void fillPair(FirstRow const& first, SecondRow const& second, float firstMass, float secondMass, float mult)
  {
    if constexpr (!Mixed) {
    ...
    const float kstar = reverse ? FemtoDreamMath::getkstar(second, MassD0, first, MassD0) : FemtoDreamMath::getkstar(first, MassD0, second, IsDstar ? MassDStar : MassD0);
  ...
  int mixingBin(CollType const& col)
  {
    switch (mixSetting.mixingBinPolicy) {
      case aod::femtodreamcollision::kMult:
        return colBinningMult.getBin({col.posZ(), col.multNtr()});
      case aod::femtodreamcollision::kMultPercentile:
        return colBinningMultPercentile.getBin({col.posZ(), col.multV0M()});
      case aod::femtodreamcollision::kMultMultPercentile:
        return colBinningMultMultPercentile.getBin({col.posZ(), col.multNtr(), col.multV0M()});
      default:
        return -1;
    }
  }
  ...
  void doMixedEvent(FilteredCollisions const& cols, D0Table const& d0Rows, OtherTable const& otherRows, Policy const& policy)
  {
    ...
    for (const auto& [collision1, collision2] : combinations(soa::CombinationsBlockFullIndexPolicy(policy, mixSetting.mixingDepth, -1, *partitionMaskedCol1.mFiltered, secondCollisions))) {
      if (collision1.globalIndex() == collision2.globalIndex()) {
        continue;
      }
      if constexpr (!IsDstar) {
        // Full policy returns both orientations for identical inputs. Count
        // once.
        if (collision1.globalIndex() > collision2.globalIndex()) {
          continue;
        }
      ...
      if constexpr (IsDstar) {
        registry.fill(HIST("MixingQA/hMECollisionBinsD0Dstar"), mixingBin(collision1));
      } else {
        registry.fill(HIST("MixingQA/hMECollisionBinsD0D0"), mixingBin(collision1));
      }
      ...
      for (const auto& [row1, row2] : combinations(soa::CombinationsFullIndexPolicy(first, second))) {
        float mass1{}, mass2{};
  ...
  void fillTables(CollType const& col, D0Slice const& d0s, DstarSlice const& dstars)
  {
    ...
    for (const auto& part : d0s) {
      ...
      rowFemtoResultCharm2Prong(col.globalIndex(), timeStamp, mass, part.pt(), part.eta(), part.phi(), part.prong0Id(), part.prong1Id(), part.charge(), part.bdtBkg(), part.bdtPrompt(), part.bdtFD(), 0, 0);
    }
    if constexpr (WithDstar) {
      for (const auto& part : dstars) {
        ...
        rowFemtoResultCharmDstar(col.globalIndex(), timeStamp, mass, part.mDaughD0(daughterMasses), part.pt(), part.eta(), part.phi(), part.prong0Id(), part.prong1Id(), part.prong2Id(), part.charge(), part.bdtBkg(), part.bdtPrompt(), part.bdtFD(), 0, 0);
      }
    }
  ...
  void runWithPolicy(FilteredCollisions const& cols, D0Table const& d0Rows, DstarTable const& dstarRows, Policy const& policy)
  {
    for (const auto& col : cols) {
      ...
      for (const auto& [row1, row2] : combinations(soa::CombinationsStrictlyUpperIndexPolicy(d0s, d0s))) {
        float mass1{}, mass2{};
      ...
      if constexpr (WithDstar) {
        auto dstars = dstarRows.sliceByCached(aod::femtodreamparticle::fdCollisionId, col.globalIndex(), cache);
        for (const auto& [row1, row2] : combinations(soa::CombinationsFullIndexPolicy(d0s, dstars))) {
          float mass1{}, mass2{};
...
{
  return WorkflowSpec{adaptAnalysisTask<HfTaskCharmHadronsCharmFemtoDream>(cfgc)};
}
