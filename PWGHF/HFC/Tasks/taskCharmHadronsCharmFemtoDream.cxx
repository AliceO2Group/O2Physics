...
struct HfTaskCharmHadronsCharmFemtoDream {
  ...
  Configurable<float> daughterMassMin{"daughterMassMin", 1.80f,
                                      "Minimum Dstar daughter D0 mass"};
  Configurable<float> daughterMassMax{"daughterMassMax", 1.93f,
                                      "Maximum Dstar daughter D0 mass"};
  Configurable<bool> useMl{"useMl", false,
                           "Require valid ML scores and apply score cuts"};
  Configurable<float> maxBkgD0{"maxBkgD0", 1.f, "Maximum D0 background score"};
  Configurable<float> minPromptD0{"minPromptD0", 0.f,
                                  "Minimum D0 prompt score"};
  Configurable<float> maxBkgDstar{"maxBkgDstar", 1.f,
  ...
  struct : ConfigurableGroup {
    ...
    Configurable<float> multPercentileMin{"multPercentileMin", 0.f,
                                          "Minimum multiplicity percentile"};
    Configurable<float> multPercentileMax{"multPercentileMax", 100.f,
                                          "Maximum multiplicity percentile"};
  } eventSel;
  ...
  void init(InitContext const&)
  {
    ...
    if (mixSetting.mixingDepth < 0 || mixSetting.mixingBinPolicy < 0 ||
        ...
        eventSel.multMin > eventSel.multMax ||
        eventSel.multPercentileMin > eventSel.multPercentileMax) {
    ...
    const AxisSpec ptStar{72, ptMinDstar.value, ptMaxDstar.value,
                          "Dstar pT (GeV/c)"};
    const AxisSpec mult{mixingBinMult, "NTracksPV"};
    const AxisSpec channel{static_cast<int>(NPairChannels), -0.5,
                           static_cast<double>(NPairChannels) - 0.5,
                           "0=D0D0 LS,1=D0barD0bar LS,2=D0D0bar US,3=D0D*+ "
                           "LS,4=D0barD*- LS,5=D0D*- US,6=D0barD*+ US"};
    ...
    registry.add("QA/events", ";0=in mixing range,1=outside;events", kTH1F,
                 {{2, -0.5, 1.5}});
    registry.add("MixingQA/hSECollisionBins", ";mixing bin;events", kTH1F,
                 {{1000, -0.5, 999.5}});
    registry.add("MixingQA/hMECollisionBinsD0D0", ";mixing bin;event pairs",
                 kTH1F, {{1000, -0.5, 999.5}});
    registry.add("MixingQA/hMECollisionBinsD0Dstar", ";mixing bin;event pairs",
                 kTH1F, {{1000, -0.5, 999.5}});
  ...
  static PairChannel pairChannel(D0Row const& d0, OtherRow const& other)
  {
    ...
    return d0.charge() > 0 ? (other.charge() > 0 ? D0DstarPlusLikeSign
                           ...
                           : (other.charge() < 0 ? D0barDstarMinusLikeSign
                ...
                float secondMass, float mult)
  {
    ...
    if constexpr (IsDstar && Mixed) {
      registry.fill(HIST("ME/D0Dstar"), kstar, mass1, mass2, pt1, pt2, mult,
                    channel);
    } else if constexpr (IsDstar) {
      registry.fill(HIST("SE/D0Dstar"), kstar, mass1, mass2, pt1, pt2, mult,
                    channel);
    } else if constexpr (Mixed) {
      registry.fill(HIST("ME/D0D0"), kstar, mass1, mass2, pt1, pt2, mult,
                    channel);
    } else {
      registry.fill(HIST("SE/D0D0"), kstar, mass1, mass2, pt1, pt2, mult,
                    channel);
    }
  }
                    ...
                    OtherTable const& otherRows, Policy const& policy)
  {
    ...
    for (auto const& [collision1, collision2] :
      ...
      if (!std::isfinite(collision1.magField()) ||
          collision1.magField() != collision2.magField()) {
        continue;
      }
      if constexpr (IsDstar) {
        registry.fill(HIST("MixingQA/hMECollisionBinsD0Dstar"),
                      mixingBin(policy, collision1));
      } else {
        registry.fill(HIST("MixingQA/hMECollisionBinsD0D0"),
                      mixingBin(policy, collision1));
      }
      ...
      for (auto const& [row1, row2] :
        ...
        if (select<false>(row1, mass1) && select<IsDstar>(row2, mass2)) {
          fillPair<true, IsDstar>(row1, row2, mass1, mass2,
                                  collision1.multNtr());
        }
      }
    }
                  ...
                  DstarSlice const& dstars)
  {
    ...
    for (auto const& part : d0s) {
      ...
      rowFemtoResultCharm2Prong(col.globalIndex(), timeStamp, mass, part.pt(),
                                part.eta(), part.phi(), part.prong0Id(),
                                part.prong1Id(), part.charge(), part.bdtBkg(),
    ...
    if (hasCandidate) {
      rowFemtoResultColl(col.globalIndex(), timeStamp, col.posZ(),
                         col.multNtr());
    }
  }
