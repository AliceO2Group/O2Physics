...
/// \file femtoDreamProducerDplus.cxx
...
struct HfFemtoDreamProducerDplus {
  ...
  // ML inference
  Configurable<int> applyMlMode{"applyMlMode", 1, "None: 0, BDT model from Dplus  selector: 1, New BDT model on Top of Dplus  selector: 2"};
    ...
    for (const auto& candidate : candidates) {
      ...
      if constexpr (useCharmMl) {
        ...
        if (applyMlMode == FillMlFromSelector) {
          ...
          if (candidate.mlProbDplusToPiKPi().size() > 0) {
            std::vector<float> inputFeaturesDplusToPiKPi = hfMlResponse.getInputFeatures(candidate);
            isSelectedMlDplusToPiKPi = hfMlResponse.isSelectedMl(inputFeaturesDplusToPiKPi, candidate.pt(), outputMlDplus);
          }
  ...
  PROCESS_SWITCH(HfFemtoDreamProducerDplus, processDataCharmHad,
  ...
  PROCESS_SWITCH(HfFemtoDreamProducerDplus, processDataCharmHadWithML,
  ...
  PROCESS_SWITCH(HfFemtoDreamProducerDplus, processMcCharmHad, "Provide Mc for charm hadron", false);
  ...
  PROCESS_SWITCH(HfFemtoDreamProducerDplus, processMcCharmHadWithML, "Provide Mc for charm hadron with ml", false);
  ...
  PROCESS_SWITCH(HfFemtoDreamProducerDplus, processMcCharmHadGen, "Provide Mc Generated charm hadron", false);
};

WorkflowSpec defineDataProcessing(ConfigContext const& cfgc)
{
  return WorkflowSpec{adaptAnalysisTask<HfFemtoDreamProducerDplus>(cfgc)};
}
