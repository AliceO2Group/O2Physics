...
struct HfCorrelatorD0Hadrons {
  ...
  Filter particlesFilter = nabs(aod::mcparticle::pdgCode) == static_cast<int>(o2::constants::physics::Pdg::kD0) || ((aod::mcparticle::flags & (uint8_t)o2::aod::mcparticle::enums::PhysicalPrimary) == (uint8_t)o2::aod::mcparticle::enums::PhysicalPrimary);
                    ...
                    soa::Join<aod::McParticles, aod::HfCand2ProngMcGen> const& mcParticles)
  {
    ...
    for (const auto& particle1 : mcParticles) {
      ...
      if (std::abs(particle1.pdgCode()) != o2::constants::physics::Pdg::kD0) { // just checking the particle PDG, not the decay channel (differently from Reco: you have a BR factor btw such levels!)
        continue;
      }
      ...
      for (const auto& particle2 : mcParticles) {
        ...
        auto indexMotherPi = RecoDecay::getMother(mcParticles, particle2, o2::constants::physics::Pdg::kDStar, true, nullptr, 1); // last arguement 1 is written to consider immediate decay mother only
        auto indexMotherD0 = RecoDecay::getMother(mcParticles, particle1, o2::constants::physics::Pdg::kDStar, true, nullptr, 1);
                             ...
                             SelectedTracks const& tracks)
  {
    ...
    for (const auto& [c1, tracks1, c2, tracks2] : pairData) {
      ...
      for (const auto& [t1, t2] : o2::soa::combinations(o2::soa::CombinationsFullIndexPolicy(tracks1, tracks2))) {
        ...
        if (yCandMax >= 0. && std::abs(hfHelper.yD0(t1)) > yCandMax) {
          continue;
        }
                              ...
                              SelectedTracksMcGen const& mcParticles)
  {
    ...
    for (const auto& [c1, tracks1, c2, tracks2] : pairMcGen) {
      for (const auto& [t1, t2] : o2::soa::combinations(o2::soa::CombinationsFullIndexPolicy(tracks1, tracks2))) {
        ...
        if (std::abs(t1.pdgCode()) != o2::constants::physics::Pdg::kD0) {
          continue;
        }
        ...
        auto indexMotherPi = RecoDecay::getMother(mcParticles, t2, o2::constants::physics::Pdg::kDStar, true, nullptr, 1); // last arguement 1 is written to consider immediate decay mother only
        auto indexMotherD0 = RecoDecay::getMother(mcParticles, t1, o2::constants::physics::Pdg::kDStar, true, nullptr, 1);
