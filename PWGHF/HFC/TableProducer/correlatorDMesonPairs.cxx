...
struct HfCorrelatorDMesonPairs {
  ...
  Configurable<bool> verbose{"--debug", false, "Enable debug mode"};
  ...
  void init(o2::framework::InitContext&)
  {
    ...
    // DPlus
    registry.add("hMassDPlus", "Dplus Pair candidates;inv. mass D0 only (#pi K) (GeV/#it{c}^{2});entries", {HistType::kTH2F, {{massAxisBins, massAxisMin, massAxisMax}, {vbins, "#it{p}_{T} (GeV/#it{c})"}}});
    registry.add("hMassDMinus", "Dplus Pair candidates;inv. mass D0bar only (#pi K) (GeV/#it{c}^{2});entries", {HistType::kTH2F, {{massAxisBins, massAxisMin, massAxisMax}, {vbins, "#it{p}_{T} (GeV/#it{c})"}}});
    ...
    registry.add("hMassDMinusMCRecSig", "DMinus signal candidates - MC reco;inv. mass D0bar only (#pi K) (GeV/#it{c}^{2});entries", {HistType::kTH2F, {{massAxisBins, massAxisMin, massAxisMax}, {vbins, "#it{p}_{T} (GeV/#it{c})"}}});
    registry.add("hMassDMinusMCRecBkg", "DMinus background candidates - MC reco;inv. mass D0bar only (#pi K) (GeV/#it{c}^{2});entries", {HistType::kTH2F, {{massAxisBins, massAxisMin, massAxisMax}, {vbins, "#it{p}_{T} (GeV/#it{c})"}}});
  ...
  {
    assert(particlePdgCode == pdg::Code::kD0); // Assure that we have the right PDG code
    ...
    for (auto& candidate1 : selectedD0CandidatesGrouped) {
      if(!KinematicCuts(candidate1)) continue;
      FillInfoPlots(candidate1, false);

      auto isWhat1 = AssignIsWhatData(candidate1, true); // Candidate type attribution.
      registry.fill(HIST("hSelectionStatus"), isWhat1);

      for (auto& candidate2 : selectedD0CandidatesGrouped) {
        if (!KinematicCuts(candidate2)) continue;
        ...
        auto isWhat2 = AssignIsWhatData(candidate2, false);         // Candidate type attribution

        // Register the pair only if we have two candidates that have passed all cuts
        if (isWhat1!=0 && isWhat2!=0) {
          entryD0Pair(getDeltaPhi(candidate2.phi(), candidate1.phi()),
                          ...
                          yD0(candidate1),
                          yD0(candidate2));
          entryD0PairRecoInfo(invMassD0ToPiK(candidate1),
                              invMassD0barToKPi(candidate2),
                              isWhat1,
                              isWhat2,
                              0, // unassigned (because we're in data)
                              0, // unassigned
                              0, // unassigned
                              0);// unassigned
        } // end if
  ...
  void processMcRecD0(aod::Collision const& collision, soa::Join<aod::Tracks, aod::TracksDCA>& tracks, soa::Join<aod::HfCand2Prong, aod::HfSelD0, aod::HfCand2ProngMcRec> const&)
  {
    ...
    for (auto& candidate1 : selectedD0CandidatesGroupedMC) {
      ...
      if (std::abs(candidate1.flagMcMatchRec()) == 1 << o2::aod::hf_cand_2prong::DecayType::D0ToPiK) {
        FillInfoPlots(candidate1, true);
      }

      auto isWhat1 = AssignIsWhatRec(candidate1, true);      // Candidate type attribution
      registry.fill(HIST("hSelectionStatusMCRec"), isWhat1);
      int8_t origin1 = 0, matchedRec1 = 0;
      if (isWhat1 < 100) { // if our event is not bkg
      ...
      for (auto& candidate2 : selectedD0CandidatesGroupedMC)
      {
        ...
        auto isWhat2 = AssignIsWhatRec(candidate2, false); // Candidate type attribution
  ...
  PROCESS_SWITCH(HfCorrelatorDMesonPairs, processMcRecD0, "Process D0 MC Rec mode", false);
  ...
  {
    assert(particlePdgCode == pdg::Code::kDPlus); // Assure that we have the right PDG code
    AnalyseMultiplicity(collision, tracks);
    ...
    for (auto& candidate1 : selectedDPlusCandidatesGrouped) {
        ...
        uint isWhat1 = AssignIsWhatDataDPlus(candidate1, mDPlus, outerParticleSign, true);
        FillInfoPlots(candidate1, false);
        ...
        for (auto& candidate2 : selectedDPlusCandidatesGrouped) {
          ...
          uint isWhat2 = AssignIsWhatDataDPlus(candidate2, mDPlus, innerParticleSign, false);
          ...
          if (isWhat1!=0 && isWhat2!=0) {
            entryDPlusPair(getDeltaPhi(candidate2.phi(), candidate1.phi()),
                            ...
                            yDplus(candidate1),
                            yDplus(candidate2));
            entryDPlusPairRecoInfo(invMassDplusToPiKPi(candidate1),
  ...
  void processMcRecDPlus(aod::Collision const& collision, soa::Join<aod::Tracks, aod::TracksDCA>& tracks, soa::Join<aod::HfCand3Prong, aod::HfSelDplusToPiKPi, aod::HfCand3ProngMcRec> const&, aod::BigTracks const&)
  {
    ...
    for (auto& candidate1 : selectedDPlusCandidatesGroupedMC) {
        ...
        double mDPlus = invMassDplusToPiKPi(candidate1);
        uint isWhat1 = AssignIsWhatRecoDPlus(candidate1, mDPlus, outerParticleSign, true);
        ...
        for (auto& candidate2 : selectedDPlusCandidatesGroupedMC)
        {
          ...
          uint isWhat2 = AssignIsWhatRecoDPlus(candidate2, mDPlus, innerParticleSign, false);
          int8_t origin2 = 0, matchedRec2 = 0;
          // if our event is not bkg
          if (isWhat2 < 100) {
            // check if it's prompt or non-prompt
  ...
  PROCESS_SWITCH(HfCorrelatorDMesonPairs, processMcRecDPlus, "Process DPlus MC Rec", false);
  ...
  void AnalyseMcGen(const T&  particlesMC)
  {
    ...
    for (auto& particle1 : particlesMC)
    {
      ...
      if (!KinematicCutsGen(particle1)) continue;
      ...
      auto isWhat1 = AssignIsWhatGen(particle1);      // Candidate sign attribution. 
      ...
      for (auto& particle2 : particlesMC)
      {
        ...
        auto isWhat2 = AssignIsWhatGen(particle2);
        if (!KinematicCutsGen(particle2)) continue;
  ...
  template <typename T>
  void FillInfoPlots(const T& candidate, bool const& isReco) {
    if (isReco) {
  ...
  template <typename T>
  uint AssignIsWhatData(const T& candidate, bool const& fillMassHists)
  {
    uint isWhat = 0;
    if (candidate.isSelD0() >= selectionFlagD0) { 
      if(fillMassHists) {
        if (verbose) {std::cout << "D0 Mass Cand1: " << invMassD0ToPiK(candidate) << std::endl;}
    ...
    if (candidate.isSelD0bar() >= selectionFlagD0bar) {    // only reco as D0bar
      if (fillMassHists) {
        if (verbose) {std::cout << "D0bar Mass Cand1: " << invMassD0barToKPi(candidate) << std::endl;}
    ...
    if (verbose) {
        // isWhat = 0 -> no candidate, isWhat = 1 -> D0, isWhat1 = 2 -> D0bar, isWhat = 3 -> both selections passed
        std::cout << "isWhat: " << isWhat << std::endl;
        std::cout << " ----------------------------- " << std::endl;
    }
    return isWhat;
  }
  ...
  template <typename T>
  uint AssignIsWhatDataDPlus(const T& candidate, double const& mDPlus, int const& particleSign, bool const& fillMassHists)
  {
    uint isWhat = 0;
    if (particleSign == 1) {
      if (fillMassHists) {
        if (verbose) {std::cout << "DPlus Mass Cand1: " << mDPlus << std::endl;}
      ...
      if (fillMassHists) {
        if (verbose) {std::cout << "DMinus Mass Cand1: " << mDPlus << std::endl;}
        registry.fill(HIST("hMass"), mDPlus, candidate.pt());
  ...
  template <typename T>
  uint AssignIsWhatRecoDPlus(const T& candidate, double const& mDPlus, int const& particleSign, bool const& fillMassHists)
  {
    uint isWhat = 0;
  ...
  template <typename T>
  uint AssignIsWhatRec(const T& candidate, bool const& fillMassHists)
  {
    uint isWhat = 0;
    if (candidate.isSelD0() >= selectionFlagD0) {  
  ...
  template <typename T>
  uint AssignIsWhatGen(const T& candidate)
  {
    uint isWhat = 0;
