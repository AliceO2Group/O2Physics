...
struct HfProducerCharmHadronsCharmFemtoDream {
  ...
  void initMl(MlConfig const& cfg, Response& response, bool withMl)
  {
    ...
    if (edges.size() < 2 ||
      ...
      LOGP(fatal, "{}: binsPtMl must be finite and strictly increasing",
           cfg.prefix);
    }
    ...
    if (cfg.nClassesMl.value != 3 || cfg.cutDirMl.value.size() != 3 ||
        ...
        (cfg.loadModelsFromCCDB.value &&
         cfg.modelPathsCCDB.value.size() != nBins)) {
      LOGP(fatal,
           "{}: provide three classes, cuts/models for every pT bin, and input "
           "features",
           cfg.prefix);
    }
    ...
    response.configure(edges, cfg.cutsMl.value, cfg.cutDirMl.value,
                       cfg.nClassesMl.value);
    response.cacheInputFeaturesIndices(cfg.namesInputFeatures.value);
    if (cfg.loadModelsFromCCDB.value) {
      ccdbApi.init(ccdbUrl.value);
      response.setModelPathsCCDB(cfg.onnxFileNames.value, ccdbApi,
                                 cfg.modelPathsCCDB.value,
                                 cfg.timestampCCDB.value);
    } else {
  ...
  void init(InitContext const&)
  {
    ...
    const bool withMl = doprocessD0D0Ml || doprocessD0DstarMl ||
                        doprocessD0D0MlWithFT0C || doprocessD0DstarMlWithFT0C;
    initMl(mlD0, hfMlResponseD0, withMl);
    if (doprocessD0Dstar || doprocessD0DstarMl || doprocessD0DstarWithFT0C ||
        doprocessD0DstarMlWithFT0C) {
      ...
      for (int hypothesis = 0; hypothesis < 2; ++hypothesis) {
        ...
        if constexpr (WithMl) {
          ...
          if (mlD0.applyMlMode == FillMlFromNewBDT) {
            ...
            if (!std::isfinite(cand.pt()) ||
                cand.pt() < mlD0.binsPtMl.value.front() ||
                cand.pt() >= mlD0.binsPtMl.value.back()) {
              continue;
            }
    ...
    for (auto const& cand : candidates) {
      ...
      if constexpr (WithMl) {
        ...
        if (mlDstar.applyMlMode == FillMlFromNewBDT) {
          if (!std::isfinite(cand.pt()) ||
              cand.pt() < mlDstar.binsPtMl.value.front() ||
              cand.pt() >= mlDstar.binsPtMl.value.back()) {
            continue;
          }
