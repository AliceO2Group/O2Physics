...
struct HfProducerCharmHadronsCharmFemtoDream {
  ...
  struct HfMlConfig : ConfigurableGroup {
    ...
    Configurable<std::vector<double>> binsPtMl{"binsPtMl", hf_cuts_ml::vecBinsPt, "pT bin limits for new BDT"};
    ...
    Configurable<std::vector<int>> cutDirMl{"cutDirMl", hf_cuts_ml::vecCutDir, "Reject scores above (0), below (1), or do not cut (2)"};
  ...
  using Dstars = soa::Join<aod::HfCandDstars, aod::HfD0FromDstar, aod::HfSelDstarToD0Pi>;
  ...
  void initMl(HfMlConfig const& cfg, Response& response, bool withMl)
  {
    ...
    if (cfg.nClassesMl.value != NMlClasses || cfg.cutDirMl.value.size() != NMlClasses || cfg.cutsMl.value.rows() != nBins || cfg.cutsMl.value.cols() != NMlClasses || cfg.onnxFileNames.value.size() != nBins || cfg.namesInputFeatures.value.empty() || (cfg.loadModelsFromCCDB.value && cfg.modelPathsCCDB.value.size() != nBins)) {
  ...
  void init(InitContext const&)
  {
    ...
    const AxisSpec candidateSpecies{4, -0.5, 3.5, "species (0=D0,1=D0bar,2=Dstar+,3=Dstar-)"};
  ...
  bool acceptCollision(CollType const& col)
  {
    ...
    const auto rejectionMask = hfEvSel.getHfCollisionRejectionMask<true, CentEstimator, aod::BCsWithTimestamps>(col, cent, ccdb, registry);
    ...
    const float fieldTesla = 0.1f * o2::base::Propagator::Instance()->getNominalBz();
  ...
  std::array<float, 3> readScores(Scores const& scores)
  {
    if (scores.size() != NMlClasses) {
      LOGP(fatal, "Expected three selector ML scores: background, prompt, nonprompt");
      return {-1.f, -1.f, -1.f};
    }
