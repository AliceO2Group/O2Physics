...
auto efficiencyLc_v = std::vector<double>{efficiencyLcDefault, efficiencyLcDefault + nBinsPtMassAndEfficiency};
...
// Code to select a Lc in a collision
struct HfLcSelection {
  ...
  PROCESS_SWITCH(HfLcSelection, processLcSelectionData, "Process Lc Selection Data", true);
                               ...
                               soa::Join<aod::HfCand3Prong, aod::HfSelLc, aod::HfCand3ProngMcRec> const& candidates)
  {
    ...
    if (selectedLcCandidatesMc.size() > 0) {
      auto selectedLcCandidatesMcGrouped = selectedLcCandidatesMc->sliceByCached(aod::hf_cand::collisionId, collision.globalIndex(), cache);
      for (const auto& candidate : selectedLcCandidatesMcGrouped) {
  ...
  PROCESS_SWITCH(HfLcSelection, processLcSelectionMcRec, "Process Lc Selection McRec", false);
  ...
  PROCESS_SWITCH(HfLcSelection, processLcSelectionMcGen, "Process Lc Selection McGen", false);
};
...
struct HfCorrelatorLcHadrons {
  ...
  HistogramRegistry registry{
    ...
    {{"hPtCand", "Lc,Hadron candidates;candidate #it{p}_{T} (GeV/#it{c});entries", {HistType::kTH1F, {{ptLcAxisBins, ptLcAxisMin, ptLcAxisMax}}}},
     ...
     {"hLcBin", "Lc selected in pool Bin;pool Bin;entries", {HistType::kTH1F, {{9, 0., 9.}}}},
                   ...
                   soa::Join<aod::HfCand3Prong, aod::HfSelLc> const& candidates)
  {
    ...
    for (const auto& candidate : selectedLcCandidatesGrouped) {
      ...
      registry.fill(HIST("hPhi"), RecoDecay::constrainAngle(candidate.phi(), -o2::constants::math::PI / 2.));
                    ...
                    soa::Join<aod::HfCand3Prong, aod::HfSelLc, aod::HfCand3ProngMcRec> const& candidates)
  {
    ...
    for (const auto& candidate : selectedLcCandidatesMcGrouped) {
      ...
      if (isLcSignal) {
        ...
        registry.fill(HIST("hPhiMcRec"), RecoDecay::constrainAngle(candidate.phi(), -o2::constants::math::PI / 2.));
                    ...
                    soa::Join<aod::McParticles, aod::HfCand3ProngMcGen> const& mcParticles)
  {
    ...
    for (const auto& particle : mcParticles) {
      ...
      if (std::abs(particle.flagMcMatchGen()) == 1 << aod::hf_cand_3prong::DecayType::LcToPKPi) {
        ...
        registry.fill(HIST("hPhiMcGen"), RecoDecay::constrainAngle(particle.phi(), -o2::constants::math::PI / 2.));
  ...
  // Event Mixing for the Data Model
  using myCollisions = soa::Join<aod::Collisions, aod::Mults, aod::LcSelection>;
  using mySelCollisions = soa::Filtered<myCollisions>;
  using myTracks = soa::Filtered<aod::TracksWDca>;
  using myCandidatesData = soa::Filtered<soa::Join<aod::HfCand3Prong, aod::HfSelLc>>;
                             ...
                             myTracks const& tracks)
  {
    ...
    for (const auto& [c1, tracks1, c2, tracks2] : pairData) {
      ...
      for (const auto& [t1, t2] : o2::soa::combinations(o2::soa::CombinationsFullIndexPolicy(tracks1, tracks2))) {
        if (!(t1.hfflag() & 1 << aod::hf_cand_3prong::DecayType::LcToPKPi)) {
          continue;
        }
  ...
  using myCandidatesMcRec = soa::Filtered<soa::Join<aod::HfCand3Prong, aod::HfSelLc, aod::HfCand3ProngMcRec>>;
  ...
  using myCollisionsMcGen = soa::Join<aod::McCollisions, aod::LcSelection>;
  using mySelCollisionsMcGen = soa::Filtered<myCollisionsMcGen>;
  using myParticlesMcGen = soa::Filtered<aod::McParticles>;
  ...
  void processMcGenMixedEvent(mySelCollisionsMcGen const& collisions,
                              myParticlesMcGen const& mcParticles)
  {
    auto getTracksSize = [&mcParticles, this](mySelCollisionsMcGen::iterator const& collision) {
      int nTracks = 0;
    ...
    Pair<mySelCollisionsMcGen, myParticlesMcGen, myParticlesMcGen, BinningTypeMcGen> pairMcGen{corrBinningMcGen, 5, -1, collisions, tracksTuple, &cache};
...
{
  return WorkflowSpec{adaptAnalysisTask<HfLcSelection>(cfgc),
