...
struct TauTau13topo {
  ...
  void processDataSG(UDCollisionFull2 const& dgcand, UDTracksFull const& dgtracks, PVTracks const& PVContributors)
  {
    ...
    // //
    // // data; good electron track cut 36
    // //
    // if (flagEl[0] * trkHasTof[0] * flagPi[0] * flagPr[0] * flagKa[0] * flagCR[0] * !flagVcalPV[0] * flagS3pi[0] * flagIM[0] * (otherTOFtracks[0] >= 1) * flagDP[0] * trkIsGood[0] +
    // 	flagEl[1] * trkHasTof[1] * flagPi[1] * flagPr[1] * flagKa[1] * flagCR[1] * !flagVcalPV[1] * flagS3pi[1] * flagIM[1] * (otherTOFtracks[1] >= 1) * flagDP[1] * trkIsGood[1] +
    // 	flagEl[2] * trkHasTof[2] * flagPi[2] * flagPr[2] * flagKa[2] * flagCR[2] * !flagVcalPV[2] * flagS3pi[2] * flagIM[2] * (otherTOFtracks[2] >= 1) * flagDP[2] * trkIsGood[2] +
    // 	flagEl[3] * trkHasTof[3] * flagPi[3] * flagPr[3] * flagKa[3] * flagCR[3] * !flagVcalPV[3] * flagS3pi[3] * flagIM[3] * (otherTOFtracks[3] >= 1) * flagDP[3] * trkIsGood[3] >
    //     0) { // good electron track cut
    //   registry.get<TH1>(HIST("global/hEventEff"))->Fill(24., 1.);
    //   registry.get<TH1>(HIST("control/cut36/h4trkPtTot"))->Fill(pttot);
    //   registry.get<TH1>(HIST("control/cut36/h4piMass"))->Fill(mass4pi);
    //   registry.get<TH2>(HIST("control/cut36/h4trkMassVsPt"))->Fill(mass4pi, pttot);
    //   registry.get<TH1>(HIST("control/cut36/hNtofTrk"))->Fill(nTofTrk);
    //   registry.get<TH2>(HIST("control/cut36/hZNACenergy"))->Fill(energyZNA, energyZNC);
    //   for (int i = 0; i < 4; i++) {
    //     if (flagEl[i] && trkHasTof[i] && flagPi[i] && flagPr[i] && flagKa[i] && flagCR[i] && !flagVcalPV[i] && flagS3pi[i] && flagIM[i] && (otherTOFtracks[i] >= 1) && flagDP[i] && trkIsGood[i]) {
    //       registry.get<TH2>(HIST("pidTPC/hpvsdedxElHipCut36"))->Fill(tmpMomentum[i], tmpDedx[i]);
    //       registry.get<TH2>(HIST("pidTOF/hpvsNsigmaElHipCut36"))->Fill(tmpMomentum[i], tmpTofNsigmaEl[i]);
    //       // for (int j = 0; j < 4; j++) {
    //       //   if (i == j) continue;
    //       //   registry.get<TH2>(HIST("pidTPC3pi/hpvsdedxElHipCut27"))->Fill(tmpMomentum[j], tmpDedx[j]);
    //       // }
    //       fillControlHistos<36>(pi3invMass[i], pi3pt[i], pi3deltaPhi[i], pi3assymav[i], pi3vector[i], pi3scalar[i], nclTPCcrossedRows[i], tmpPt[i], chi2TOF[i]);
    //       registry.get<TH2>(HIST("control/cut36/h3piMassVsPt"))->Fill(pi3invMass[i], pi3pt[i]);
    //       registry.get<TH1>(HIST("control/cut36/hsigma3Pi"))->Fill(nSigma3Pi[i]);
    //       registry.get<TH1>(HIST("control/cut36/h3pi1eMass"))->Fill(mass3pi1e[i]);
    //     } else if (!flagEl[i] && trkHasTof[i]) {
    //       registry.get<TH1>(HIST("pidTOF/h3piTOFchi2Cut36"))->Fill(chi2TOF[i]);
    //       // LOGF(info, "<tautau13topo> chi2TOF %f", chi2TOF[i]);
    //     }
    //   }
    // } else {
    //   if (verbose) {
    //     LOGF(debug, "<tautau13topo> Candidate rejected: all electrons vetoed by trkIsGood");
    //   }
    //   return;
    // } // end of good electron track cut 36

    // //
    // // good TOF track with chi2 cut 37
    // //
    // if (flagEl[0] * trkHasTof[0] * flagPi[0] * flagPr[0] * flagKa[0] * flagCR[0] * !flagVcalPV[0] * flagS3pi[0] * flagIM[0] * (otherTOFtracks[0] >= 1) * flagDP[0] * trkIsTOFGood[0] +
    // 	flagEl[1] * trkHasTof[1] * flagPi[1] * flagPr[1] * flagKa[1] * flagCR[1] * !flagVcalPV[1] * flagS3pi[1] * flagIM[1] * (otherTOFtracks[1] >= 1) * flagDP[1] * trkIsTOFGood[1] +
    // 	flagEl[2] * trkHasTof[2] * flagPi[2] * flagPr[2] * flagKa[2] * flagCR[2] * !flagVcalPV[2] * flagS3pi[2] * flagIM[2] * (otherTOFtracks[2] >= 1) * flagDP[2] * trkIsTOFGood[2] +
    // 	flagEl[3] * trkHasTof[3] * flagPi[3] * flagPr[3] * flagKa[3] * flagCR[3] * !flagVcalPV[3] * flagS3pi[3] * flagIM[3] * (otherTOFtracks[3] >= 1) * flagDP[3] * trkIsTOFGood[3] >
    //     0) {
    //   registry.get<TH1>(HIST("global/hEventEff"))->Fill(25., 1.);
    //   registry.get<TH1>(HIST("control/cut37/h4trkPtTot"))->Fill(pttot);
    //   registry.get<TH1>(HIST("control/cut37/h4piMass"))->Fill(mass4pi);
    //   registry.get<TH2>(HIST("control/cut37/h4trkMassVsPt"))->Fill(mass4pi, pttot);
    //   registry.get<TH1>(HIST("control/cut37/hNtofTrk"))->Fill(nTofTrk);
    //   registry.get<TH2>(HIST("control/cut37/hZNACenergy"))->Fill(energyZNA, energyZNC);
    //   for (int i = 0; i < 4; i++) {
    //     if (flagEl[i] && trkHasTof[i] && flagPi[i] && flagPr[i] && flagKa[i] && flagCR[i] && !flagVcalPV[i] && flagS3pi[i] && flagIM[i] && (otherTOFtracks[i] >= 1) && flagDP[i] && trkIsTOFGood[i]) {
    //       registry.get<TH2>(HIST("pidTPC/hpvsdedxElHipCut37"))->Fill(tmpMomentum[i], tmpDedx[i]);
    //       registry.get<TH2>(HIST("pidTOF/hpvsNsigmaElHipCut37"))->Fill(tmpMomentum[i], tmpTofNsigmaEl[i]);
    //       // for (int j = 0; j < 4; j++) {
    //       //   if (i == j) continue;
    //       //   registry.get<TH2>(HIST("pidTPC3pi/hpvsdedxElHipCut27"))->Fill(tmpMomentum[j], tmpDedx[j]);
    //       // }
    //       fillControlHistos<37>(pi3invMass[i], pi3pt[i], pi3deltaPhi[i], pi3assymav[i], pi3vector[i], pi3scalar[i], nclTPCcrossedRows[i], tmpPt[i], chi2TOF[i]);
    //       registry.get<TH2>(HIST("control/cut37/h3piMassVsPt"))->Fill(pi3invMass[i], pi3pt[i]);
    //       registry.get<TH1>(HIST("control/cut37/hsigma3Pi"))->Fill(nSigma3Pi[i]);
    //       registry.get<TH1>(HIST("control/cut37/h3pi1eMass"))->Fill(mass3pi1e[i]);
    //     } else if (!flagEl[i] && trkHasTof[i]) {
    //       registry.get<TH1>(HIST("pidTOF/h3piTOFchi2Cut37"))->Fill(chi2TOF[i]);
    //       // LOGF(info, "<tautau13topo> chi2TOF %f", chi2TOF[i]);
    //     }
    //   }
    // } else {
    //   if (verbose) {
    //     LOGF(debug, "<tautau13topo> Candidate rejected: all electrons vetoed by isrkTOFGood");
    //   }
    //   return;
    // } // end of good TOF track
