...
struct upcRhoPrimeAnalysis {
    ...
    void process(UDCollisions::iterator const& collision, UDtracks const& tracks)
    {
        ...
        if (collision.vtxITSTPC() != vtxITSTPCcut) return;
        registry.fill(HIST("Events/Flow"), 1);
        
        if (collision.sbp() != sbpCut) return;
        registry.fill(HIST("Events/Flow"), 2);
        
        if (collision.itsROFb() != itsROFbCut) return;
        registry.fill(HIST("Events/Flow"), 3);
        
        if (collision.tfb() != tfbCut) return;
        registry.fill(HIST("Events/Flow"), 4);
        
        if (specifyGapSide && collision.gapSide() != gapSide) return;
        if (collision.totalFV0AmplitudeA() > fv0Cut) return;
        if (collision.totalFT0AmplitudeA() > ft0aCut) return;
        if (collision.totalFT0AmplitudeC() > ft0cCut) return;
        if (collision.energyCommonZNA() > zdcCut || collision.energyCommonZNC() > zdcCut) return;
        registry.fill(HIST("Events/Flow"), 5);
        
        if (collision.numContrib() != numPVContrib) return;
        ...
        for (const auto& track : tracks) {
            ...
            if (posPions.size() == 2 && negPions.size() == 2) break;
        }
        ...
        if (fourPionSystem.M() < systemMassMinCut || fourPionSystem.M() > systemMassMaxCut) return;
        if (fourPionSystem.Pt() > systemPtCut) return;
        if (std::abs(fourPionSystem.Rapidity()) > systemYCut) return;
