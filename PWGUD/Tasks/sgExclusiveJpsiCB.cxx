...
struct sgExclusiveJpsiCB {
  ...
  Configurable<float> fv0Cut{"fv0", 100., "fv0aThreshold"};
  Configurable<float> ft0aCut{"ft0a", 100., "ft0aThreshold"};
  Configurable<float> ft0cCut{"ft0c", 50., "ft0cThreshold"};
  Configurable<float> fddaCut{"fdda", 10000., "fddaThreshold"};
  Configurable<float> fddcCut{"fddc", 10000., "fddcThreshold"};
  Configurable<float> zdcCut{"zdc", 10., "zdcThreshold"};
  Configurable<float> gapSide{"gap", 2, "gapSelection"};

  // Track Selections
  Configurable<float> pvCut{"PV_cut", 1.0, "Use Only PV tracks"};
  Configurable<float> dcazCut{"dcaZ_cut", 2.0, "dcaZ cut"};
  Configurable<float> dcaxyCut{"dcaXY_cut", 0.0, "dcaXY cut (0 for Pt-function)"};
  Configurable<float> tpcChi2Cut{"tpcChi2_cut", 4, "Max tpcChi2NCl"};
  Configurable<float> tpcNClsFindableCut{"tpcNClsFindable_cut", 70, "Min tpcNClsFindable"};
  Configurable<float> itsChi2Cut{"itsChi2_cut", 36, "Max itsChi2NCl"};
  Configurable<float> etaCut{"etaCut", 0.9, "Track Pseudorapidity"};
  Configurable<float> ptCut{"pt_cut", 0.01, "Track Pt"};
  Configurable<float> tpcCluster{"TPC_cluster", 50, "No.of TPC cluster"};
...
{
  return WorkflowSpec{
    adaptAnalysisTask<sgExclusiveJpsiCB>(cfgc)};
}
