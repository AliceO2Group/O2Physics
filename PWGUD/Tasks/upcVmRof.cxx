...
struct upcVmRof {
  ...
  int getRof(int64_t thisBC)
  {
    ...
    if (bctmp < 0)
      bctmp = o2::constants::lhc::LHCMaxBunches - kRofShift - 1;
  ...
  void addBcHistos(int run)
  {
    ...
    bcTH2Pointers[Form("bc/%d/ft0Vtx_H", run)] = bcTH2Registry.add<TH2>(Form("bc/%d/ft0Vtx_H", run), "ft0Vtx triggers; TF; ROF; Counter",
                                                                        {HistType::kTH2F, {{static_cast<int>(nTF / tfPerBin), -0.5, static_cast<double>(nTF) - 0.5}, {kRofPerOrbit, -0.5, kRofPerOrbit - 0.5}}});
