...
struct SGPIDSpectraTable {
  ...
  void process(UDCollisionFull const& coll, UDTracksFull const& tracks)
  {
    ...
    //check occupancies:
    if (occ_bit1_cut && !coll.trs()) return;
    if (occ_bit2_cut && !coll.trofs()) return;
    if (occ_bit3_cut && !coll.hmpr()) return;    
    if (coll.occupancyInTime() > occ_cut) return;
    if (coll.hadronicRate() > ir_cut) return; 
