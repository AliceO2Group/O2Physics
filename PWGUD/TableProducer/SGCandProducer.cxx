...
struct SGCandProducer {
  ...
  //    LOGF(info, "Current BC: %i, %i, %i", bc.globalBC(), newbc.globalBC(), issgevent);
	 ...
	 if (track.isPVContributor() || saveAllTracks){
	    if( track.itsClusterSizes() && track.itsChi2NCl() > 0 && ((track.tpcNClsFindable()==0 && savenonPVCITSOnlyTracks) || track.tpcNClsFindable()>50) && track.pt()>sameCuts.minPt() && track.eta() > sameCuts.minEta() && track.eta() < sameCuts.maxEta())  updateUDTrackTables(outputCollisions.lastIndex(), track, bc.globalBC());
