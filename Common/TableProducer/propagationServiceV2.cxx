...
/// \brief V2: every conditions object sourced from declarative CCDB tables, so the task
///        itself performs no CCDB query.
/// \author ALICE
...
struct propagationServiceV2 {
  // No CCDB client of any kind: every conditions object this task consumes — magnetic
  // field, mean vertex, material LUT, TPC drift correction and the two TrackTuner
  // calibrations — arrives as a declarative CCDB column, so the task issues no query.
  //
  // No ConfigurableCCDBPath is declared either: ArrowSupport already registers a
  // "ccdb:fXxx" option per declared column on the CCDB fetcher device, defaulting to the
  // column's own query string. A task-side ConfigurableCCDBPath would only re-supply that
  // identical default. Override a path with --ccdb:fMatLUT (and friends).
