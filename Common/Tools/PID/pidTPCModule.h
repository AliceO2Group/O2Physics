...
class pidTPCModule
{
 public:
  ...
  std::vector<float> createNetworkPrediction(TCCDB& ccdb, TCCDBApi& ccdbApi, soa::Join<aod::Collisions, aod::EvSels> const& collisions, M const& mults, T const& tracks, B const& bcs, const size_t size)
  {
    ...
    constexpr int kExpectedInputDimensionsNNV2 = 7;
    constexpr int kExpectedInputDimensionsNNV3 = 8;
    constexpr auto kNetworkVersionV2 = "2";
    constexpr auto kNetworkVersionV3 = "3";
    for (int i = 0; i < NParticleTypes; i++) { // Loop over particle number for which network correction is used
      for (auto const& trk : tracks) {
        ...
        if (input_dimensions == kExpectedInputDimensionsNNV2 && networkVersion == kNetworkVersionV2) {
        ...
        if (input_dimensions == kExpectedInputDimensionsNNV3 && networkVersion == kNetworkVersionV3) {
          track_properties[counter_track_props + 6] = trk.has_collision() ? collisions.iteratorAt(trk.collisionId()).ft0cOccupancyInTimeRange() / 60000. : 1.;
  ...
  void makePidTables(const int flagFull, NSF& tableFull, const int flagTiny, NST& tableTiny, const o2::track::PID::ID pid, const float tpcSignal, const T& trk, const long multTPC, const std::vector<float>& network_prediction, const int& count_tracks, const int& tracksForNet_size)
  {
    ...
    constexpr int kNumOutputNodesSymmetricSigma = 2;
    constexpr int kNumOutputNodesAsymmetricSigma = 3;
    if (pidTPCopts.useNetworkCorrection && speciesNetworkFlags[pid] && trk.has_collision() && bg > pidTPCopts.networkBetaGammaCutoff) {
      ...
      } else if (network.getNumOutputNodes() == kNumOutputNodesSymmetricSigma) { // Symmetric sigma correction
        expSigma = (network_prediction[kNumOutputNodesSymmetricSigma * (count_tracks + tracksForNet_size * pid) + 1] - network_prediction[kNumOutputNodesSymmetricSigma * (count_tracks + tracksForNet_size * pid)]) * expSignal;
        nSigma = (tpcSignal / expSignal - network_prediction[kNumOutputNodesSymmetricSigma * (count_tracks + tracksForNet_size * pid)]) / (network_prediction[kNumOutputNodesSymmetricSigma * (count_tracks + tracksForNet_size * pid) + 1] - network_prediction[kNumOutputNodesSymmetricSigma * (count_tracks + tracksForNet_size * pid)]);
      } else if (network.getNumOutputNodes() == kNumOutputNodesAsymmetricSigma) { // Asymmetric sigma corection
        if (tpcSignal / expSignal >= network_prediction[kNumOutputNodesAsymmetricSigma * (count_tracks + tracksForNet_size * pid)]) {
          expSigma = (network_prediction[kNumOutputNodesAsymmetricSigma * (count_tracks + tracksForNet_size * pid) + 1] - network_prediction[kNumOutputNodesAsymmetricSigma * (count_tracks + tracksForNet_size * pid)]) * expSignal;
          nSigma = (tpcSignal / expSignal - network_prediction[kNumOutputNodesAsymmetricSigma * (count_tracks + tracksForNet_size * pid)]) / (network_prediction[kNumOutputNodesAsymmetricSigma * (count_tracks + tracksForNet_size * pid) + 1] - network_prediction[kNumOutputNodesAsymmetricSigma * (count_tracks + tracksForNet_size * pid)]);
        } else {
          expSigma = (network_prediction[kNumOutputNodesAsymmetricSigma * (count_tracks + tracksForNet_size * pid)] - network_prediction[kNumOutputNodesAsymmetricSigma * (count_tracks + tracksForNet_size * pid) + 2]) * expSignal;
          nSigma = (tpcSignal / expSignal - network_prediction[kNumOutputNodesAsymmetricSigma * (count_tracks + tracksForNet_size * pid)]) / (network_prediction[kNumOutputNodesAsymmetricSigma * (count_tracks + tracksForNet_size * pid)] - network_prediction[kNumOutputNodesAsymmetricSigma * (count_tracks + tracksForNet_size * pid) + 2]);
        }
      } else {
