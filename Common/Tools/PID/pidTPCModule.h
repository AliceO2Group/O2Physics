...
class pidTPCModule
{
 public:
  ...
  //__________________________________________________
  template <typename TCCDB, typename C, typename M, typename T, typename B>
  std::vector<float> createNetworkPrediction(TCCDB& ccdb, C const& collisions, M const& mults, T const& tracks, B const& bcs, const size_t size)
  {
  ...
  //__________________________________________________
  template <typename TCCDB, typename TBCs, typename TCollisions, typename TTracks, typename TTracksQA, typename TProducts>
  void process(TCCDB& ccdb, TBCs const& bcs, TCollisions const& cols, TTracks const& tracks, TTracksQA const& tracksQA, TProducts& products)
  {
    if (tracks.size() == 0) {
