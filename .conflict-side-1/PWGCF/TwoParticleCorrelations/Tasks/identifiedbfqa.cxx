...
struct IdentifiedBfFilterQA {
  Configurable<std::string> cfgDataType{"datatype", "data", "Data type: data, MC, FastMC, OnTheFlyMC. Default data"};
  HistogramRegistry histos{"IdentifiedBfFilterQA", {}, OutputObjHandlingPolicy::AnalysisObject, true, true};
  o2::analysis::identifiedbffilter::DataType datatype;

  template <o2::analysis::identifiedbffilterqa::innerdatatype dir>
  void createHistograms()
  {

    using namespace o2::analysis::identifiedbffilterqa;
  ...
  template <o2::analysis::identifiedbffilterqa::innerdatatype dir, typename FilteredCollision, typename FilteredTracks>
  ...
  Filter onlyacceptedtracks = (int8_t(0) <= aod::identifiedbffilter::trackacceptedid);

  void processGeneratorLevel(soa::Filtered<aod::IdentifiedBfCFAcceptedTrueCollisions>::iterator const& collision, soa::Filtered<aod::ScannedTrueTracks> const& tracks)
  {
    using namespace o2::analysis::identifiedbffilterqa;
    LOGF(IDENTIFIEDBFFILTERLOGCOLLISIONS, "New filtered generated collision with BC id %d and with %d accepted tracks", collision.bcId(), tracks.size());
    processQATask<kGEN>(collision, tracks);
  }
  PROCESS_SWITCH(IdentifiedBfFilterQA, processGeneratorLevel, "Process generator level filter task QA", true);

  void processDetectorLevel(soa::Filtered<aod::IdentifiedBfCFAcceptedCollisions>::iterator const& collision, soa::Filtered<aod::ScannedTracks> const& tracks)
  {
    using namespace o2::analysis::identifiedbffilterqa;
    LOGF(IDENTIFIEDBFFILTERLOGCOLLISIONS, "New filtered collision with BC id %d and with %d accepted tracks", collision.bcId(), tracks.size());
    processQATask<kRECO>(collision, tracks);
  }
  PROCESS_SWITCH(IdentifiedBfFilterQA, processDetectorLevel, "Process detector level filter task QA", true);
};

WorkflowSpec defineDataProcessing(ConfigContext const& cfgc)
{
  WorkflowSpec workflow{adaptAnalysisTask<IdentifiedBfFilterQA>(cfgc)};
  return workflow;
}
