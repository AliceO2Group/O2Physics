...
struct JetFinderV0QATask {
  ...
  bool isAcceptedJet(V const& jet)
  {
    ...
    if (leadingConstituentPtMin > -98.0) {
      ...
      for (auto& hfconstituent : jet.template v0candidates_as<U>()) {
  ...
  void fillHistograms(T const& jet, float centrality, float weight = 1.0)
  {
    ...
    if (jet.r() == round(selectedJetsRadius * 100.0f)) {
      ...
      registry.fill(HIST("h_jet_ntracks"), jet.tracksIds().size() + jet.v0candidatesIds().size(), weight);
      ...
      registry.fill(HIST("h2_centrality_jet_ntracks"), centrality, jet.tracksIds().size() + jet.v0candidatesIds().size(), weight);
    }
    ...
    registry.fill(HIST("h3_jet_r_jet_pt_jet_ntracks"), jet.r() / 100.0, jet.pt(), jet.tracksIds().size() + jet.v0candidatesIds().size(), weight);
    ...
    for (auto& v0candidate : jet.template v0candidates_as<std::decay_t<U>>()) {
  ...
  void fillMCPHistograms(T const& jet, float weight = 1.0)
  {
    ...
    if (jet.r() == round(selectedJetsRadius * 100.0f)) {
      ...
      registry.fill(HIST("h_jet_ntracks_part"), jet.tracksIds().size() + jet.v0candidatesIds().size(), weight);
    }
    ...
    registry.fill(HIST("h3_jet_r_part_jet_pt_part_jet_ntracks_part"), jet.r() / 100.0, jet.pt(), jet.tracksIds().size() + jet.v0candidatesIds().size(), weight);
