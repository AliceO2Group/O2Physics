...
// Jet-to-jet matching columns — MCD-matched & MCP-matched tables only 
...
enum BinCollCntr {
  AllCollisions = 1, //< every collision seen
  Sel8ZCut      = 2  //< after sel8 + |z| < vertexZCut
};


enum BinJetCntr {
  ChargedJets  = 1, //< all charged jets passing pT and R filters
  D0TaggedJets = 2  //< subset that contain ≥1 D0 candidate
};
...
struct JetHFAngularityTask {
  ...
  float computeLambda(JET const& jet, TRACKS const& tracks,
  ...
  /// Massless-constituent approximation: E_i = |p_i|
  ...
  float computeJetMass(TRACKS const& tracks)
  {
    ...
    for (auto const& trk : tracks) {
      ...
      sumPx += px; sumPy += py; sumPz += pz; sumE += p;
    }
    const double m2 = sumE * sumE
    ...
    aod::JetTracks const&)
  {
    ...
    for (const auto& jet : jets) {
      ...
      //Note: computing for everyone but filling the histogram which has d0 

      const int   nConst   = static_cast<int>(jetTracks.size());
      const float angularity = computeLambda(jet, jetTracks, 1.f, 1.f); 
      const float girth      = computeLambda(jet, jetTracks, 2.f, 1.f); 
      ...
      //  D0 candidate loop 
      for (const auto& d0Candidate : jet.template candidates_as<D0CandidatesData>()) {
  ...
  void processMCDChargedSubstructure(
    ...
    aod::JetTracks const&)
  {
    //  Event selection (same sel8 + |z| logic as DATA) 
    if (!jetderiveddatautilities::selectCollision(collision, eventSelectionBits)) {
      return;
    }
    ...
    for (const auto& jet : mcdjets) {
      ...
      for (const auto& d0Candidate : jet.template candidates_as<D0CandidatesMCD>()) {
    ...
    aod::JetParticles const&)
  {
    ...
    for (const auto& jet : mcdjets) {
      ...
      //  Geometric match: do the jet AXES line up? 
      ...
      if (jet.has_matchedJetGeo()) {
        ...
        for (const auto& mcpjet : jet.template matchedJetGeo_as<D0MCPJetsMatched>()) {
          ...
          if (!isGeoMatched) {
            ...
            matchedAngularity    = computeLambda(mcpjet, mcpjetParticles, 1.f, 1.f); 
            LOG(info)
    		<< "matched pt = " << mcpjet.pt()
    		<< " n particles = " << mcpjetParticles.size()
    		<< " matchedAngularity = " << matchedAngularity;            
          }
        }
      }
      registry.fill(HIST("h_jet_matching_ngeo_mcd"), nGeoMatches, mcWeight);

      // Candidate-based match: did the SAME underlying D0 seed both
      // the detector jet and a particle-level jet? 
      ...
      // Production response matrix / ΔR: only the single best (first) clean
      // match per jet 
      if (isCleanMatched) {
      ...
      for (const auto& d0Candidate : jet.template candidates_as<D0CandidatesMCD>()) {
        ...
        if (isCleanMatched) {
          LOG(info)
        	<< "Reco angularity = " << angularity
        	<< " Truth angularity = " << matchedAngularity;        
          if (originMc == 1) {
    ...
    aod::JetParticles const&)
  {
    ...
    for (const auto& jet : mcpjets) {
      ...
      const int   nConst     = static_cast<int>(jetParticles.size());
      const float angularity = computeLambda(jet, jetParticles, 1.f, 1.f); // λ_1^1
      const float girth      = computeLambda(jet, jetParticles, 2.f, 1.f); // λ_2^1
      const float mjet       = computeJetMass(jetParticles);
      ...
      // ---- Generated D0 loop 
      for (const auto& d0Particle : jet.template candidates_as<D0CandidatesMCP>()) {
        ...
        const int8_t originMc    = d0Particle.originMcGen(); 
