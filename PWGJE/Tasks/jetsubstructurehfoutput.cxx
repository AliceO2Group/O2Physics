    ...
    for (const auto& jet : jets) {
      ...
      for (const auto& jetRadiiValue : jetRadiiValues) {
        if (jet.r() == round(jetRadiiValue * 100.0f)) {
          ...
          auto candidateCollision = jethfutilities::getCandidateCollision<typename CandidateTable::iterator>(candidate, candidateCollisions); // this is getting the wrong object
          if (nJetInCollision == 0) {
