...
struct ChJetTriggerQATask {
  ...
  {
    fiducialVolume = ((float)cfgTPCVolume) - ((float)cfgJetR);
  }

  // declare filters on collisions
  Filter collisionFilter = (nabs(aod::collision::posZ) < ((float)cfgVertexCut));

  // declare filters on tracks
  Filter trackFilter = (nabs(aod::track::eta) < ((float)cfgTPCVolume)) && (aod::track::phi > ((float)cfgTrackPhiMinCut)) && (aod::track::phi < ((float)cfgTrackPhiMaxCut)) && (aod::track::pt > ((float)cfgJetPtMin));
            ...
            soa::Filtered<TrackCandidates> const& tracks, filteredJets const& jets)
  {
    ...
    if (((bool)cfgEventSel8) && !collision.sel8()) {
      return;
    }

    if (collision.hasJetChHighPt() >= ((int)bTriggerDecision)) {
      ...
      for (auto& jet : jets) {
        if (fabs(jet.eta()) < ((float)cfgTPCVolume)) {
