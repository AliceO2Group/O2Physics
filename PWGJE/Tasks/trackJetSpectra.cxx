...
struct TrackJetSpectra {
                      ...
                      
  {
    ...
    if (analyzeSignal==0&&refTrackPhi.size()>0){
        ...
        spectra.fill(HIST("hPocetReferenceTriggeru"), (float)refTrackPhi.size(), weight); 
        ...
        spectra.fill(HIST("hPocetSignalTriggeru"), (float)sigTrackPhi.size(), weight);
    ...
    while(bCloseJet){
       ...
       if(dist1 > randomConeDistanceFactor * jetR &&
          ...
          distTT > randomConeDistanceFactor * jetR) bCloseJet = false;
    }
                             ...
                             float weight = 1.)
  {
     ...
     for (const auto& track : tracks) {
        if (skipTrack(track)) continue;
        if((signalTriggerMin<=track.pt())&&(track.pt()<=signalTriggerMax))   sigTrackPhi.push_back(track.phi());
     }   
        
     bool bIsThereTTSig = sigTrackPhi.size() > 0;

    if (bIsThereTTSig) phiTT = sigTrackPhi[rand->Integer(sigTrackPhi.size())];
    ...
    for (const auto& jetBase : jetsBase) {
       ...
       if(bIsThereTTSig){
          ...
          if(dPhi > constants::math::PI - dPhiCut)  bIsBaseJetRecoil = true;
       }
 ...
 //++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
  ...
  {
    // check whether event contains the required flag for event selection (sel8 in this case)
    if (skipEvent(collision)|| collision.isOutlier()) return;
 ...
 {
  if (collisionMC.isOutlier()) return;
...
PROCESS_SWITCH(TrackJetSpectra, processTrackEfficiencyMB, "process efficiency of tracks", false);
...

  {
   ...
   for (auto const& particle : mcParticles){
     ...
     if (static_cast<int8_t>(pdgParticle->Charge()) == 0) continue;
     if (!particle.isPhysicalPrimary()) continue;

     if (std::fabs(particle.eta()) > trkEtaCut) continue;
     if (particle.pt() < trkPtMin) continue;
 ...
 for (auto const& collision : collisions){
   if (skipEvent(collision)) continue;
   if (!(std::fabs(collision.posZ()) < vertexZCut)) continue;
   ...
   for (auto const& track : collTracks) {
     ...
     if (std::fabs(track.eta()) > trkEtaCut) continue;
     if (track.pt() < trkPtMin) continue;
     ...
     if (std::fabs(mcPart.eta()) > trkEtaCut) continue;
     if (mcPart.pt() < trkPtMin) continue;
...
//+++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
                      ...
                      
  {
    ...
    if (rnd < sigToRefFraction) analyzeSignal=1;
    ...
    for (const auto& particle : particles) {
      ...
      if (bParticleNeutral || !particle.isPhysicalPrimary()) continue;
    ...
    if (bIsThereMatchedJet) {
       ...
       for (const auto& jetMatched : jetsMatched) {
          ...
          if (skipMatchedDetJet) {
             ...
             if (bIsBaseJetRecoil) spectra.fill(HIST("hMissedJets_pT_RecoilJets"), partJetPt, weight);
