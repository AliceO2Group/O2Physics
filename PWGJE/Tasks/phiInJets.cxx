...
struct phiInJets {
  ...
  {       
	...
	if(mcParticle.isPhysicalPrimary() && fabs(mcParticle.y())<=0.5){ // do this in the context of the track ! (context matters!!!)
	  if(abs(mcParticle.pdgCode())==211) JEhistos.fill(HIST("ptJEHistogramPion"), mcParticle.pt());
	  if(abs(mcParticle.pdgCode())==321) JEhistos.fill(HIST("ptJEHistogramKaon"), mcParticle.pt());
	  if(abs(mcParticle.pdgCode())==2212) JEhistos.fill(HIST("ptJEHistogramProton"), mcParticle.pt());
	}
      }
    }
  ...
  void processSim(o2::aod::JMcCollision const& collision, aod::JMcParticles const& mcParticles, soa::Filtered<aod::ChargedMCParticleLevelJets> const& mcpjets )
  {
	...
	if(abs(mcParticle.pdgCode())==211) JEhistos.fill(HIST("ptGeneratedPion"), mcParticle.pt());
	if(abs(mcParticle.pdgCode())==321) JEhistos.fill(HIST("ptGeneratedKaon"), mcParticle.pt());
	if(abs(mcParticle.pdgCode())==2212) JEhistos.fill(HIST("ptGeneratedProton"), mcParticle.pt());
	...
	lResonance.SetPxPyPzE(mcParticle.px(), mcParticle.py(), mcParticle.pz(), mcParticle.e());
       	if(abs(mcParticle.pdgCode())==333) JEhistos.fill(HIST("ptGeneratedPhi"), mcParticle.pt());
	if(abs(mcParticle.pdgCode())==333) JEhistos.fill(HIST("mGeneratedPhi"), lResonance.M());	
      }
    }
    //Check jets
