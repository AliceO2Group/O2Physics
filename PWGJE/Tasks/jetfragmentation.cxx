...
struct JetFragmentation {
  ...
  void init(InitContext& initContext)
  {
    ...
    if (doprocessDataV0 || doprocessDataV0Frag) {
      registry.add("data/V0/nV0s", "nV0s", HistType::kTH1F, {trackCount});
  ...
  {
    // TODO: For now, don't apply cuts and always assume Lambda0
    double ctauLambda = v0.distovertotmom(collision.posX(), collision.posY(), collision.posZ()) * o2::constants::physics::MassLambda0;
  ...
  void processMcV0(soa::Join<aod::Collisions, aod::McCollisionLabels>::iterator const& collision,
                   ...
                   //  soa::Filtered<soa::Join<aod::Tracks, aod::TracksExtra, aod::TracksDCA, aod::TrackSelection, aod::McTrackLabels>> const& tracks,
  ...
  void processDataV0(aod::Collisions::iterator const& collision,
                     aod::V0Datas const& V0s,
                     soa::Join<aod::Tracks, aod::TracksExtra, aod::TracksDCA, aod::TrackSelection> const& tracks)
  {
    registry.fill(HIST("data/V0/nV0s"), V0s.size());
    fillDataV0Histograms(collision, V0s, tracks);
  }
  PROCESS_SWITCH(JetFragmentation, processDataV0, "Data V0", false);

  void processDataV0Frag(aod::Collisions::iterator const& collision,
                         soa::Join<aod::ChargedJets, aod::ChargedJetConstituents> const& jets,
                         aod::V0Datas const& V0s,
                         soa::Join<aod::Tracks, aod::TracksExtra, aod::TracksDCA, aod::TrackSelection> const& tracks)
  {
    ...
    registry.fill(HIST("data/V0/nV0s"), nV0s);

    for (const auto& jet : jets) {
      fillDataRun3Histograms(jet);
      int iv0 = 0;
      // Loop over V0s to correct jet momentum
      ...
      //   dR = TMath::Sqrt(dR);
      //   if (dR < jet.r() * 1e-2) {
      //     // Correct jet pt, depending on daughters
      //   }
      // }
