...
struct JetChargedV2 {
      ...
      for (auto const& jet : jets) {
        ...
        if (nmode == cfgNmodA) {
          ...
          if ((phiMinusPsi2 < o2::constants::math::PIQuarter) || (phiMinusPsi2 >= evtPlnAngleA * o2::constants::math::PIQuarter) || (phiMinusPsi2 >= evtPlnAngleB * o2::constants::math::PIQuarter && phiMinusPsi2 < evtPlnAngleC * o2::constants::math::PIQuarter)) {
            ...
            registry.fill(HIST("h2_mcp_centrality_jet_pt_in_plane_v2_rho"), collision.centrality(), jet.pt(), weight);
          } else {
            registry.fill(HIST("h_mcp_jet_pt_out_of_plane_v2_rho"), jet.pt(), weight);
            registry.fill(HIST("h2_mcp_centrality_jet_pt_out_of_plane_v2_rho"), collision.centrality(), jet.pt(), weight);
          }
        } else if (nmode == cfgNmodB) {
          double phiMinusPsi3;
          if (collision.qvecAmp()[detId] < collQvecAmpDetId) {
          ...
          if ((phiMinusPsi3 < o2::constants::math::PIQuarter) || (phiMinusPsi3 >= evtPlnAngleA * o2::constants::math::PIQuarter) || (phiMinusPsi3 >= evtPlnAngleB * o2::constants::math::PIQuarter && phiMinusPsi3 < evtPlnAngleC * o2::constants::math::PIQuarter)) {
            ...
            registry.fill(HIST("h2_mcp_centrality_jet_pt_in_plane_v3_rho"), collision.centrality(), jet.pt(), weight);
          } else {
            registry.fill(HIST("h_mcp_jet_pt_out_of_plane_v3_rho"), jet.pt(), weight);
            registry.fill(HIST("h2_mcp_centrality_jet_pt_out_of_plane_v3_rho"), collision.centrality(), jet.pt(), weight);
          }
        }
      }
    }
  ...
  void fillGeoMatchedCorrHistograms(TBase const& jetMCD, TF1* fFitModulationRM, float tempparaA, double ep2, float rho, bool subtractMCPBackground, float mcrho = 0.0, float weight = 1.0)
  {
    float pTHat = 10. / (std::pow(weight, 1.0 / pTHatExponent));
  ...
  void processSigmaPtAreaSubMCP(McParticleCollision::iterator const& mccollision,
                         ...
                         aod::JetParticles const&)
  {
    ...
    for (auto const& collision : collisions) {
      fitFncMCP(collision, jets, mcLevelIsParticleLevel);
    }
    evtnum += 1;
  }
                          ...
                          aod::JetTracks const& tracks, aod::JetParticles const&)
  {
    ...
    for (const auto& collision : collisions) {
      ...
      for (const auto& mcdjet : collmcdjets) {
        ...
        fillGeoMatchedCorrHistograms<ChargedMCDMatchedJets::iterator, ChargedMCPMatchedJets>(mcdjet, fFitModulationRM, tempparaA, ep2, collision.rho(), mcrho, subtractMCPBackground);
