...
struct JetChargedV2 {
  ...
  void fillGeoMatchedCorrHistograms(TBase const& jetMCD, TF1* fFitModulationRM, float tempparaA, double ep2, float rho, float cent, float mcrho = 0.0, float weight = 1.0)
  {
    float pTHat = 10. / (std::pow(weight, 1.0 / pTHatExponent));
                                    ...
                                    aod::JetTracks const& tracks, aod::JetParticles const&)
  {
    ...
    for (const auto& collision : collisions) {
      ...
      for (const auto& mcdjet : collmcdjets) {
        ...
        double phiMinusPsi2;
        phiMinusPsi2 = mcdjet.phi() - ep2;

        fillGeoMatchedCorrHistograms<ChargedMCDMatchedJets::iterator, ChargedMCPMatchedJets>(mcdjet, fFitModulationRM, tempparaA, ep2, collision.rho(), collision.centrality(), mcrho);
