...
void doPairMatching(T const& pairsBase, U const& pairsTag, std::vector<std::vector<int>>& baseToTagMatching, std::vector<std::vector<int>>& tagToBaseMatching, V const& /*candidatesBase*/, M const& tracksBase, N const& /*candidatesTag*/, O const& tracksTag)
{
  ...
  for (const auto& pairBase : pairsBase) {
    if (pairBase.template has_track1()) {
      hasTrackBase1 = true;
    }
    if (pairBase.template has_track2()) {
      hasTrackBase2 = true;
    }
    if (pairBase.template has_candidate1()) {
      hasCandidateBase1 = true;
    }
    if (pairBase.template has_candidate2()) {
    ...
    for (auto pairTagIndex : pairsTagIndices) {
      ...
      if (hasTrackBase1 && !pairTag.template has_track1()) {
        continue;
      }
      if (hasTrackBase2 && !pairTag.template has_track2()) {
        continue;
      }
      if (hasCandidateBase1 && !pairTag.template has_candidate1()) {
        continue;
      }
      if (hasCandidateBase2 && !pairTag.template has_candidate2()) {
        continue;
      }
      int nMatched = 0;
