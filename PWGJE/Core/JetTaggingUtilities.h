...
void orderForIPJetTracks(T const& collision, U const& jet, V const& /*jtracks*/, W const& /*tracks*/, float const &trackDcaXYMax, float const &trackDcaZMax, Vec& vecSignImpSig)
{
  for (auto& jtrack : jet.template tracks_as<V>()) {
    ...
    if (!trackAcceptanceWithDca(track, trackDcaXYMax, trackDcaZMax)) continue;
...
// For secaondy vertex method utilites
  class bjetCandSV {
    public:
      ...
      bjetCandSV(float xpv, float ypv, float zpv, float xsv, float ysv, float zsv,
        ...
        : m_xPVertex(xpv), m_yPVertex(ypv), m_zPVertex(zpv),
        m_xSecondaryVertex(xsv), m_ySecondaryVertex(ysv), m_zSecondaryVertex(zsv),
        m_px(pxVal), m_py(pyVal), m_pz(pzVal), m_e(eVal), m_m(mVal), m_chi2PCA(chi2Val),
        m_errorDecayLength(errDecayLength), m_errorDecayLengthXY(errDecayLengthXY),
        m_rSecondaryVertex(rSecVertex), m_pt(ptVal), m_p(pVal),
        m_pVector(pVec), m_eta(etaVal), m_phi(phiVal),
        m_y(yVal), m_decayLength(decayLen), m_decayLengthXY(decayLenXY),
        m_decayLengthNormalised(decayLenNorm), m_decayLengthXYNormalised(decayLenXYNorm),
        m_cpa(cpaVal), m_impactParameterXY(impParXY)
    {}
  ...
  bool isTaggedJetSV (T const jet, U const& /*prongs*/, float const& doXYZ=false, float const& tagPointForSV = 15.)
  {
    ...
    if (!doXYZ) {
      ...
      if (maxSxy < tagPointForSV) return false;
    } else {
      auto maxSxyz = bjetCand.decayLength() / bjetCand.errorDecayLength();
      if (maxSxyz < tagPointForSV) return false;
    }
    return true;
  }
