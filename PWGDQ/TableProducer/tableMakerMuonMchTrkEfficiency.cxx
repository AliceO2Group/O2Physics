...
struct tableMakerMuonMchTrkEfficiency {
    ...
    void init(o2::framework::InitContext&)
    {
        ...
        for(int i=0; i<6; i++)  hEvent->GetXaxis()->SetBinLabel(i+1,elabels[i]);
    ...
    void extrapolate(TLorentzVector vec, int ich, double &x, double &y) { // i = 0..9
    ...
    void FillHistos(double mEta, double mPhi, double mPt, uint16_t mchBitmap, bool isSel) {
        ...
        if(isSel) {
            ...
            if(fillBitMapCorr) registry.fill(HIST("selected/hMchBitMapEtaPtPhi"),mchBitmap,mEta,mPt,mPhi);
    ...
    void runMuonSelection(TMuons const& tracksMuon)
    {
        ...
        int nselmuons[ncuts];
        for (int i=0; i<ncuts; i++) nselmuons[i]=0;
        ...
        for (auto& muon : tracksMuon) {
            ...
            if(!isKineAcc) isMuonSelectedAny=false;
        ...
        if(nselmuons[0]>0) registry.fill(HIST("hEventCounter"),2);
        if(nselmuons[0]>1) registry.fill(HIST("hEventCounter"),3);
        if(nselmuons[0]>2) registry.fill(HIST("hEventCounter"),4);
        if(nselmuons[0]>3) registry.fill(HIST("hEventCounter"),5);
