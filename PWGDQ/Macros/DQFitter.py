...
class DQFitter:
    ...
    def FitInvMassSpectrum(self, fitRangeMin, fitRangeMax):
        ...
        # Ratio fit / data
        #m_modelcopyOfEverything = RooArgSet(pdf).snapshot(True)
        #RooArgSet *m_modelcopyOfEverything = static_cast<RooArgSet *>(RooArgSet(*m_model).snapshot(true));
        #auto &m_modelcopiedPdf = static_cast<RooAbsPdf &>((*m_modelcopyOfEverything)["m_model"]);

        #m_modelcopiedPdf = RooAbsPdf(m_modelcopyOfEverything("SUM"))
        #m_modelcopiedPdf = m_modelcopyOfEverything[3]


        #cust = RooCustomizer(pdf, "cust")
        #cust.replaceArg(sig, sigsum);
        #sclone2 = cust.build(True)
        #pt_modelobs = sclone2.getObservables(rooDs)
        #pt_modelPars = sclone2.getParameters(pt_modelobs)

        #histClone = self.fInput.Clone()
        #canvasRatio = DoRatioPlot(pdf, rooDs, self.fInput, self.fRooMass, trialName)
