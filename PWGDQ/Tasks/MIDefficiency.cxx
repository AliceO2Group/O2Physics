    ...
    void runMidEffCounters(TEvent const& event, Muons const& muons)
    {
        ...
        for (auto& track : muons) {
            ...
            for (int ich = 0; ich < 4; ++ich) {
                ...
                if (isFiredBP) histos.fill(HIST("nFiredBPperPlane"),ich); //BP - Plane
                if (isFiredNBP) histos.fill(HIST("nFiredNBPperPlane"),ich); //NBP - Plane
                if (isFiredBP && isFiredNBP) histos.fill(HIST("nFiredBothperPlane"),ich); //Both planes - plane
                ...
                if (isFiredBP) histos.fill(HIST("nFiredBPperRPC"),deId); //BP - RPC
                if (isFiredNBP) histos.fill(HIST("nFiredNBPperRPC"),deId); //NBP - RPC
                if (isFiredBP && isFiredNBP) histos.fill(HIST("nFiredBothperRPC"),deId); //Both planes - RPC
                ...
                if (isFiredBP) histos.fill(HIST("nFiredBPperBoard"),LB);
                if (isFiredNBP) histos.fill(HIST("nFiredNBPperBoard"),LB);
                if (isFiredBP && isFiredNBP) histos.fill(HIST("nFiredBothperBoard"),LB);
