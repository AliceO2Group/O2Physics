...
struct trackPIDQA {
  ...
  void processQA(soa::Join<aod::Collisions, aod::EvSels>::iterator const& collision, soa::Join<FullTracksExt, aod::V0Bits> const& tracks)
  {
    ...
    for (auto& track : tracks) {
      ...
      // if (!(track.itsClusterMap() & uint8_t(1)) && !(track.itsClusterMap() & uint8_t(2)) && !(track.itsClusterMap() & uint8_t(4))) { // ITS2 IBor
      //   continue;
      // }

      // if (track.itsNCls() < 2.9) {
      //   continue;
      // }

      // if (track.itsChi2NCl() > 36.0) {
      //   continue;
      // }
