...
void o2::aod::dqmixing::SetUpMixing(MixingHandler* mh, const char* mixingVarible)
{
  ...
  if (!nameStr.compare("Centrality1")) {
    ...
    mh->AddMixingVariable(VarManager::kCentVZERO, fCentLimsHashing.size(), fCentLimsHashing, "Cent");
  }
  ...
  if (!nameStr.compare("Vtx1")) {
    ...
    mh->AddMixingVariable(VarManager::kVtxZ, fZLimsHashing.size(), fZLimsHashing, "Z");
  }
