...
void VarManager::FillEvent(T const& event, float* values)
{
  ...
  if constexpr ((fillMap & CollisionQvect) > 0) {
    ...
    values[kQ1X0C] = -999;
    values[kQ1Y0C] = -999;
    values[kQ2X0A] = (event.qvecTPCposRe() * event.nTrkTPCpos() + event.qvecTPCnegRe() * event.nTrkTPCneg()) / (event.nTrkTPCpos() + event.nTrkTPCneg());
    values[kQ2Y0A] = (event.qvecTPCposIm() * event.nTrkTPCpos() + event.qvecTPCnegIm() * event.nTrkTPCneg()) / (event.nTrkTPCpos() + event.nTrkTPCneg());
    ...
    values[kMultA] = event.nTrkTPCpos() + event.nTrkTPCneg();
...
void VarManager::FillQVectorFromCentralFW(C const& collision, float* values)
{
  ...
  float xQVecTPCpos = collision.qvecTPCposRe(); // already normalised
  float yQVecTPCpos = collision.qvecTPCposIm(); // already normalised
  float xQVecTPCneg = collision.qvecTPCnegRe(); // already normalised
  float yQVecTPCneg = collision.qvecTPCnegIm(); // already normalised

  values[kQ2X0A] = (collision.nTrkTPCpos() * xQVecTPCpos + collision.nTrkTPCneg() * xQVecTPCneg) / (collision.nTrkTPCpos() + collision.nTrkTPCneg());
  values[kQ2Y0A] = (collision.nTrkTPCpos() * yQVecTPCpos + collision.nTrkTPCneg() * yQVecTPCneg) / (collision.nTrkTPCpos() + collision.nTrkTPCneg());
  values[kQ2X0APOS] = xQVecTPCpos;
  values[kQ2Y0APOS] = yQVecTPCpos;
  values[kQ2X0ANEG] = xQVecTPCneg;
  values[kQ2Y0ANEG] = yQVecTPCneg;
  ...
  values[kR2SP_AC] = (values[kQ2X0A] * values[kQ2X0C] + values[kQ2Y0A] * values[kQ2Y0C]);
  values[kR2SP_BC] = (values[kQ2X0B] * values[kQ2X0C] + values[kQ2Y0B] * values[kQ2Y0C]);
  values[kR2SP_FT0CTPCPOS] = (xQVecFT0c * xQVecTPCpos + yQVecFT0c * yQVecTPCpos);
  values[kR2SP_FT0CTPCNEG] = (xQVecFT0c * xQVecTPCneg + yQVecFT0c * yQVecTPCneg);
  values[kR2SP_FT0ATPCPOS] = (xQVecFT0a * xQVecTPCpos + yQVecFT0a * yQVecTPCpos);
  values[kR2SP_FT0ATPCNEG] = (xQVecFT0a * xQVecTPCneg + yQVecFT0a * yQVecTPCneg);
  values[kR2SP_FT0MTPCPOS] = (xQVecFT0m * xQVecTPCpos + yQVecFT0m * yQVecTPCpos);
  values[kR2SP_FT0MTPCNEG] = (xQVecFT0m * xQVecTPCneg + yQVecFT0m * yQVecTPCneg);
  values[kR2SP_FV0ATPCPOS] = (xQVecFV0a * xQVecTPCpos + yQVecFV0a * yQVecTPCpos);
  values[kR2SP_FV0ATPCNEG] = (xQVecFV0a * xQVecTPCneg + yQVecFV0a * yQVecTPCneg);
  ...
  float epTPCposs = epHelper.GetEventPlane(xQVecTPCpos, yQVecTPCpos, 2);
  float epTPCnegs = epHelper.GetEventPlane(xQVecTPCneg, yQVecTPCneg, 2);
  ...
  values[kR2EP_FT0CTPCPOS] = std::cos(2 * getDeltaPsiInRange(epFT0c, epTPCposs, 2));
  values[kR2EP_FT0CTPCNEG] = std::cos(2 * getDeltaPsiInRange(epFT0c, epTPCnegs, 2));
  values[kR2EP_FT0ATPCPOS] = std::cos(2 * getDeltaPsiInRange(epFT0a, epTPCposs, 2));
  values[kR2EP_FT0ATPCNEG] = std::cos(2 * getDeltaPsiInRange(epFT0a, epTPCnegs, 2));
  values[kR2EP_FT0MTPCPOS] = std::cos(2 * getDeltaPsiInRange(epFT0m, epTPCposs, 2));
  values[kR2EP_FT0MTPCNEG] = std::cos(2 * getDeltaPsiInRange(epFT0m, epTPCnegs, 2));
  values[kR2EP_FV0ATPCPOS] = std::cos(2 * getDeltaPsiInRange(epFV0a, epTPCposs, 2));
  values[kR2EP_FV0ATPCNEG] = std::cos(2 * getDeltaPsiInRange(epFV0a, epTPCnegs, 2));
}
