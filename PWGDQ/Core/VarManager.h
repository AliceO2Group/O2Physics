...
void VarManager::FillPairME(T1 const& t1, T2 const& t2, float* values)
{
  ...
  if constexpr ((fillMap & ReducedEventQvector) > 0 || (fillMap & CollisionQvect) > 0) {
    ...
    values[kWV22ME] = (std::isnan(V22ME) || std::isinf(V22ME) || std::isnan(V24ME) || std::isinf(V24ME) || values[kV22m] < -900 || values[kV22p] < -900) ? 0. : 1.0;
    values[kV24ME] = (std::isnan(V22ME) || std::isinf(V22ME) || std::isnan(V24ME) || std::isinf(V24ME)) ? 0. : V24ME;
    values[kWV24ME] = (std::isnan(V22ME) || std::isinf(V22ME) || std::isnan(V24ME) || std::isinf(V24ME) || values[kV24m] < -900 || values[kV24p] < -900) ? 0. : 1.0;
