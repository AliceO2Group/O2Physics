...
TString histNames[nHists] = {"", "_Pi0", "_Eta", "_EtaP", "_Rho", "_Omega", "_Phi"};
TString histLegends[nHists] = {"Cocktail sum", "#pi^{0}#rightarrow#gammae^{+}e^{-}", "#eta#rightarrow#gammae^{+}e^{-}", "#eta'#rightarrow#gammae^{+}e^{-}, #eta'#rightarrow#omegae^{+}e^{-}", "#rho#rightarrowe^{+}e^{-}", "#omega#rightarrow#pi^{0}e^{+}e^{-}, #omega#rightarrowe^{+}e^{-}", "#phi#rightarrow#etae^{+}e^{-}, #phi#rightarrow#pi^{0}e^{+}e^{-}, #phi#rightarrowe^{+}e^{-}"};

void loadHistos(TFile* file, TH1F* hists[], TString name_extra, int rebin, int nEvents)
{
  for (int i = 0; i < nHists; i++) {
    hists[i] = (TH1F*)file->GetDirectory("em-lmee-lf-cocktail")->Get(TString("mee") + name_extra + histNames[i]);
    hists[i]->Rebin(rebin);
...
void plotLFCocktail(TString filename = "AnalysisResults.root", int rebin = 1)
{
  ...
  TH1I* hNEvents = (TH1I*)file->GetDirectory("em-lmee-lf-cocktail")->Get("NEvents");
